...
$smarty->display('carga_val_2463_ERC.html.tpl');
...
if(isset($_POST["accion"]) && $_POST["accion"]=="validar" && isset($_FILES["2463_ERC_file"]) && $_FILES["2463_ERC_file"]["error"]==0)
{
	...
	//creacion archivo separado creatinina
	$ruta_registros_creatinina=$rutaTemporal."_RegFechaCreatininaNoVigente".$fecha_actual."_".$tiempo_actual_string.".txt";
	if(file_exists($ruta_registros_creatinina))
	{
		unlink($ruta_registros_creatinina);
	}
	$file_archivo_registros_creatinina = fopen($ruta_registros_creatinina, "w") or die("fallo la creacion del archivo");
	$titulo_rc="REGISTROS FECHA CREATININA NO VIGENTE PARA ESTADIO ";
	fwrite($file_archivo_registros_creatinina,$titulo_rc);
	
	//creacion archivo separado inconsistencias registros malos
	$ruta_inconsistencias_registros_malos=$rutaTemporal."inconsERC_RM_".$fecha_actual."_".$tiempo_actual_string.".csv";
	...
	$ruta_archivo_inconsistencias_ERC=$rutaTemporal."inconsERC_RT_".$fecha_actual."_".$tiempo_actual_string.".csv";
	if(file_exists($ruta_archivo_inconsistencias_ERC))
	{
		unlink($ruta_archivo_inconsistencias_ERC);
	}
	$file_inconsistencias_r4725_ERC = fopen($ruta_archivo_inconsistencias_ERC, "w") or die("fallo la creacion del archivo");
	$titulos="";
	$titulos.="consecutivo,nombre archivo,codigo tipo inconsistencia,tipo inconsistencia,codigo grupo inconsistencia,grupo inconsistencia,";
	$titulos.="codigo detalle inconsistencia,detalle inconsistencia, numero de linea, numero de campo, tipo identificacion, numero identificacion";
	//PARTE REESCRIBE TITULOS INCONSISTENCIAS PARA CSV SIMPLE DE ABRIR
    ...
    $error_titulos_procesado_csv_excel.="=\"".implode("\";=\"", $columnas_titulos_inconsistencias_para_bd)."\"";
    //FIN PARTE REESCRIBE TITULOS INCONSISTENCIAS PARA CSV SIMPLE DE ABRIR
	fwrite($file_inconsistencias_r4725_ERC,$error_titulos_procesado_csv_excel."\n");
	...
	if ($archivo_fuente_after_post['size'] > 250000000)
	{
	...
	else
	{
		...
		{
			$ruta_archivo_erc = $rutaTemporal.$archivo_fuente_after_post['name'];
			move_uploaded_file($archivo_fuente_after_post['tmp_name'], $ruta_archivo_erc);
			
			$array_nombre_sin_sigla=explode("ERC",$archivo_fuente_after_post['name']);
			if(count($array_nombre_sin_sigla)!=2)
			{
				$es_valido_nombre_archivo=false;
				$errores.="El encabezado del archivo $nombre_archivo_file no corresponde a un archivo ERC. <br>";
			}
			else
			{
				...
				/*para agrupado remover de las tablas gioss_tabla_estado_informacion_r2463_erc y gioss_numero_de_secuencia_archivos_erc las condiciones de llave foranea para prestador
				ALTER TABLE gioss_numero_de_secuencia_archivos_erc DROP CONSTRAINT gioss_numero_de_secuencia_arc_codigo_prestador_servicios_s_fkey;
				ALTER TABLE gioss_tabla_estado_informacion_r2463_erc DROP CONSTRAINT gioss_tabla_estado_informacion_codigo_prestador_servicios_fkey2;
				ALTER TABLE gioss_tabla_registros_cargados_exito_r2463_erc DROP CONSTRAINT gioss_tabla_registros_cargado_codigo_prestador_reportante_fkey2;
				ALTER TABLE gioss_tabla_registros_no_cargados_rechazados_r2463_erc DROP CONSTRAINT gioss_tabla_registros_no_carg_codigo_prestador_reportante_fkey2;
				ALTER TABLE gioss_tabla_consolidacion_registros_validados_r2463_erc DROP CONSTRAINT gioss_tabla_consolidacion_regi_codigo_entidad_reportadora_fkey2;
				*/
	...
	//VERIFICA SI EL ARCHIVO YA ESTA SIENDO VALIDADO ACTUALMENTE
        ...
        $query_verificacion_esta_siendo_procesado.=" SELECT * FROM gioss_2463_esta_validando_actualmente ";
	...
	{
		$sql_query_delete="";
		$sql_query_delete.=" DELETE FROM gioss_tabla_registros_no_cargados_rechazados_r2463_erc ";
		...
		$numero_secuencia_actual=$utilidades->obtenerSecuencia("gioss_numero_secuencia_r2463_erc");
					
		$sql_query_inserta_seq="";
		$sql_query_inserta_seq.=" INSERT INTO gioss_numero_de_secuencia_archivos_erc ";
		$sql_query_inserta_seq.=" ( ";
		...
		$query_insert_esta_siendo_procesado.=" INSERT INTO gioss_2463_esta_validando_actualmente ";
		$query_insert_esta_siendo_procesado.=" ( ";
		...
		// parte donde valida los campos del archivo ERC 
		...
		{
			$mensaje_errores_ERC="";
			$lineas_del_archivo = count(file($ruta_archivo_erc)); 
			...
			while (!feof($file_ERC)) 
			{
				...
				$verificar_si_ejecucion_fue_cancelada.=" SELECT esta_ejecutando FROM gioss_2463_esta_validando_actualmente ";
				...
				if(count($campos)==119)
				{
					...
					if($muestra_mensaje_nuevo)
					{
						...
						$mensaje_contador_errores="revisando linea ".($nlinea+1)." de $lineas_del_archivo del archivo ERC. <br> registros buenos $registros_buenos , registros malos $registros_malos .<br>Porcentaje actual $porcentaje %  ";
					$html_del_mensaje="";
					...
					*/
					...
					$query_update_esta_siendo_procesado.=" UPDATE gioss_2463_esta_validando_actualmente ";
					...
					//PARTE VALIDACION ERC AFILIADO ESTA EN BD
					...
					if($si_existe==true)
					{
					//validar_ERC($campos,$nlinea,&$consecutivo_errores,$array_tipo_validacion,$array_grupo_validacion,$array_detalle_validacion,$nombre_archivo,$fecha_remision,$fecha_de_corte,$cod_prestador,$cod_eapb)
					$array_resultados_validacion=validar_ERC($campos,
										 $nlinea,
										 $consecutivo_errores,
					...
					if($array_resultados_validacion["error"]==false)
					{
						...
						if($se_creo_tabla_indice==false)
						{
							...
							$sql_insertar_en_tabla_indice_analisis_coherencia.="insert into gioss_indice_archivo_para_analisis_2463";
							$sql_insertar_en_tabla_indice_analisis_coherencia.="(";
						...
						if($se_creo_tabla_indice==true)
						{
							...
							$sql_insertar_en_tabla_analisis_coherencia.="insert into gioss_archivo_para_analisis_2463";
							$sql_insertar_en_tabla_analisis_coherencia.="(";
							...
							$cont_campo_ins=0;
							while($cont_campo_ins<119)
							{
								
								$sql_insertar_en_tabla_analisis_coherencia.="campo_erc_de_numero_orden_".$cont_campo_ins.",";
					...
					//PARTE ESCRIBE LOS QUE NO TIENEN FECHA DE CREATININA VIGENTE PARA ESTADIO
						...
						{
								//parte calculo campo 27.1 con la fecha de corte
								...
								//fin parte calculo campo 27.1 con la fecha de corte
								
								//inferior en 1 year fecha de corte
								if($campo_n27_1!="1800-01-01" && $campo_n27_1!="1845-01-01"
								   && floatval($campo_n35)>=60
								   && $campo_n35!="999"
								   && $campo_n35!="988"
								   && $verificador_1_year>=0)
								{
									fwrite($file_archivo_registros_creatinina,"\n".trim($linea_mod_depende_afiliados));
								}//fin if
						...
						}//fin else campos noestan en blanco
					//FIN ESCRIBE LOS QUE NO TIENEN FECHA DE CREATININA VIGENTE PARA ESTADIO
					
					$query_insertar_campos_a_bd="";
					$query_insertar_campos_a_bd.="INSERT INTO gioss_tabla_registros_no_cargados_rechazados_r2463_erc ";
					$query_insertar_campos_a_bd.="(";				
					$cont_campo_ins=0;
					while($cont_campo_ins<119)
					{
						$query_insertar_campos_a_bd.="campo_erc_de_numero_orden_".$cont_campo_ins.",";
						$cont_campo_ins++;
					}
					...
					foreach($array_mensajes_errores_campos as $msg_error)
					{
						...
						if($msg_error!="")
						{
							...
							fwrite($file_inconsistencias_r4725_ERC, $error_procesado_csv_excel."\n");
						...
						if(count($columnas_inconsistencias_para_bd)==10)
						{
							...
							$sql_insertar_inconsistencia_erc="";
							$sql_insertar_inconsistencia_erc.=" INSERT INTO gioss_reporte_inconsistencias_r2463_erc ";
							$sql_insertar_inconsistencia_erc.=" ( ";
							$sql_insertar_inconsistencia_erc.=" numero_orden, ";
							$sql_insertar_inconsistencia_erc.=" nombre_archivo, ";
							$sql_insertar_inconsistencia_erc.=" cod_tipo_inconsitencia, ";
							$sql_insertar_inconsistencia_erc.=" nombre_tipo_inconsistencia, ";
							$sql_insertar_inconsistencia_erc.=" cod_grupo_inconsistencia, ";
							$sql_insertar_inconsistencia_erc.=" nombre_grupo_inconsistencia, ";
							$sql_insertar_inconsistencia_erc.=" cod_detalle_inconsistencia, ";
							$sql_insertar_inconsistencia_erc.=" detalle_inconsistencia, ";
							$sql_insertar_inconsistencia_erc.=" numero_linea, ";
							$sql_insertar_inconsistencia_erc.=" numero_campo, ";
							$sql_insertar_inconsistencia_erc.=" fecha_validacion, ";
							$sql_insertar_inconsistencia_erc.=" hora_validacion ";
							$sql_insertar_inconsistencia_erc.=" ) ";
							$sql_insertar_inconsistencia_erc.=" VALUES ";
							$sql_insertar_inconsistencia_erc.=" ( ";
							$sql_insertar_inconsistencia_erc.=" '".$numero_secuencia_actual."', ";
							$sql_insertar_inconsistencia_erc.=" '".$nombre_archivo_registrado."', ";
							$sql_insertar_inconsistencia_erc.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[2]) )."', ";
							$sql_insertar_inconsistencia_erc.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[3]) )."', ";
							$sql_insertar_inconsistencia_erc.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[4]) )."', ";
							$sql_insertar_inconsistencia_erc.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[5]) )."', ";
							$sql_insertar_inconsistencia_erc.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[6]) )."', ";
							$sql_insertar_inconsistencia_erc.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[7]) )."', ";
							$sql_insertar_inconsistencia_erc.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[8]) )."', ";
							$sql_insertar_inconsistencia_erc.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[9]) )."', ";
							$sql_insertar_inconsistencia_erc.=" '".$fecha_actual."', ";
							$sql_insertar_inconsistencia_erc.=" '".$tiempo_actual."' ";
							$sql_insertar_inconsistencia_erc.=" ); ";
							$error_bd_ins="";
							$bool_hubo_error_query=$coneccionBD->insertar_no_warning_get_error_no_crea_cierra($sql_insertar_inconsistencia_erc, $error_bd_ins);
							if($error_bd_ins!="")
							{
					...
					if(count($campos)>=119)
				    {
					    try
					    {
							...
							FROM gioss_index_reporte_ips_2463 
							WHERE 
							...
							$sql_update_en_reporte_ips.="UPDATE gioss_index_reporte_ips_2463 SET ";
							...
							if($filas_afectadas==0)
							{
								...
								$sql_insertar_en_reporte_ips.="insert into gioss_index_reporte_ips_2463";
								$sql_insertar_en_reporte_ips.="(";
				...
				else
				{
					...
					$mensaje_contador_errores="revisando linea ".($nlinea+1)." de $lineas_del_archivo del archivo ERC ";
					$html_del_mensaje="";
					$html_del_mensaje.="<table>";
					$html_del_mensaje.="<tr>";
					$html_del_mensaje.="<td colspan=\'2\'>";
		...
		fclose($file_inconsistencias_r4725_ERC);
		fclose($file_archivo_registros_buenos);
		fclose($file_archivo_registros_malos);
		fclose($file_archivo_registros_creatinina);
		...
		if($es_valido_nombre_archivo)
		{
			...
			if($hubo_inconsistencias_en_ERC==false)
			{
				...
				$sql_query_delete="";
				$sql_query_delete.=" DELETE FROM gioss_tabla_registros_cargados_exito_r2463_erc ";
				$sql_query_delete.=" WHERE fecha_corte='".$fecha_de_corte."' ";
				$sql_query_delete.=" AND codigo_eapb_a_reportar='".$cod_eapb."' ";
				$sql_query_delete.=" AND codigo_prestador_reportante='".$cod_prestador."' ";
				$sql_query_delete.=" AND nombre_archivo='".$nombre_archivo_registrado."' ; ";
				...
				while (!feof($file_ERC)) 
				{
					...
					{
						$query_insertar_campos_a_bd="";
						$query_insertar_campos_a_bd.="INSERT INTO gioss_tabla_registros_cargados_exito_r2463_erc ";
						$query_insertar_campos_a_bd.="(";
						...
						$mensaje_contador_errores="Subiendo a cargados con exito el registro ".($nlinea+1)." de $lineas_del_archivo del archivo ERC ";
						$html_del_mensaje="";
						$html_del_mensaje.="<table>";
						$html_del_mensaje.="<tr>";
						$html_del_mensaje.="<td colspan=\'2\'>";
				...
				$query_registrar_resumen_cargado_con_exito.="INSERT INTO gioss_resumen_cargados_exito_2463 ";
				$query_registrar_resumen_cargado_con_exito.="(";
			...
			//SUBIR A TABLA CONSOLIDADOS PARA ERC
			$estado_validacion_erc=2;
			if($hubo_inconsistencias_en_ERC==false)
			{
				$estado_validacion_erc=1;
			}
			...
			$query_delete_estado_validacion.="DELETE FROM gioss_tabla_consolidacion_registros_validados_r2463_erc ";
			...
			$query_registrar_estado_validacion.="INSERT INTO gioss_tabla_consolidacion_registros_validados_r2463_erc ";
			$query_registrar_estado_validacion.="(";
			...
			$query_registrar_estado_validacion.="(";
			$query_registrar_estado_validacion.="'".$estado_validacion_erc."',";
			...
			//FIN SUBIR A TABLA CONSOLIDADOS PARA ERC
			
			
			//SUBIR A TABLA DE ESTADO INFORMACION PARA ERC
			$estado_validacion_erc=2;
			if($hubo_inconsistencias_en_ERC==false)
			{
				$estado_validacion_erc=1;
			...
			$query_descripcion_estado_validacion.=" SELECT * FROM gioss_estado_validacion_archivos WHERE codigo_estado_validacion='$estado_validacion_erc' ; ";
			...
			$query_delete_estado_informacion.=" DELETE FROM gioss_tabla_estado_informacion_r2463_erc ";
			...
			$query_registrar_estado_informacion.="INSERT INTO gioss_tabla_estado_informacion_r2463_erc ";
			$query_registrar_estado_informacion.="(";
			...
			$query_registrar_estado_informacion.="(";
			$query_registrar_estado_informacion.="'".$estado_validacion_erc."',";//1
			...
			//FIN SUBIR A TABLA DE ESTADO INFORMACION PARA ERC
			
			
			
			if(connection_aborted()==false)
			{
				echo "<script>document.getElementById('mensaje').innerHTML='Se ha terminado de revisar y validar el archivo ERC <br> registros buenos $registros_buenos , registros malos $registros_malos';</script>";
				ob_flush();
				flush();
			}
		...
		if($hubo_inconsistencias_en_ERC)
		{
			...
			$errores.="Hubo inconsistencias en el archivo ERC.<br>";
		...
		if($numero_registros_para_reporte_por_ips>0)
		{
			...
			$parte_inicial_reporte_registros_por_ips.="RESOLUCION 2463\n";
		...
		$parte_inicial_reporte_evaluacion_resultado_validacion.="Resolucion 2463\n";
		...
		$archivos_a_comprimir[0]=$ruta_archivo_inconsistencias_ERC;
		...
		$archivos_a_comprimir[4]=$ruta_registros_creatinina;
		...
		$ruta_zip=$rutaTemporal."iERC_".$nombre_archivo_registrado."_".$fecha_actual."_".$tiempo_actual_string.'.zip';
		...
		//BOTONES DESCARGA
		$botones="";
		$botones.=" <input type=\'button\' value=\'Descargar archivo de inconsistencias para ERC\'  class=\'btn btn-success color_boton\' onclick=\"download_inconsistencias_campos(\'$ruta_zip\');\"/> ";
		...
		try
		{
			...
			{	
				...
				$mail->FromName = "GIOSS";
				$mail->Subject = "Inconsistencias ERC 2463 ";
