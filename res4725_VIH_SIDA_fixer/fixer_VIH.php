...
//funciones migradas desde reparador
...
//SELECTOR PERIODO
$query_periodos_rips="SELECT * FROM gioss_periodo_reporte_r4725_sida_vih ORDER BY codigo_periodo;";
...
$smarty->display('fixer_VIH.html.tpl');
...
$query3_detalle_validacion="SELECT * FROM gioss_detalle_inconsistencia_r4725_sida_vih;";
...
$ruta_archivo_inconsistencias_vih="";
...
if(isset($_POST["accion"]) && $_POST["accion"]=="validar" && isset($_FILES["r4725_file"]) && $_FILES["r4725_file"]["error"]==0)
{
	...
	$nombre_archivo_registrado=explode(".",$_FILES["r4725_file"]["name"])[0];	
	$archivo_sida=$_FILES["r4725_file"];
	//$cod_prestador=$_POST["prestador"];
	$cod_eapb=$_POST["eapb"];
	...
	$ruta_carpeta_del_reparado=$rutaTemporal."rVIH_".$nombre_archivo_registrado."_".$fecha_actual."_".$tiempo_actual_string;
	...
	$ruta_archivo_inconsistencias_vih=$ruta_carpeta_del_reparado."/"."iVIH_".$nombre_archivo_registrado."_".$fecha_actual."_".$tiempo_actual_string.".csv";
	if(file_exists($ruta_archivo_inconsistencias_vih))
	{
		unlink($ruta_archivo_inconsistencias_vih);
	}
	$file_inconsistencias_r4725_vih_sida = fopen($ruta_archivo_inconsistencias_vih, "w") or die("fallo la creacion del archivo");
	$titulos="";
	...
	fwrite($file_inconsistencias_r4725_vih_sida, $titulos."\n");
	...
	$ruta_archivo_reparado_VIH=$ruta_carpeta_del_reparado."/".$nombre_archivo_registrado."_dupl.txt";
	if(file_exists($ruta_archivo_reparado_VIH))
	{
		unlink($ruta_archivo_reparado_VIH);
	}
	
	$ruta_archivo_excluido_VIH=$ruta_carpeta_del_reparado."/".$nombre_archivo_registrado."_excluido.txt";
	if(file_exists($ruta_archivo_excluido_VIH))
	{
		unlink($ruta_archivo_excluido_VIH);
	}
	
	$ruta_archivo_reparado_sin_duplicados_VIH=$ruta_carpeta_del_reparado."/".$nombre_archivo_registrado.".txt";
	if(file_exists($ruta_archivo_reparado_sin_duplicados_VIH))
	{
		unlink($ruta_archivo_reparado_sin_duplicados_VIH);
	}
	
	$file_reparado_r4725_VIH = fopen($ruta_archivo_reparado_VIH, "w") or die("fallo la creacion del archivo");
	fclose($file_reparado_r4725_VIH);
	
	$file_reparado_r4725_sin_dupl_VIH = fopen($ruta_archivo_reparado_sin_duplicados_VIH, "w") or die("fallo la creacion del archivo");
	fclose($file_reparado_r4725_sin_dupl_VIH);
	...
	//PARTE VALIDACION ESTRUCTURA NOMBRE DEL ARCHIVO VIH
	$es_valido_nombre_archivo=true;
	
	$exitos="Archivo $nombre_archivo_registrado. <br>";
	
	if ($archivo_sida['size'] > 250000000)
	{
	...
	else
	{
		...
		{
			$ruta_archivo_sida = $rutaTemporal.$archivo_sida['name'];
			move_uploaded_file($archivo_sida['tmp_name'], $ruta_archivo_sida);
			
			$array_nombre_sin_sigla=explode("VIH",$archivo_sida['name']);
			if(count($array_nombre_sin_sigla)!=2)
			{
				$es_valido_nombre_archivo=false;
				$errores.="El encabezado del archivo $nombre_archivo_registrado no corresponde a un archivo VIH-SIDA. <br>";
			}
			else
			{
				...
				*/
				...
				{
					$nombre_sin_txt_para_verificacion=str_replace(".txt","",$archivo_sida['name']);
				...
				else if($tipo_entidad_que_efectua_el_cargue=="agrupado_eapb")
				{
					...
					else if(strlen($nombre_sin_txt_para_verificacion)==19)
					{
			...
			}//fin else contiene la sigla
		}//fin if nombre del archivo no es vacio
		else
		{
			$es_valido_nombre_archivo=false;
			$errores.="El nombre del archivo para VIH es invalido. <br>";
		}
	}//fin else
	//FIN PARTE VALIDACION ESTRUCTURA NOMBRE DEL ARCHIVO VIH
	
	//VERIFICA SI EL ARCHIVO YA ESTA SIENDO VALIDADO ACTUALMENTE
        $bool_esta_siendo_reparado=false;
        
        $query_verificacion_esta_siendo_procesado="";
        $query_verificacion_esta_siendo_procesado.=" SELECT * FROM gioss_4725_esta_reparando_ar_actualmente ";
	...
	$sql_query_verificar="";
	$sql_query_verificar.=" SELECT * FROM gioss_tabla_estado_informacion_r4725_sida ";
	...
	{
		//SE BORRA SI HABIA ALGO ANTES EN LA TABLA TEMPORAL DE CORREGIDOS PARA EL USUARIO no se usa
		/*
		$sql_delete_corregidos_temp="";
		$sql_delete_corregidos_temp.=" DELETE FROM corregidos_con_duplicados_sida4725 ";
		...
		$sql_delete_corregidos_temp="";
		$sql_delete_corregidos_temp.=" DELETE FROM corregidos_solo_duplicados_sida4725 ";
		$sql_delete_corregidos_temp.=" WHERE ";
		$sql_delete_corregidos_temp.=" tipo_id_usuario='$tipo_id' ";
		$sql_delete_corregidos_temp.=" AND ";
		$sql_delete_corregidos_temp.=" id_usuario='$identificacion' ";
		...
		$sql_delete_corregidos_temp="";
		$sql_delete_corregidos_temp.=" DELETE FROM corregidos_sin_duplicados_sida4725 ";
		$sql_delete_corregidos_temp.=" WHERE ";
		$sql_delete_corregidos_temp.=" tipo_id_usuario='$tipo_id' ";
		$sql_delete_corregidos_temp.=" AND ";
		$sql_delete_corregidos_temp.=" id_usuario='$identificacion' ";
		...
		*/
		...
		$query_insert_esta_siendo_procesado.=" INSERT INTO gioss_4725_esta_reparando_ar_actualmente ";
		$query_insert_esta_siendo_procesado.=" ( ";
		...
		if($error_bd!="")
		{
			if(connection_aborted()==false)
			{
				echo "<script>alert('error al iniciar el estado actual de validacion en tiempo real  4725 ');</script>";
			}
		}
		...
		$hubo_inconsistencias_en_vih=false;
		...
		{
			$mensaje_errores_vih="";
			$lineas_del_archivo = contar_lineas_archivo($ruta_archivo_sida); 
			$file_vih = fopen($ruta_archivo_sida, 'r') or exit("No se pudo abrir el archivo");
			...
			while (!feof($file_vih)) 
			{
				...
				$mensaje_estado_registros.="<tr><td style=text-align:left;width:60%>Numero de registros con numero de campos invalidos (menor o mayor de 184):</td><td style=text-align:left>".$numero_lineas_campos_incorrectos."</td></tr>";
				...
				if($muestra_mensaje_nuevo)
				{
					...
					*/
					...
					$query_update_esta_siendo_procesado.=" UPDATE gioss_4725_esta_reparando_ar_actualmente ";
					...
					if($error_bd!="")
					{
						if($fue_cerrada_la_gui==false)
						{
							echo "<script>alert('error al actualizar el estado actual de reparacion en tiempo real  4725 ');</script>";
						}
					}
					//FIN UPDATE MENSAJE
					...
					$verificar_si_ejecucion_fue_cancelada.=" SELECT esta_ejecutando FROM gioss_4725_esta_reparando_ar_actualmente ";
				...
				$linea_tmp = fgets($file_vih);
				$linea= explode("\n", $linea_tmp)[0];
				$linea=str_replace(",",".",$linea);
				$campos = explode("\t", $linea);
				
				//arreglo especial sida para poner al validar y quitar al escribir la primer columna
				array_unshift($campos,$cod_prestador);
				
				//pasa a validar los campos
				if(count($campos)==185)
				{
					$cont_pre_fix_campos=0;
					while($cont_pre_fix_campos<185)
					{
					...
					while($cont_campos_ver_vacios<185)
					{
					...
					$campo_n8_ti=preg_replace("/[^a-zA-Z0-9]+/", "", trim($campos[8]) );//TIPO IDENTIFICACION 
					$campo_n8_ti=strtoupper($campo_n8_ti);
					$campos[8]=$campo_n8_ti;//se reasigna para que quede corregido en caso de que no exista el afiliado
					$campo_n9_ni=preg_replace("/[^a-zA-Z0-9]+/", "",trim($campos[9]) );//NUMERO IDENTIFICACION
					$campos[9]=$campo_n9_ni;//se reasigna para que quede corregido en caso de que no exista el afiliado
					$tipo_de_regimen_de_la_informacion_reportada=trim($campos[2]);//REGIMEN	
					$tipo_de_regimen_de_la_informacion_reportada=strtoupper($tipo_de_regimen_de_la_informacion_reportada);
					$campos[2]=$tipo_de_regimen_de_la_informacion_reportada;
					$cod_eapb_global=$cod_eapb;//se reasigna para que quede semi corregido 
					if($tipo_de_regimen_de_la_informacion_reportada=="C" || $tipo_de_regimen_de_la_informacion_reportada=="1")
					{
					...
					if($tipo_de_regimen_de_la_informacion_reportada=="S" || $tipo_de_regimen_de_la_informacion_reportada=="2")
					{
					...
					if($tipo_de_regimen_de_la_informacion_reportada=="E" || $tipo_de_regimen_de_la_informacion_reportada=="O" || $tipo_de_regimen_de_la_informacion_reportada=="5")
					{
					...
					if($tipo_de_regimen_de_la_informacion_reportada=="P" || $tipo_de_regimen_de_la_informacion_reportada=="4")
					{
					...
					if($tipo_de_regimen_de_la_informacion_reportada=="N" || $tipo_de_regimen_de_la_informacion_reportada=="6")
					{
						...
						)
					{
						...
						//se modifica nombres						
						$campos[4]=procesar_mensaje(trim($resultados_query_existe_afiliado_tablas_regimen[0]['primer_nombre']) );
						$campos[5]=procesar_mensaje(trim($resultados_query_existe_afiliado_tablas_regimen[0]['segundo_nombre']) );
						$campos[6]=procesar_mensaje(trim($resultados_query_existe_afiliado_tablas_regimen[0]['primer_apellido']) );
						$campos[7]=procesar_mensaje(trim($resultados_query_existe_afiliado_tablas_regimen[0]['segundo_apellido']) );
					...
					$primer_nombre="".$campos[4];
					$segundo_nombre="".$campos[5];
					$primer_apellido="".$campos[6];
					$segundo_apellido="".$campos[7];


					//echo "Fecha de Corte: ".$fecha_de_corte." TI: ".$campos[8]." Numero ID ".$campos[9]." Regimen numerico VIH ".$campos[2]." Sexo ".$campos[11]." Fecha Nacimiento ".$campos[10]." primer nombre ".$campos[4]." segundo nombre ".$campos[5]." primer apellido ".$campos[6]." segundo apellido ".$campos[7]."<br>";
					...
					if($si_existe==false)
					{
						$linea_datos_afiliado_no_existe="El afiliado $campo_n8_ti $campo_n9_ni NO EXISTE";
					...
					//echo "fecha_nacimiento_a_verificar: ".$fecha_nacimiento_a_verificar."<br>";
					...
					$array_resultados_validacion=reparacion_campo_en_blanco_formato_vih($campos,
						...
						$array_resultados_validacion=reparacion_valor_permitido_formato_vih($campos,$cont_linea_para_indexador,
										 ...
										 $cod_eapb,$diccionario_identificacion,$diccionario_identificacion_lineas,
										 $coneccionBD, $array_numero_campo_bd);
					
						$array_resultados_validacion_2=reparacion_calidad_formato_vih($campos,$cont_linea_para_indexador,
										 $consecutivo_errores,$array_tipo_validacion,
										 $array_grupo_validacion,$array_detalle_validacion,
										 $nombre_archivo_registrado,$fecha_de_corte,$cod_prestador,
										 $cod_eapb,$diccionario_identificacion,$diccionario_identificacion_lineas,
					...
					if(count($campos)==185)
					{
						...
						//FASE 1 consulta por el campo 8 y 9 (tipo id,  numero id afiliado) si existe duplicado
						...
						$query_consultar_en_indexador.=" gioss_indexador_duplicados_del_reparador_4725 ";
						...
						$query_consultar_en_indexador.=" AND ";
						$query_consultar_en_indexador.="campo_sida_de_numero_orden_8_tipo_id='".$campos[8]."'";
						$query_consultar_en_indexador.=" AND ";
						$query_consultar_en_indexador.="campo_sida_de_numero_orden_9_numero_id='".$campos[9]."'";
						$query_consultar_en_indexador.=" ; ";
						$error_bd_seq="";		
						...
						if($error_bd_seq!="")
						{
						    $mensajes_error_bd.=" ERROR Al consultar en la tabla gioss_indexador_duplicados_del_reparador_4725 ".procesar_mensaje($error_bd_seq).".<br>";
						    
						    if($fue_cerrada_la_gui==false)
						    {
							    echo "<script>alert('ERROR Al consultar en la tabla gioss_indexador_duplicados_del_reparador_4725 ".procesar_mensaje($error_bd_seq)."');</script>";
						    }
						...
						if($existe_afiliado==false)
						{
						    ...
						    $query_insert_updt_en_indexador.=" gioss_indexador_duplicados_del_reparador_4725 ";				
						    $query_insert_updt_en_indexador.=" ( ";	
						    ...
						    $query_insert_updt_en_indexador.=" campo_sida_de_numero_orden_8_tipo_id, ";
						    $query_insert_updt_en_indexador.=" campo_sida_de_numero_orden_9_numero_id, ";
						    ...
						    $query_insert_updt_en_indexador.="'".$campos[8]."',";
						    $query_insert_updt_en_indexador.="'".$campos[9]."',";
						    ...
						    if($error_bd_seq!="")
						    {
							$mensajes_error_bd.=" ERROR Al subir en la tabla gioss_indexador_duplicados_del_reparador_4725 ".procesar_mensaje($error_bd_seq).".<br>";
							
							if($fue_cerrada_la_gui==false)
							{
								echo "<script>alert('ERROR Al subir en la tabla gioss_indexador_duplicados_del_reparador_4725  ".procesar_mensaje($error_bd_seq)."');</script>";
							}
						    }
						    else
						    {
						    ...
						    //sube a corregidos_sin_duplicados_sida4725
						    $query_subir_registro_corregido="";
						    $query_subir_registro_corregido.=" INSERT INTO ";
						    $query_subir_registro_corregido.=" corregidos_sin_duplicados_sida4725 ";				
						    $query_subir_registro_corregido.=" ( ";				
						    $numero_actual_campo_registro_corregido=0;
						    while($numero_actual_campo_registro_corregido<185)
						    {
							    $query_subir_registro_corregido.=" campo_sida_de_numero_orden_".$numero_actual_campo_registro_corregido." , ";
						    ...
						    $query_subir_registro_corregido.=" nombre_archivo_vih_sida ";
						    $query_subir_registro_corregido.=" ) ";
						    ...
						    //fin sube a corregidos_sin_duplicados_sida4725
						...
						else if($existe_afiliado==true)
						{
						    ...
						    //borra el que estaba en corregidos_sin_duplicados_sida4725
						    //entrea si el nuemro de filas es igual a dos
						    if(count($array_check_tiene_2_filas_coincidentes)==2)
						    {
							//BORRANDO el afiliado duplicado de corregidos_sin_duplicados_sida4725
							$sql_delete_corregidos_temp="";
							$sql_delete_corregidos_temp.=" DELETE FROM corregidos_sin_duplicados_sida4725  ";
							...
							$sql_delete_corregidos_temp.=" AND ";
							$sql_delete_corregidos_temp.=" nombre_archivo_vih_sida='".$nombre_archivo_registrado."'  ";
							$sql_delete_corregidos_temp.=" AND ";
							$sql_delete_corregidos_temp.=" tipo_id_usuario='$tipo_id' ";
							$sql_delete_corregidos_temp.=" AND ";
							$sql_delete_corregidos_temp.=" id_usuario='$identificacion' ";
							...
							$sql_delete_corregidos_temp.=" nick_usuario='$nick_user' ";
							$sql_delete_corregidos_temp.=" AND ";
							$sql_delete_corregidos_temp.=" campo_sida_de_numero_orden_8='".$campos[8]."' ";
							$sql_delete_corregidos_temp.=" AND ";
							$sql_delete_corregidos_temp.=" campo_sida_de_numero_orden_9='".$campos[9]."' ";
							$sql_delete_corregidos_temp.=" ; ";
							$error_bd_seq="";		
							$bool_hubo_error_query=$coneccionBD->insertar_no_warning_get_error_no_crea_cierra($sql_delete_corregidos_temp, $error_bd_seq);
							if($error_bd_seq!="")
							{
								$mensajes_error_bd.=" ERROR Al eliminar en la tabla corregidos_sin_duplicados_sida4725 ".procesar_mensaje($error_bd_seq).".<br>";
								
							}
							//FIN BORRANDO el afiliado duplicado de corregidos_sin_duplicados_sida4725
						    }
						    //fin borra el que estaba en corregidos_sin_duplicados_sida4725
						    ...
						    $query_insert_updt_en_indexador.=" AND ";
						    $query_insert_updt_en_indexador.="campo_sida_de_numero_orden_8_tipo_id='".$campos[8]."'";
						    $query_insert_updt_en_indexador.=" AND ";
						    $query_insert_updt_en_indexador.="campo_sida_de_numero_orden_9_numero_id='".$campos[9]."'";						    
						    $query_insert_updt_en_indexador.=" ; ";
						    $error_bd_seq="";		
						    $bool_hubo_error_query=$coneccionBD->insertar_no_warning_get_error_no_crea_cierra($query_insert_updt_en_indexador, $error_bd_seq);
						    if($error_bd_seq!="")
						    {
							$mensajes_error_bd.=" ERROR Al actualizar en la tabla gioss_indexador_duplicados_del_reparador_4725 ".procesar_mensaje($error_bd_seq).".<br>";
							
							if($fue_cerrada_la_gui==false)
							{
								echo "<script>alert('ERROR Al actualizar en la tabla gioss_indexador_duplicados_del_reparador_4725  ".procesar_mensaje($error_bd_seq)."');</script>";
							}
						    }
						...
						$query_subir_registro_corregido="";
						$query_subir_registro_corregido.=" INSERT INTO ";
						$query_subir_registro_corregido.=" corregidos_con_duplicados_sida4725 ";				
						$query_subir_registro_corregido.=" ( ";				
						$numero_actual_campo_registro_corregido=0;
						while($numero_actual_campo_registro_corregido<185)
						{
							$query_subir_registro_corregido.=" campo_sida_de_numero_orden_".$numero_actual_campo_registro_corregido." , ";
							$numero_actual_campo_registro_corregido++;
						}//fin while para nombres columnas de bd correspondientes a los campos de 4725 a insertar
						$query_subir_registro_corregido.=" tipo_id_usuario, ";
						$query_subir_registro_corregido.=" id_usuario, ";
						$query_subir_registro_corregido.=" nick_usuario, ";
						$query_subir_registro_corregido.=" numero_registro, ";
						...
						while($numero_actual_campo_registro_corregido<185)
						{
						    $query_subir_registro_corregido.="'".$campos[$numero_actual_campo_registro_corregido]."',";
						    $numero_actual_campo_registro_corregido++;
						}//fin while con los valores de los campos 4725 a insertar en la tabla
						$query_subir_registro_corregido.="'".$tipo_id."',";
						$query_subir_registro_corregido.="'".$identificacion."',";
						$query_subir_registro_corregido.="'".$nick_user."',";	
							$query_subir_registro_corregido.="'".($cont_linea_para_indexador+1)."',";							
					...
					while($cont_campo_reparado<185)
					{
						if($linea_reparada!=""){$linea_reparada.="\t";}
						//aqui escribe en la linea todos los campos excepto el primero
						//mirar correccion ese enredo de los numeros de los campos
						if($cont_campo_reparado!=0)
						{
					...
					$file_reparado_r4725_VIH = fopen($ruta_archivo_reparado_VIH, "a") or die("fallo la creacion del archivo");
					if($es_primera_linea==false)
					{
						fwrite($file_reparado_r4725_VIH,$linea_reparada);
						$es_primera_linea=true;
					}
					else
					{
						fwrite($file_reparado_r4725_VIH,"\n".$linea_reparada);
					}
					fclose($file_reparado_r4725_VIH);
					//FIN ESCRIBE LINEA REPARADA
					
					if($hubo_inconsistencias_en_vih==false)
					{
						$hubo_inconsistencias_en_vih=$array_resultados_validacion["error"];
					}
					if($hubo_inconsistencias_en_vih==false)
					{
						$hubo_inconsistencias_en_vih=$array_resultados_validacion_2["error"];
					}
					
					//escribe los errores
					$mensaje_errores_vih=$array_resultados_validacion["mensaje"];
					$array_mensajes_errores_campos=explode("|",$mensaje_errores_vih);
					foreach($array_mensajes_errores_campos as $msg_error)
					{
						fwrite($file_inconsistencias_r4725_vih_sida, $msg_error."\n");
				...
				else
				{
					...
					//echo "<script>alert(' ".($cont_linea+1)." ".count($campos)."');</script>";
					
					$mensaje_contador_errores="revisando linea ".($cont_linea+1)." de $lineas_del_archivo del archivo VIH.  <br> Numero incorrecto campos ".count($campos)." por favor revisar.";
					$html_del_mensaje="";
					$html_del_mensaje.="<table>";
					$html_del_mensaje.="<tr>";
					$html_del_mensaje.="<td colspan=\'2\'>";
					...
					$error_longitud=$consecutivo_errores.",".$nombre_archivo_registrado.",01,".$array_tipo_validacion["01"].",0104,".$array_grupo_validacion["0104"].",0104099,$cadena_descripcion_inconsistencia ,".($cont_linea+1).","."-1";
					$consecutivo_errores++;
					
					if($hubo_inconsistencias_en_vih==false)
					{
						$hubo_inconsistencias_en_vih=true;
					}
					fwrite($file_inconsistencias_r4725_vih_sida, $error_longitud."\n");
					...
					$file_excluido_r4725_VIH = fopen($ruta_archivo_excluido_VIH, "a") or die("fallo la creacion del archivo");
					if($es_primera_linea_exc==false)
					{
						fwrite($file_excluido_r4725_VIH,$linea_original);
						$es_primera_linea_exc=true;
					}
					else
					{
						fwrite($file_excluido_r4725_VIH,"\n".$linea_original);
					}
					fclose($file_excluido_r4725_VIH);
					...
					if($es_primera_linea==false)
					{
						fwrite($file_reparado_r4725_VIH,"NO_SUBIO");
						$es_primera_linea=true;
					}
					else
					{
		...
		fclose($file_inconsistencias_r4725_vih_sida);
		
		//FIN PARTE CORRIGE ARCHIVO, IDENTIFICA E INDEXA DUPLICADOS
		
		
		//ARREGLO DE DUPLICADOS EN UNO SOLO
		$nombre_vista_index_duplicados="indxd4725".$nombre_archivo_registrado.$nick_user.$fecha_y_hora_para_view;
		...
		$sql_vista_duplicados_reporte_obligatorio .="SELECT * from gioss_indexador_duplicados_del_reparador_4725  ";	
		...
		$sql_vista_duplicados_reporte_obligatorio.=" ORDER BY campo_sida_de_numero_orden_8_tipo_id asc,campo_sida_de_numero_orden_9_numero_id asc  ";
		$sql_vista_duplicados_reporte_obligatorio.=";";
		$error_bd_seq="";		
		...
		{								
			...
			$error_bd_seq="";
			$resultados_query_vih4725_duplicados=$coneccionBD->consultar_no_warning_get_error_no_crea_cierra($sql_query_busqueda_personas_bloques,$error_bd_seq);
			if($error_bd_seq!="")
			{
				$mensajes_error_bd.="ERROR AL CONSULTAR de vista de las personas: ".$error_bd_seq."<br>";
			}
			
			if(count($resultados_query_vih4725_duplicados)>0 && is_array($resultados_query_vih4725_duplicados))
			{
				foreach($resultados_query_vih4725_duplicados as $duplicado_actual)
				{
					//echo "<script>alert('entro ".count($resultados_query_vih4725_duplicados)."');</script>";
			
				    //TOMA LOS DATOS DEL DUPLICADO ACTUAL DE LA VISTA DE LA TABLA DEL INDEXADOR
				    $tipo_id_duplicado_actual=trim($duplicado_actual["campo_sida_de_numero_orden_8_tipo_id"]);
				    $numero_id_duplicado_actual=trim($duplicado_actual["campo_sida_de_numero_orden_9_numero_id"]);
					...
					$fileHandler = new SplFileObject($ruta_archivo_reparado_VIH);		
					...
					//arreglo especial sida para poner al validar y quitar al escribir la primer columna solo aplica VIH
					array_unshift($array_campos_del_duplicado_del_afiliado,$cod_prestador);
					...
					$identificadores_de_cambios_duplicados_registro.="VIH"."||";//tipo reporte
					...
					//if mira que la linea contega los 185 campos
					if(count($array_campos_del_duplicado_del_afiliado)==185)
					{
					    ...
					    {
						//sube a gioss_temp_dupl_afiliado_actual_reparador_sida4725
						//para agrupar solo los registros duplicados para dicho afiliado
						$query_subir_registro_corregido="";
						$query_subir_registro_corregido.=" INSERT INTO ";
						$query_subir_registro_corregido.=" gioss_temp_dupl_afiliado_actual_reparador_sida4725 ";				
						$query_subir_registro_corregido.=" ( ";				
						$numero_actual_campo_registro_corregido=0;
						while($numero_actual_campo_registro_corregido<185)
						{
						...
						while($numero_actual_campo_registro_corregido<185)
						{
						    if($numero_actual_campo_registro_corregido!=8 &&  $numero_actual_campo_registro_corregido!=9 )
						    {
						    ...
						    else if($numero_actual_campo_registro_corregido==8)
						    {
						    ...
						    else if($numero_actual_campo_registro_corregido==9)
						    {
						...
						if($error_bd_seq!="")
						{
						    ...
						    $mensajes_error_bd.=" ERROR Al subir en la tabla gioss_temp_dupl_afiliado_actual_reparador_sida4725 ".procesar_mensaje($error_bd_seq).".<br>";
							
						}
						//fin sube a gioss_temp_dupl_afiliado_actual_reparador_sida4725
						
						
						//sube a corregidos_solo_duplicados_sida4725 para reportes futuros
						$query_subir_registro_corregido="";
						$query_subir_registro_corregido.=" INSERT INTO ";
						$query_subir_registro_corregido.=" corregidos_solo_duplicados_sida4725 ";				
						$query_subir_registro_corregido.=" ( ";				
						$numero_actual_campo_registro_corregido=0;
						while($numero_actual_campo_registro_corregido<185)
						{
						...
						if($error_bd_seq!="")
						{
						    echo "<script>alert('ERROR Al subir en la tabla corregidos_solo_duplicados_sida4725 ".procesar_mensaje($error_bd_seq)." ');</script>";
						    $mensajes_error_bd.=" ERROR Al subir en la tabla corregidos_solo_duplicados_sida4725 ".procesar_mensaje($error_bd_seq).".<br>";
							
						}
						//fin sube a corregidos_solo_duplicados_sida4725 para reportes futuros
						
						
					    }//fin else if si habian varias filas en la lista por ende tiene duplicados el afiliado 
					}//fin if si la linea posee 185 campos
				    ...
				    {
					$nombre_vista_con_los_duplicados_del_afiliado_actual="duppa4725".$nombre_archivo_registrado.$nick_user.$fecha_y_hora_para_view;
					...
					$sql_vista_duplicados_de_la_persona_actual .="SELECT * from gioss_temp_dupl_afiliado_actual_reparador_sida4725  ";	
					...
					$sql_vista_duplicados_de_la_persona_actual.=" AND ";
					$sql_vista_duplicados_de_la_persona_actual.=" nombre_archivo_vih_sida='".$nombre_archivo_registrado."'  ";
					$sql_vista_duplicados_de_la_persona_actual.=" AND ";
					...
					$sql_vista_duplicados_de_la_persona_actual.=" AND ";
					$sql_vista_duplicados_de_la_persona_actual.=" campo_sida_de_numero_orden_8='$tipo_id_duplicado_actual' ";
					$sql_vista_duplicados_de_la_persona_actual.=" AND ";
					$sql_vista_duplicados_de_la_persona_actual.=" campo_sida_de_numero_orden_9='$numero_id_duplicado_actual' ";
					...
					//echo "<script>alert('duplicados de la persona actual $numero_personas_de_duplicado ');</script>";
					...
					{						
						if(count($array_campos_procesados_de_los_duplicados_del_duplicado)!=185)
						{
						...
						$cont_orden_campo_a_string=1;//a partir de 1 debido a que VIH son 184 campos									
						while($cont_orden_campo_a_string<185)
						{
						...
						$array_errores=reparacion_campo_en_blanco_formato_vih($array_campos_procesados_de_los_duplicados_del_duplicado,
													    $numero_registro_para_procesado,
													    $consecutivo_errores,
													    $array_tipo_validacion,
													    $array_grupo_validacion,
						...
						$array_errores=reparacion_valor_permitido_formato_vih($array_campos_procesados_de_los_duplicados_del_duplicado,$numero_registro_para_procesado,
											 $consecutivo_errores,$array_tipo_validacion,
											 $array_grupo_validacion,$array_detalle_validacion,
											 $nombre_archivo_registrado,$fecha_de_corte,$cod_prestador,
											 $cod_eapb,$diccionario_identificacion,$diccionario_identificacion_lineas,
						...
						$array_errores=reparacion_calidad_formato_vih($array_campos_procesados_de_los_duplicados_del_duplicado,$numero_registro_para_procesado,
											 $consecutivo_errores,$array_tipo_validacion,
											 $array_grupo_validacion,$array_detalle_validacion,
											 $nombre_archivo_registrado,$fecha_de_corte,$cod_prestador,
											 $cod_eapb,$diccionario_identificacion,$diccionario_identificacion_lineas,
						...
						*/
						...
						$sql_insert_procesado_en_reporte_obligatorio.=" corregidos_sin_duplicados_sida4725 ";									    
						$sql_insert_procesado_en_reporte_obligatorio.=" ( ";				
						$cont_orden_campo_vih=0;
						while($cont_orden_campo_vih<185)
						{
							$sql_insert_procesado_en_reporte_obligatorio.=" campo_sida_de_numero_orden_".$cont_orden_campo_vih." , ";
							$cont_orden_campo_vih++;
						}//fin while para nombres columnas de bd correspondientes a los campos de 4725 a insertar
						...
						$sql_insert_procesado_en_reporte_obligatorio.=" nombre_archivo_vih_sida ";
						$sql_insert_procesado_en_reporte_obligatorio.=" ) ";
						...
						while($cont_orden_campo_vih<185)
						{
							$sql_insert_procesado_en_reporte_obligatorio.="'".$array_campos_procesados_de_los_duplicados_del_duplicado[$cont_orden_campo_vih]."',";
							$cont_orden_campo_vih++;
						}//fin while con los valores de los campos 4725 a insertar en la tabla de reporte obligatorio
					...
					}//fin if si fueron procesados duplicados inserta el porcesado en la tabla de archivos reportados obligatorios exitosos de 4725
					...
					$sql_delete_corregidos_temp="";
					$sql_delete_corregidos_temp.=" DELETE FROM gioss_temp_dupl_afiliado_actual_reparador_sida4725  ";
					$sql_delete_corregidos_temp.=" WHERE fecha_de_generacion='$fecha_actual'  AND hora_generacion='$tiempo_actual' ";
					$sql_delete_corregidos_temp.=" AND ";
					$sql_delete_corregidos_temp.=" codigo_entidad_prestadora='".$cod_prestador."'  ";
					$sql_delete_corregidos_temp.=" AND ";
		...
		//PARTE ESCRIBE CSV
		$nombre_vista_consulta_definitiva_corregidos="vcrosida4725_".$nick_user."_".$tipo_id."_".$identificacion;
		...
		$sql_vista_consulta_reporte_obligatorio.=" AS SELECT * FROM corregidos_sin_duplicados_sida4725 ";
		...
		$sql_vista_consulta_reporte_obligatorio.=" AND ";
		$sql_vista_consulta_reporte_obligatorio.=" nombre_archivo_vih_sida='".$nombre_archivo_registrado."'  ";
		$sql_vista_consulta_reporte_obligatorio.=" AND ";
		...
		while($contador_offset<$numero_registros)
		{
			...
			if(count($resultado_query_reporte_obligatoria)>0)
			{
				...
				$file_reparado_r4725_sin_dupl_VIH= fopen($ruta_archivo_reparado_sin_duplicados_VIH, "a") or die("fallo la creacion del archivo");
						    
				foreach($resultado_query_reporte_obligatoria as $resultado)
				{
					$cadena_escribir_linea="";
					$cont_orden_campo_sida=0;
					while($cont_orden_campo_sida<185)
					{
						if($cadena_escribir_linea!="")
						{
							$cadena_escribir_linea.="\t";
						}
						//arreglo especial sida para quitar primer columna
						if($cont_orden_campo_sida!=0)
						{
							$cadena_escribir_linea.=$resultado["campo_sida_de_numero_orden_".$cont_orden_campo_sida];
						}
						$cont_orden_campo_sida++;
					}
					if($flag_para_salto_linea_inicial==false)
					{
						fwrite($file_reparado_r4725_sin_dupl_VIH, $cadena_escribir_linea);
						$flag_para_salto_linea_inicial=true;
					}
					else
					{
						fwrite($file_reparado_r4725_sin_dupl_VIH, "\n".$cadena_escribir_linea);
					}
		...
		if($es_valido_nombre_archivo)
		{
			...
			if(connection_aborted()==false)
			{
				...
				{
					echo "<script>alert('No se encontraron duplicados.');</script>";
				}
				echo "<script>document.getElementById('mensaje').style.textAlign='center';</script>";
				echo "<script>document.getElementById('mensaje').innerHTML='$mensaje_perm_estado $mensaje_perm_estado_reg_dupl $mensaje_perm_estado_reg_recuperados Se ha terminado de corregir VIH<br> lineas con numero de campos incorrectos, que no permitieron corregir: $numero_lineas_campos_incorrectos';</script>";
		...
		if($hubo_inconsistencias_en_vih)
		{
			$se_genero_archivo_de_inconsistencias=true;		
			$errores.="Hubo inconsistencias en el archivo VIH.<br>";
			
		}
		
		//CREAR ZIP
		$archivos_a_comprimir=array();
		$archivos_a_comprimir[]=$ruta_archivo_inconsistencias_vih;
		$archivos_a_comprimir[]=$ruta_archivo_reparado_VIH;
		$archivos_a_comprimir[]=$ruta_archivo_reparado_sin_duplicados_VIH;
		$archivos_a_comprimir[]=$ruta_archivo_excluido_VIH;
		$archivos_a_comprimir[]=$ruta_cambios_duplicados_campos;
		$archivos_a_comprimir[]=$ruta_cambios_campos_correccion_norma_pre;//agregado 18 10 2017
		$archivos_a_comprimir[]=$ruta_cambios_campos_correccion_norma_pos;//agregado 18 10 2017
		$archivos_a_comprimir[]=$ruta_archivo_afiliado_no_existe;//agregado 18 10 2017
		$archivos_a_comprimir[]=$ruta_archivo_afiliado_existe_cambio_sexo;//agregado 18 10 2017
		$archivos_a_comprimir[]=$ruta_archivo_afiliado_existe_cambio_fecha_nacimiento;//agregado 18 10 2017
		$archivos_a_comprimir[]=$ruta_archivo_registros_excluidos_no_afiliados;//agregado 18 10 2017
		$archivos_a_comprimir[]=$ruta_archivo_registros_excluidos_fecha_nacimiento_invalida;//agregado 18 10 2017
		$ruta_zip=$rutaTemporal."ReparadoR4725VIH_".$cod_prestador."_".$fecha_actual."_".$tiempo_actual_string.'.zip';
		...
		//BOTONES DESCARGA
		$botones="";
		$botones.=" <input type=\'button\' value=\'Descargar archivo reparado para VIH\'  class=\'btn btn-success color_boton\' onclick=\"download_inconsistencias_campos(\'$ruta_zip\');\"/> ";
		...
		if($errores!="")
		{
			if(connection_aborted()==false)
			{
				...
				echo "<script>document.getElementById('parrafo_mensaje_error').innerHTML='$errores <br> Se reparo el archivo <br> $botones';</script>";
				ob_flush();
				flush();
			}
		}
		else
		{
			if(connection_aborted()==false)
			{
				...
				echo "<script>document.getElementById('parrafo_mensaje_exito').innerHTML='$exitos <br> Se reparo el archivo <br> $botones';</script>";
				ob_flush();
				flush();
			}
		}
		...
		$query_delete_log_dupl_anterior.=" AND ";
		$query_delete_log_dupl_anterior.=" tipo_reporte='VIH' ";
		$query_delete_log_dupl_anterior.=" AND ";
		...
		if($error_bd!="")
		{
			if(connection_aborted()==false)
			{
				echo "<script>alert('error al actualizar el estado actual de validacion en tiempo real  4725 ');</script>";
			}
		}
		...
		try
		{
			...
			{	
				...
				$mail->FromName = "GIOSS";
				$mail->Subject = "Reparacion VIH R4725 ";
				$mail->AltBody = "Cordial saludo,\n El sistema ha reparado el archivo VIH";
		    
				$mail->MsgHTML("Cordial saludo,\n El sistema ha reparado el archivo VIH para la EAPB $cod_eapb.<strong>GIOSS</strong>.");
	...
	}//fin if si no fue validado como exitoso previamente
	else
	{
		if($errores!="")
		{
		...
		if($bool_esta_siendo_reparado)
		{
			...
			{
				echo "<script>document.getElementById('div_mensaje_error').style.display='inline';</script>";
				echo "<script>document.getElementById('parrafo_mensaje_error').innerHTML='<br>El archivo 4725 esta siendo reparado actualmente.';</script>";
				ob_flush();
				flush();
			}
		}
