...
$smarty->display('olvido_usuario.html.tpl');
...
if(isset($_POST["tipo_identificacion"]) && isset($_POST["identificacion"]) && isset($_POST["codigo_entidad"]) && isset($_POST["fecha_nacimiento"]) )
{
	if($_POST["tipo_identificacion"]!="none" && $_POST["identificacion"]!="" && $_POST["codigo_entidad"]!="" && $_POST["fecha_nacimiento"]!="")
	{
		...
		if(count($resultado_existe_usuario)>0)
		{
			...
			$mail->FromName = "GIOSS";
			$mail->Subject = "Olvido Usuario ";
			$mail->AltBody = "Cordial saludo,\n ";

			$mail->MsgHTML("Cordial saludo,\n El sistema ha recuperado su usuario \"$nick\".<strong>GIOSS</strong>.");
			...
			if (!$mail->Send()) 
			{
				...
				echo "<script>ventana_mensaje_estilizada('No se pudo enviar el usuario a su correo','Error:')</script>";
			} else 
			{
				// echo "Mensaje enviado.";
				echo "<script>ventana_mensaje_estilizada('Se ha enviado su usuario a su correo: $correo_usuario','Usuario encontrado:')</script>";
			}
