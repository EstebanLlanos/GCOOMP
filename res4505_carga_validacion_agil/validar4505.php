...
class LecturaArchivo extends criterios_validacion_posafil_4505
{
    ...
    var $global_ruta_destino_ordenado;
    var $global_consecutivo_ruta_destino_ordenado;
    var $global_ruta_indice_ordenado_original;

    var $global_bool_excluidos_ordenar=false;
	...
	*/
	...
	public function validacionCamposPyPenArray($array_tipo_inconsistencia, $array_grupo_inconsistencia, $array_detalle_inconsistencia, $array_fields, $numLinea, $boolean_or_string, &$consecutivo_errores,&$conexion_bd_validar_campos,&$ultimo_afiliado)
	{
		...
		*/
		...
		{
			//PARTE NUMERO LINEA ORIGINAL CORRESPONDIENTE CONTRA EL ORDENADO
			$handler_archivo_indice_ordenado_original = new SplFileObject($this->global_ruta_indice_ordenado_original);
			$handler_archivo_indice_ordenado_original->seek($numLinea);
			//now print the line
			$valor_linea_indice=trim($handler_archivo_indice_ordenado_original->current());
			$array_valor_linea=explode("_", $valor_linea_indice);

			$numero_linea_original_desde_indice=intval( $array_valor_linea[0] );
			$error_en_consolidado=$array_valor_linea[1];
			//FIN PARTE LINEA ORIGINAL CORRESPONDIENTE CONTRA EL ORDENADO
			...
			if(true)
			{
				...
				$cont_linea=0;
				$cont_linea=$numero_linea_original_desde_indice;
				...
				*/			
				...
				//IDENTIFICACION AFILIADOS DUPLICADO PARA ARCHIVOS ORDENADOS
				$tipo_id_actual=trim($array_fields[3]);
				$numero_id_actual=trim($array_fields[4]);
				$entidad_119="";
				$entidad_119_con_separador_previo="";
				if($this->tipo_entidad_que_efectua_el_cargue=="agrupado_ips120")
				{
					$entidad_119=trim($array_fields[119]);
					$entidad_119_con_separador_previo="_123separador123_".$entidad_119;
				}
				if($ultimo_afiliado=="")
				{
					//nicializa ultimo afiliado
					$ultimo_afiliado=$tipo_id_actual."_123separador123_".$numero_id_actual.$entidad_119_con_separador_previo;
				}//fin
				//verifica duplicado
				if($ultimo_afiliado==$tipo_id_actual."_123separador123_".$numero_id_actual.$entidad_119_con_separador_previo)
				{
					if(isset($this->diccionario_identificacion_para_bool[$ultimo_afiliado])==false)
					{
						$this->diccionario_identificacion_para_bool[$ultimo_afiliado]=$cont_linea;
					...
					else
					{
						...
						$validador_boolean=false;
						$this->diccionario_identificacion_para_bool[$ultimo_afiliado]=$this->diccionario_identificacion_para_bool[$ultimo_afiliado].";;".$cont_linea;

						$lista_lineas_duplicados_para_csv=str_replace(";;", "-", $this->diccionario_identificacion_para_bool[$ultimo_afiliado] );
						//ESCRIBE INCONSISTENCIA DUPLICADOS
						if($mensajes_error_campos!=""){$mensajes_error_campos.="|";}
						//consecutivo|nombre|codigo_tipo_inconsistencia|desc_tipo_inconsistencia|codigo_grupo_inconsistencia|desc_tipo_inconsistencia|codigo_detalle_inconsistencia|desc_detalle|linea|campo
						...
						if($this->tipo_entidad_que_efectua_el_cargue=="agrupado_ips120")
						{
							$mensajes_error_campos.=$consecutivo_errores.",".$nombre_archivo4505.",01,".$array_tipo_inconsistencia["01"].",0105,".$array_grupo_inconsistencia["0105"].",$var_numero_codigo,$cadena_descripcion_inconsistencia ...VR:".$tipo_id_actual."_".$numero_id_actual." y EAPB c119 ".$entidad_119." Registros Coincidentes: $lista_lineas_duplicados_para_csv ,".$numero_linea_original_desde_indice.",".$numero_campo_actual.",".$this->codigo_habilitacion_para_inconsistencias.",".$tipo_id_actual.",".$numero_id_actual;
						}
						else
						{
							$mensajes_error_campos.=$consecutivo_errores.",".$nombre_archivo4505.",01,".$array_tipo_inconsistencia["01"].",0105,".$array_grupo_inconsistencia["0105"].",$var_numero_codigo,$cadena_descripcion_inconsistencia ...VR:".$tipo_id_actual."_".$numero_id_actual." Registros Coincidentes: $lista_lineas_duplicados_para_csv ,".$numero_linea_original_desde_indice.",".$numero_campo_actual.",".$this->codigo_habilitacion_para_inconsistencias.",".$tipo_id_actual.",".$numero_id_actual;
						}//fin else

						$consecutivo_errores++;
						//FIN ESCRIBE INCONSISTENCIA DUPLICADOS
					}
				}//fin if
				else if($ultimo_afiliado!=$tipo_id_actual."_123separador123_".$numero_id_actual.$entidad_119_con_separador_previo
					&& isset($this->diccionario_identificacion_para_bool[$ultimo_afiliado])==true
				)//fin condicion
				{
					$array_ultimo_afiliado=explode("_123separador123_", $ultimo_afiliado);
					$tipo_id_ultimo_afiliado=$array_ultimo_afiliado[0];
					$numero_id_ultimo_afiliado=$array_ultimo_afiliado[1];
					$entidad_119_ultimo_afiliado="";
					if($this->tipo_entidad_que_efectua_el_cargue=="agrupado_ips120")
					{
						$entidad_119_ultimo_afiliado=$array_ultimo_afiliado[2];
					}//fin if
				    ...
				    $query_insert_updt_en_indexador.="'".$tipo_id_ultimo_afiliado."',";
				    $query_insert_updt_en_indexador.="'".$numero_id_ultimo_afiliado."',";
				    if($this->tipo_entidad_que_efectua_el_cargue=="agrupado_ips120")
					{
						$query_insert_updt_en_indexador.="'".$entidad_119_ultimo_afiliado."',";
					}
				    $query_insert_updt_en_indexador.="'".$this->diccionario_identificacion_para_bool[$ultimo_afiliado]."'";
				    $query_insert_updt_en_indexador.=" ) ";
				    $query_insert_updt_en_indexador.=" ; ";
				    $error_bd_seq="";		
				    ...
				    unset($this->diccionario_identificacion_para_bool[$ultimo_afiliado]);
				...
				//FIN IDENTIFICACION AFILIADOS DUPLICADO PARA ARCHIVOS ORDENADOS
			}//fin if true
			//FIN PARTE DUPLICADOS

			
			$numero_campo_actual=1;
			if($error_en_consolidado=="SI")
			{
				$validador_boolean=false;
				if($mensajes_error_campos!=""){$mensajes_error_campos.="|";}
				//consecutivo|nombre|codigo_tipo_inconsistencia|desc_tipo_inconsistencia|codigo_grupo_inconsistencia|desc_tipo_inconsistencia|codigo_detalle_inconsistencia|desc_detalle|linea|campo
				...
				$cadena_descripcion_inconsistencia=explode(";;",$array_detalle_inconsistencia[$var_numero_codigo])[1];
				$mensajes_error_campos.=$consecutivo_errores.",".$nombre_archivo4505.",01,".$array_tipo_inconsistencia["01"].",0105,".$array_grupo_inconsistencia["0105"].",$var_numero_codigo,$cadena_descripcion_inconsistencia ...VR:".$array_fields[$numero_campo_actual]." ,".$numero_linea_original_desde_indice.",".$numero_campo_actual.",".$this->codigo_habilitacion_para_inconsistencias.",".$array_fields[3].",".$array_fields[4];
				$consecutivo_errores++;
			...
			$this->criterios_validacion($array_fields, $numero_linea_original_desde_indice, $boolean_or_string, $date_fin_reporte, $array_tipo_inconsistencia, $array_grupo_inconsistencia, $array_detalle_inconsistencia, $nombre_archivo4505, $year_corte, $consecutivo_errores, $validador_boolean, $mensajes_error_campos, $conexion_bd_validar_campos);	
	...
	public function Inconsistencias_y_Validaciones_CamposPyP($array_tipo_inconsistencia, $array_grupo_inconsistencia, $array_detalle_inconsistencia, $linea_con_campos_de_archivo_pyp,$numLinea,&$consecutivo_errores,&$conexion_bd_validar_campos,&$ultimo_afiliado)
	{
		...
		*/
		...
		else if($numLinea>0 && count($array_campos)==119)
		{
			...
			$array_inconsistencias_linea=$this->validacionCamposPyPenArray($array_tipo_inconsistencia, $array_grupo_inconsistencia, $array_detalle_inconsistencia, $array_campos,$numLinea,"ambos",$consecutivo_errores,$conexion_bd_validar_campos,$ultimo_afiliado);
    ...
    public function ValidarArchivo4505($file, $nombreArchivo, $tipoArchivo, $cantArchivos) 
    {
		...
		if($verificacion_inicial)
		{
		    ...
		    $error_linea="";
		    $ruta_destino_ordenado="";
		    $ruta_destino_ordenado=$ruta_temporales.$nombre_archivo_sin_extension."ORDORIG.txt";
		    $id_archivo_concatenada="";
		    $id_archivo_concatenada=$nombre_archivo4505.$this->nick_user.$fecha_actual.$tiempo_actual;
		    $this->global_ruta_indice_ordenado_original=$ruta_temporales."indice_ordenado_original.txt";
		    $this->global_ruta_destino_ordenado=$ruta_destino_ordenado;
		    $bool_archivo_ordenado=$this->ordenar_archivo($file,
		    	$ruta_destino_ordenado,
		    	$lineas_del_archivo,
		    	$id_archivo_concatenada,
		    	$array_tipo_inconsistencia,
		    	$array_grupo_inconsistencia,
		    	$array_detalle_inconsistencia,
		    	$this->nombre_archivo_4505,
		    	$error_linea,
		    	$consecutivo_errores,
		    	$contador_registros_malos,
		    	$obj);
		    
		    $hubo_errores_en_los_campos_del_archivo=$this->global_bool_excluidos_ordenar;
		    

		    $handler_archivo_ordenado=fopen($ruta_destino_ordenado, 'r');

		    $ultimo_afiliado="";
		    $cont_porcentaje=0;
		    $contador_filtrados=0;
		    $fue_cerrada_la_gui=false;
		    while (!feof($handler_archivo_ordenado)) 
		    {
			    ...
			    $linea = fgets($handler_archivo_ordenado);		    
			    ...
			    $errores_campos_linea.=$error_linea;
			    $error_linea="";
			    if($this->tipo_entidad_que_efectua_el_cargue=="individual_ips"
			    	|| $this->tipo_entidad_que_efectua_el_cargue=="agrupado_ips"
		       || $this->tipo_entidad_que_efectua_el_cargue=="agrupado_ips120"
			    ...
			    {
				$array_resultado_validacion_linea=$this->Inconsistencias_y_Validaciones_CamposPyP($array_tipo_inconsistencia, $array_grupo_inconsistencia, $array_detalle_inconsistencia, $linea,$numLinea,$consecutivo_errores,$obj,$ultimo_afiliado);
			    ...
			    if($errores_campos_linea!="")
			    {
				    ...
				    foreach ($array_errores_linea as $error) 
				    {
					    ...
					    //echo "numero_campo_temp ".$numero_campo_temp."<br>";

					    if( ($array_booleano_primer_error_por_linea[intval($numero_campo_temp)]==true || intval($numero_campo_temp)==999)
		    ...
		    fclose($handler_archivo_ordenado);
		    ...
		    $this->reescribe_primera_linea_function($this->global_consecutivo_ruta_destino_ordenado,
		    	$this->global_ruta_indice_ordenado_original,
		    	$array_linea_procesada);
		    ...
		    $archivos_a_comprimir[]=$this->global_ruta_destino_ordenado;
	...
	*/
	...
	public function ordenar_archivo($file,
		$ruta_destino_ordenado,
		$lineas_del_archivo,
	...
	{
		/*
		--drop table ordenar4505;
		create table ordenar4505
		(
			id_archivo text,
			id_completa_afiliado text,
			numero_fila_original int,
			datos text,	
			error_en_consolidado character varying(320),		
			primary key(id_archivo,id_completa_afiliado,numero_fila_original)
		);
		*/
		...
		date_default_timezone_set ("America/Bogota");
	    $fecha_y_tiempo_inicia_inserts = date('Y-m-d H:i:s');
	    echo "Inicia Fase 1 Ordenamiento a ".$fecha_y_tiempo_inicia_inserts."<br>";
		
		$original=fopen($file, 'r') or exit("No se pudo abrir el archivo");
		$cont_orig_linea=0;
		$cont_inserts=0;
		$acc_inserts=0;
		$bloque_inserts=1024;

		$query_insertar_ordenar="";
		$seccion_values="";
		while (!feof($original) ) 
		{
			$linea_actual = fgets($original);	    
		    $linea_actual = str_replace(",",".",$linea_actual);
		    $linea_actual = str_replace(array("'","\""),"",$linea_actual);
		    $linea_actual = str_replace(array("\n","\r","\t"),"",$linea_actual);

		    $campos_ord = explode("|", $linea_actual);

		    if(count($campos_ord)>=119)	
		    {	    

			    $identificador=strtoupper(trim($campos_ord[3]))."_".trim($campos_ord[4]);
			    //parte ordenamiento por tipo_id numero id y entidad 119
			    if($this->tipo_entidad_que_efectua_el_cargue=="agrupado_ips120")
			    {
			    	$identificador.="_".trim($campos_ord[119]);
			    }//fin if

			    $linea_actual=implode("|", $campos_ord);

			    $hubo_error_consolidado="NO";
			    $numero_campo_actual=1;
			    if($campos_ord[$numero_campo_actual]!=$cont_orig_linea)
				{					
					

					$hubo_error_consolidado="SI";


				}//fin if

			    /*
			    $query_insertar_ordenar="INSERT INTO ordenar4505(id_archivo,id_completa_afiliado,numero_fila_original,datos)
				    VALUES ('$identificador_archivo','$identificador','$cont_orig_linea','$linea_actual') ; ";

			    $error_bd_ins="";
			    $bool_hubo_error_query=$conexionBD->insertar_no_warning_get_error_no_crea_cierra($query_insertar_ordenar, $error_bd_ins);
			    if($error_bd_ins!="")
			    {
				    echo "No pudo subir registro a tabla ordenar: $error_bd_ins <br>";
			    }//fin if
			    */

			    if($seccion_values!=""){$seccion_values.=",";}
				$seccion_values.=" ('$identificador_archivo','$identificador','$cont_orig_linea','$linea_actual','$hubo_error_consolidado')  ";
				$cont_inserts++;
				$acc_inserts++;

				//echo "seccion ci $cont_inserts acci $acc_inserts bi $bloque_inserts la $lineas_del_archivo: ".$seccion_values."<br>";
			    
			    if($cont_inserts>=$bloque_inserts || $acc_inserts>=($lineas_del_archivo-1) )
			    {
			    	//echo "ENTRA A REALIZAR INSERT AGRUPADOS $cont_inserts acci $acc_inserts bi $bloque_inserts la $lineas_del_archivo<br>";

				    $query_insertar_ordenar="INSERT INTO ordenar4505(id_archivo,id_completa_afiliado,numero_fila_original,datos,error_en_consolidado)
				    VALUES  ".$seccion_values." ; ";
				    ...
				    //echo "query: ".$query_insertar_ordenar."<br>";
				    $seccion_values="";
				    $cont_inserts=0;
			...
			else if($cont_orig_linea==0)
			{
				$handler_resultado=fopen($ruta_destino_ordenado, 'w')  or exit("No se pudo abrir el archivo");
				fwrite($handler_resultado, $linea_actual);
				fclose($handler_resultado);

				$handler_indice_ordenado_original=fopen($this->global_ruta_indice_ordenado_original, 'w');
				fwrite($handler_indice_ordenado_original, "0");
				fclose($handler_indice_ordenado_original);
			}
			else
			{
				//cuando estan con un numero de campos no validos y no es la linea inicial
				$this->global_bool_excluidos_ordenar=true;
			    if($error_linea!=""){$error_linea.="|";}
			    //consecutivo|nombre|codigo_tipo_inconsistencia|desc_tipo_inconsistencia|codigo_grupo_inconsistencia|desc_tipo_inconsistencia|codigo_detalle_inconsistencia|desc_detalle|linea|campo
				$var_numero_codigo="0301002";
			    $cadena_descripcion_inconsistencia=explode(";;",$array_detalle_inconsistencia[$var_numero_codigo])[1];
			    $error_linea.=$consecutivo_errores.",".$nombre_archivo4505.",03,".$array_tipo_inconsistencia["03"].",0301,".$array_grupo_inconsistencia["0301"].",$var_numero_codigo,$cadena_descripcion_inconsistencia ...VR:".count($campos_ord)." ,".$cont_orig_linea.",999,".$this->codigo_habilitacion_para_inconsistencias;
			    $consecutivo_errores++;
			    ...
			    if($this->global_ruta_estructura_mala!="")
			    {
				    //se remplaza el archivo si ya existe con modo w		
				    $archivo_excluido_estructura_mala = fopen($this->global_ruta_estructura_mala, "a") or die("fallo la creacion del archivo modo:a estructura mala");		    		    
				    fwrite($archivo_excluido_estructura_mala, "\n".trim($linea_actual) );		 // $this->global_consecutivo_estructura_mala no es necesario aca   
				    fclose($archivo_excluido_estructura_mala);	
				    $this->global_consecutivo_estructura_mala++;
			    }//fin if
			    $cont_inserts++;
			    ...
			    if($cont_inserts>=$bloque_inserts || $acc_inserts>=($lineas_del_archivo-1) )
			    {
			    	//echo "ENTRA A REALIZAR INSERT AGRUPADOS DESDE VACIA $cont_inserts acci $acc_inserts bi $bloque_inserts la $lineas_del_archivo<br>";
		...
		$fecha_y_tiempo_finaliza_inserts = date('Y-m-d H:i:s');
		echo "Finaliza Fase 1 Ordenamiento a ".$fecha_y_tiempo_finaliza_inserts."<br>";

		$nombre_vista="ord".$identificador_archivo;
		$nombre_vista=str_replace(array("-","_",":","."),"",$nombre_vista);
		$nombre_vista=$this->alphanumericAndSpace3($nombre_vista);

		$query_vista_ordenados="CREATE OR REPLACE VIEW $nombre_vista AS (SELECT *  FROM ordenar4505 where id_archivo='$identificador_archivo' order by id_completa_afiliado asc) ; ";
		$error_bd_ins="";
	    $bool_hubo_error_query=$conexionBD->insertar_no_warning_get_error_no_crea_cierra($query_vista_ordenados, $error_bd_ins);
	    if($error_bd_ins!="")
	    {
		    echo "No pudo crear vista: $error_bd_ins <br>";
		    $error_ordenar=true;
	    }

	    $fecha_y_tiempo_inicia_consulta = date('Y-m-d H:i:s');
	    echo "Inicia Fase 2 Ordenamiento a ".$fecha_y_tiempo_inicia_consulta."<br>";

		$numero_registros=0;
		$query_consultar_contar_registros="SELECT count(*) as numero_registros FROM $nombre_vista;";
		$resultado_query1_tipo_validacion=$conexionBD->consultar2_no_crea_cierra($query_consultar_contar_registros);
		if(is_array($resultado_query1_tipo_validacion)==true && count($resultado_query1_tipo_validacion)>0)
		{
			$numero_registros=$resultado_query1_tipo_validacion[0]['numero_registros'];
		}//fin if

		$bloque=25000;
		$offset=0;

		$consecutivo_de_ordenado=1;//valor del campo 1
		if($numero_registros>0)
		{
			while($offset<$numero_registros)
			{
				$query_consultar_registros="SELECT *  FROM $nombre_vista  offset $offset limit $bloque ;";
				$resultado_ordenados=$conexionBD->consultar2_no_crea_cierra($query_consultar_registros);

				$handler_resultado=fopen($ruta_destino_ordenado, 'a')  or exit("No se pudo abrir el archivo");
				$handler_indice_ordenado_original=fopen($this->global_ruta_indice_ordenado_original, 'a');
				foreach ($resultado_ordenados as $key => $registro) 
				{
					$numero_fila_original_desde_bd=trim($registro['numero_fila_original']);
					$error_en_consolidado=trim($registro['error_en_consolidado']);
					$linea_desde_bd=$registro['datos'];
					$array_campos_linea_desde_bd=explode("|", $linea_desde_bd);
					$array_campos_linea_desde_bd[1]=$consecutivo_de_ordenado;
					$linea_desde_bd=implode("|", $array_campos_linea_desde_bd);
					fwrite($handler_resultado, "\n".$linea_desde_bd);
					fwrite($handler_indice_ordenado_original, "\n".$numero_fila_original_desde_bd."_".$error_en_consolidado);
					$this->global_consecutivo_ruta_destino_ordenado=$consecutivo_de_ordenado;
					$consecutivo_de_ordenado++;
				}//fin foreach
				fclose($handler_resultado);
				fclose($handler_indice_ordenado_original);
				$offset+=$bloque;
		...
		$fecha_y_tiempo_finaliza_consulta = date('Y-m-d H:i:s');
	    echo "Finaliza Fase 2 Ordenamiento a ".$fecha_y_tiempo_finaliza_consulta."<br>";

		$query_drop_vista_ordenados="DROP VIEW $nombre_vista ; ";
		$error_bd_ins="";
	    $bool_hubo_error_query=$conexionBD->insertar_no_warning_get_error_no_crea_cierra($query_drop_vista_ordenados, $error_bd_ins);
	    if($error_bd_ins!="")
	    {
		    echo "No pudo borrar vista: $error_bd_ins <br>";
		    $error_ordenar=true;
	    }//fin if

	    $query_drop_ordenados="DELETE FROM ordenar4505 where id_archivo='$identificador_archivo' ; ";
		$error_bd_ins="";
	    $bool_hubo_error_query=$conexionBD->insertar_no_warning_get_error_no_crea_cierra($query_drop_ordenados, $error_bd_ins);
	    if($error_bd_ins!="")
	    {
		    echo "No pudo borrar vista: $error_bd_ins <br>";
		    $error_ordenar=true;
