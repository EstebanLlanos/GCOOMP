...
$smarty->display('act_ent_tipo_perfil.html.tpl');
...
if(isset($_FILES["archivo_info"]))
{
	...
	$ruta_1=$rutaTemporal."tabla_perfil_tipo_entidad.sql";
	$ruta_2=$rutaTemporal."tabla_perfil_tipo_entidad.error.csv";
	...
	while (!feof($archivo_cargar)) 
	{
		...
		/*
		  cod_tipo_entidad character varying(320) NOT NULL,
		  descripcion_nombre_tipo_entidad character varying(320) NOT NULL,
		...
		*/		
		...
		if($linea_res!="")
		{
			...
			{
				$campo_cod_tipo_entidad = substr($campos[$numero_campo_pk], 0, strlen($campos[$numero_campo_pk])-1);
				$campo_cod_tipo_entidad = str_replace("\n","",$campo_cod_tipo_entidad);
				$campo_cod_tipo_entidad = str_replace("\"","",$campo_cod_tipo_entidad);
				$campo_cod_tipo_entidad = str_replace("'","",$campo_cod_tipo_entidad);
			}
			else
			{
			...
			if($numero_campo_pk_1 == count($campos)-1 )
			{
				$campo_cod_perfil = substr($campos[$numero_campo_pk_1], 0, strlen($campos[$numero_campo_pk_1])-1);
				$campo_cod_perfil = str_replace("\n","",$campo_cod_perfil);
				$campo_cod_perfil = str_replace("\"","",$campo_cod_perfil);
				$campo_cod_perfil = str_replace("'","",$campo_cod_perfil);
			}
			else
			{
				$campo_cod_perfil = str_replace("\n","",$campos[$numero_campo_pk_1]);
				$campo_cod_perfil = str_replace("\"","",$campo_cod_perfil);
				$campo_cod_perfil = str_replace("'","",$campo_cod_perfil);
			}
				
			$sql_verificar="SELECT cod_tipo_entidad,cod_perfil FROM  gioss_tipo_entidades_perfiles WHERE cod_tipo_entidad='".$campo_cod_tipo_entidad."' AND cod_perfil='".$campo_cod_perfil."' ";
			...
			if(count($campos)==3)
			{
				...
				{
					$sql_upsert="";
					//INICIA QUERY INSERT gioss_tipo_entidades_perfiles
					$sql_upsert.="insert into gioss_tipo_entidades_perfiles";
					$sql_upsert.="(";
					$sql_upsert.="cod_tipo_entidad,";
					$sql_upsert.="descripcion_nombre_tipo_entidad,";
					$sql_upsert.="cod_perfil";
					$sql_upsert.=")";
					$sql_upsert.="values";
					$sql_upsert.="(";
					$cont_campos=0;
					...
					$sql_upsert.=");";
					//FIN QUERY INSERT gioss_tipo_entidades_perfiles
				...
				else
				{
					...
					$sql_upsert="";
					//INICIA QUERY UPDATE gioss_tipo_entidades_perfiles
					$sql_upsert.="UPDATE gioss_tipo_entidades_perfiles SET ";
					
					$sql_upsert.="cod_tipo_entidad='".$array_campos_procesados[0]."',";
					$sql_upsert.="descripcion_nombre_tipo_entidad='".$array_campos_procesados[1]."',";
					$sql_upsert.="cod_perfil='".$array_campos_procesados[2]."' ";
					$sql_upsert.=" WHERE ";
					$sql_upsert.=" cod_tipo_entidad='".$campo_cod_tipo_entidad."' AND cod_perfil='".$campo_cod_perfil."' ";
					$sql_upsert.=";";
					//FIN QUERY UPDATE gioss_tipo_entidades_perfiles
			...
			{
				$mensaje="<p style='color:green;'> Fila para perfil tipo entidad insertada en la base de datos. Linea(cuenta desde 0): ".$cont_linea.". num columnas ".count($campos)."  </p>";
