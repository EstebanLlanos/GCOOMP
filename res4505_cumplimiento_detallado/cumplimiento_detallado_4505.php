...
$smarty->display('cumplimiento_detallado_4505.html.tpl');
...
if(isset($_POST["year_de_validacion"]) && isset($_POST["eapb"]) && $_POST["eapb"]!="none" && ($_POST["year_de_validacion"]==""||ctype_digit($_POST["year_de_validacion"])))
{
	...
	*/
	...
	$sql_vista_calidad_de_datos.=" AS SELECT * ";
	...
	while($contador_offset<$numero_registros_vista)
	{
		...
		if(count($resultado_para_cumplimiento_detallado)>0 && is_array($resultado_para_cumplimiento_detallado))
		{
			...
			$nombre_archivo_calidad_de_datos=$cod_eapb."_cumplimiento_detallado_pyp.csv";
			...
			if($hubo_resultados==false)
			{
				...
				$html_nueva_ventana.="<title>Reporte de Cumplimiento Detallado</title>";
				...
				$titulos.="Indicador cumplimiento Periodo I,Indicador cumplimiento Periodo II,Indicador cumplimiento Periodo III,Indicador cumplimiento Periodo IV,";
				$titulos.="Total Registros Periodo I,Total Registros Periodo II,Total Registros Periodo III,Total Registros Periodo IV,a?o";
				...
				foreach($array_titulos as $titulo_columna)
				{
					...
					{
						$html_nueva_ventana.="<th>A&ntildeo</th>";
					}
