...
//require_once 'reparadorCANCER.php';
//require_once '../res_0247_cancer/reparadorCANCER_v2017.php';
//require_once '../res_0247_cancer/reparadorCANCER_v2018.php';
...
function imprimir_por_pantalla_controlado($cadena_a_imprimir, $con_flush=false)
{
	try
	{
		echo $cadena_a_imprimir;
		if($con_flush==true)
		{
			ob_flush();
			flush();
...
$proveniente_de_prestador_o_eapb.="<tr><td style='text-align:left;'><h5 id='sub_titulo_tipo_entidad' style=\"color:#0000FF;text-shadow: 2px 2px 5px #A8A8FF;\">Tipo Entidad que realiza el cargue</h5></td></tr>
					<tr>
						...
						<select id='tipo_archivo_norma' name='tipo_archivo_norma' class='campo_azul' onchange='mostrar_selectores_geograficos();validar_antes_seleccionar_archivos();' style='width:228px;' >							    
...
//SELECTOR PERIODO
$query_periodos_rips="SELECT * FROM gioss_periodo_reporte_0247_CANCER ORDER BY codigo_periodo;";
...
$smarty->display('fixer_CANCER.html.tpl');
...
$si_fue_validado_antes=false;

$mensaje_advertencia_tiempo="";
$mensaje_advertencia_tiempo .="Estimado usuario, se ha iniciado el proceso de reparaci&oacuten del archivo,<br> lo que puede tomar varios minutos, dependiendo del volumen de registros.<br>";
$mensaje_advertencia_tiempo .="Una vez reparado dicho archivo corregido,  se enviar&aacute a su Correo electr&oacutenico o puede descargarlo directamente del aplicat&iacutevo.<br>";

if(isset($_POST["accion"]) && $_POST["accion"]=="validar" && isset($_FILES["0247_cancer_file"]) && $_FILES["0247_cancer_file"]["error"]==0)
{
	...
	$ruta_carpeta_del_reparado=$rutaTemporal."rCANCER_".$nombre_archivo_registrado."_".$fecha_actual."_".$tiempo_actual_string;
	...
	//parte verifica si es mayor de 01-01 del year
	$year_corte_inferior=trim($_POST["year_de_corte"]);
	$year_corte_para_buscar=trim($_POST["year_de_corte"]);
	$fecha_corte_de_acuerdo_norma=trim($_POST["year_de_corte"])."-01-01";
	$diferencia_dias_con_fecha_corte_de_acuerdo_norma=diferencia_dias_entre_fechas($fecha_de_corte,$fecha_corte_de_acuerdo_norma);
	if($diferencia_dias_con_fecha_corte_de_acuerdo_norma<0)
	{
	...
	echo "Version Criterios Reparacion a usar ".$nombre_archivo_criterios_seleccionado."<br>";
	...
	//ARCHIVO CAMPO 17 NO EXISTE corresponde 4 char
	$ruta_archivo_campo_17_no_existe=$ruta_carpeta_del_reparado."/".$nombre_archivo_registrado."_c17DIAGNOEXISTE.txt";		    
	//se remplaza el archivo si ya existe con modo w		
	$file_archivo_campo_17_no_existe = fopen($ruta_archivo_campo_17_no_existe, "w") or die("fallo la creacion del archivo");
	fwrite($file_archivo_campo_17_no_existe, "registros donde el codigo de diagnostico del campo 17 no existe");
	fclose($file_archivo_campo_17_no_existe);
	//FIN ARCHIVO CAMPO 17 NO EXISTE corresponde 4 char
	
	//ARCHIVO CAMPO 17 NO EXISTE no corresponde 4 char
	$ruta_archivo_campo_17_no_existe_no4ch=$ruta_carpeta_del_reparado."/".$nombre_archivo_registrado."_c17DIAGNOEXno4ch.txt";		    
	//se remplaza el archivo si ya existe con modo w		
	$file_archivo_campo_17_no_existe_no4ch = fopen($ruta_archivo_campo_17_no_existe_no4ch, "w") or die("fallo la creacion del archivo");
	fwrite($file_archivo_campo_17_no_existe_no4ch, "registros donde el codigo de diagnostico del campo 17 no existe");
	fclose($file_archivo_campo_17_no_existe_no4ch);
	//FIN ARCHIVO CAMPO 17 NO EXISTE no corresponde 4 char
	
	//ARCHIVO CAMPO 17 NO EXISTE en blanco
	$ruta_archivo_campo_17_no_existe_blanco=$ruta_carpeta_del_reparado."/".$nombre_archivo_registrado."_c17DIAGNOEXblanco.txt";		    
	//se remplaza el archivo si ya existe con modo w		
	$file_archivo_campo_17_no_existe_blanco = fopen($ruta_archivo_campo_17_no_existe_blanco, "w") or die("fallo la creacion del archivo");
	fwrite($file_archivo_campo_17_no_existe_blanco, "registros donde el codigo de diagnostico del campo 17 no existe");
	fclose($file_archivo_campo_17_no_existe_blanco);
	//FIN ARCHIVO CAMPO 17 NO EXISTE en blanco
	
	//ABRE O CREA EL ARCHIVO DONDE SE REPARARA EL ARCHIVO
	$ruta_archivo_reparado_CANCER=$ruta_carpeta_del_reparado."/".$nombre_archivo_registrado."_dupl.txt";
	if(file_exists($ruta_archivo_reparado_CANCER))
	{
		unlink($ruta_archivo_reparado_CANCER);
	}
	
	$ruta_archivo_excluido_CANCER=$ruta_carpeta_del_reparado."/".$nombre_archivo_registrado."_excluido.txt";
	if(file_exists($ruta_archivo_excluido_CANCER))
	{
		unlink($ruta_archivo_excluido_CANCER);
	}
	
	$ruta_archivo_reparado_sin_duplicados_CANCER=$ruta_carpeta_del_reparado."/".$nombre_archivo_registrado.".txt";
	if(file_exists($ruta_archivo_reparado_sin_duplicados_CANCER))
	{
		unlink($ruta_archivo_reparado_sin_duplicados_CANCER);
	}
	
	$file_reparado_r0247_CANCER = fopen($ruta_archivo_reparado_CANCER, "w") or die("fallo la creacion del archivo");
	fclose($file_reparado_r0247_CANCER);
	
	$file_reparado_r0247_sin_dupl_CANCER = fopen($ruta_archivo_reparado_sin_duplicados_CANCER, "w") or die("fallo la creacion del archivo");
	fclose($file_reparado_r0247_sin_dupl_CANCER);
	...
	//PARTE VALIDACION ESTRUCTURA NOMBRE DEL ARCHIVO CANCER
	$es_valido_nombre_archivo=true;
	
		
	$exitos="Archivo $nombre_archivo_registrado. <br>";
		
	if ($archivo_cancer_inicial['size'] > 250000000)
	{
	...
	else
	{
		...
		{
			$ruta_archivo_cancer = $rutaTemporal.$archivo_cancer_inicial['name'];
			move_uploaded_file($archivo_cancer_inicial['tmp_name'], $ruta_archivo_cancer);
			
			$array_nombre_sin_sigla=explode("CANCER",$archivo_cancer_inicial['name']);
			if(count($array_nombre_sin_sigla)!=2)
			{
				$es_valido_nombre_archivo=false;
				$errores.="El encabezado del archivo $nombre_archivo_registrado no corresponde a un archivo CANCER. <br>";
			}
			else
			{
				...
				/*para agrupado remover de las tablas gioss_tabla_estado_informacion_r0247_cancer y gioss_numero_de_secuencia_r0247_cancer las condiciones de llave foranea para prestador
				ALTER TABLE gioss_numero_de_secuencia_r0247_cancer DROP CONSTRAINT gioss_numero_de_secuencia_r02_codigo_prestador_servicios_s_fkey;
				ALTER TABLE gioss_tabla_estado_informacion_r0247_cancer DROP CONSTRAINT gioss_tabla_estado_informacion__codigo_prestador_servicios_fkey;
				ALTER TABLE gioss_tabla_registros_cargados_exito_r0247_cancer DROP CONSTRAINT gioss_tabla_registros_cargados_codigo_prestador_reportante_fkey;
				ALTER TABLE gioss_tabla_registros_no_cargados_rechazados_r0247_cancer DROP CONSTRAINT gioss_tabla_registros_no_carga_codigo_prestador_reportante_fkey;
				ALTER TABLE gioss_tabla_consolidacion_registros_validados_r0247_cancer DROP CONSTRAINT gioss_tabla_consolidacion_regis_codigo_entidad_reportadora_fkey;
				*/
				...
				else if($tipo_entidad_que_efectua_el_cargue=="agrupado_eapb" && strlen($nombre_sin_txt_para_verificacion)!=30 && strlen($nombre_sin_txt_para_verificacion)!=28)
				{
					...
					else if(strlen($nombre_sin_txt_para_verificacion)==22)
					{
		...
		{
			$es_valido_nombre_archivo=false;
			$errores.="El nombre del archivo para CANCER es invalido. <br>";
		}
	}//fin else
	//FIN PARTE VALIDACION ESTRUCTURA NOMBRE DEL ARCHIVO CANCER
	...
	//VERIFICA SI EL ARCHIVO YA ESTA SIENDO VALIDADO ACTUALMENTE
        ...
        $query_verificacion_esta_siendo_procesado.=" SELECT * FROM gioss_0247_esta_reparando_ar_actualmente ";
	...
	//VERIFICA SI FUE CARGADO  PREVIAMENTE
	$sql_query_verificar="";
	$sql_query_verificar.=" SELECT * FROM gioss_tabla_estado_informacion_r0247_cancer ";
	...
	//FIN VERIFICA SI FUE CARGADO  PREVIAMENTE
	else
	{
		$es_valido_nombre_archivo=false;
		$errores.="No se ha validado previamente de el archivos, validelos para proceder a reparar.<br>";
	}
	
	if($si_fue_validado_antes==true && $bool_esta_siendo_reparado==false)
	{
		//SE BORRA SI HABIA ALGO ANTES EN LA TABLA TEMPORAL DE CORREGIDOS PARA EL USUARIO
		/*
		$sql_delete_corregidos_temp="";
		$sql_delete_corregidos_temp.=" DELETE FROM corregidos_con_duplicados_cancer0247 ";
		...
		$sql_delete_corregidos_temp.=" AND ";
		$sql_delete_corregidos_temp.=" nombre_archivo_cancer='$nombre_archivo_registrado' ";
		$sql_delete_corregidos_temp.=" AND ";
		...
		$sql_delete_corregidos_temp="";
		$sql_delete_corregidos_temp.=" DELETE FROM corregidos_solo_duplicados_cancer0247 ";
		$sql_delete_corregidos_temp.=" WHERE ";
		$sql_delete_corregidos_temp.=" tipo_id_usuario='$tipo_id' ";
		$sql_delete_corregidos_temp.=" AND ";
		$sql_delete_corregidos_temp.=" id_usuario='$identificacion' ";
		...
		if($error_bd_seq!="")
		{
			$mensajes_error_bd.=" ERROR Al eliminar en la tabla temporal de registros corregidos con solo los duplicados encontrados para corrector ".procesar_mensaje($error_bd_seq).".<br>";
			
		}
		
		$sql_delete_corregidos_temp="";
		$sql_delete_corregidos_temp.=" DELETE FROM corregidos_sin_duplicados_cancer0247 ";
		$sql_delete_corregidos_temp.=" WHERE ";
		$sql_delete_corregidos_temp.=" tipo_id_usuario='$tipo_id' ";
		$sql_delete_corregidos_temp.=" AND ";
		$sql_delete_corregidos_temp.=" id_usuario='$identificacion' ";
		...
		if($error_bd_seq!="")
		{
			$mensajes_error_bd.=" ERROR Al eliminar en la tabla temporal de registros corregidos pos correccion sin duplicados  para corrector ".procesar_mensaje($error_bd_seq).".<br>";
		...
		*/
		...
		$query_insert_esta_siendo_procesado.=" INSERT INTO gioss_0247_esta_reparando_ar_actualmente ";
		$query_insert_esta_siendo_procesado.=" ( ";
		...
		*/
		...
		$hubo_inconsistencias_en_CANCER=false;
		...
		if($es_valido_nombre_archivo)
		{
			...
			*/
			...
			$mensaje_errores_CANCER="";
			$lineas_del_archivo = contar_lineas_archivo($ruta_archivo_cancer); 
			$file_CANCER = fopen($ruta_archivo_cancer, 'r') or exit("No se pudo abrir el archivo");
			
			//la variable $consecutivo_errores pasara como referencia y aumentara cada que se haye un error y su incremento es independiente de la variable $cont_linea
			...
			while (!feof($file_CANCER)) 
			{
				...
				$mensaje_estado_registros.="<tr><td style=text-align:left;width:60%>Numero de registros:</td><td style=text-align:left>".($lineas_del_archivo)."</td></tr>";
				...
				$mensaje_estado_registros.="<tr style=background-color:#80bfff><td style=text-align:left;width:60%>Numero de personas (Identificaci&oacuten y diagnostico iguales)con registros duplicados:</td><td style=text-align:left>".$personas_con_duplicados_hasta_el_momento."</td></tr>";
				$mensaje_estado_registros.="<tr><td style=text-align:left;width:60%>Numero de registros con numero de campos invalidos (menor o mayor de 210):</td><td style=text-align:left>".$numero_lineas_campos_incorrectos."</td></tr>";
				...
				if($muestra_mensaje_nuevo)
				{
					...
					*/
					...
					$query_update_esta_siendo_procesado.=" UPDATE gioss_0247_esta_reparando_ar_actualmente ";
					...
					$verificar_si_ejecucion_fue_cancelada.=" SELECT esta_ejecutando FROM gioss_0247_esta_reparando_ar_actualmente ";
				...
				//pasa a coregir los campos
				if(count($campos)==210)
				{
					$cont_pre_fix_campos=0;
					while($cont_pre_fix_campos<210)
					{
					...
					//parte separa campo 17 que no coincide
					$campo_n17=trim($campos[16]);
					
					$consulta_campo17="";
					$consulta_campo17.="SELECT * FROM gioss_diagnostico_ciex_cancer_tumor_maligno WHERE codigo_diagnostico='".$campo_n17."' ; ";
					$resultado_c17=$coneccionBD->consultar2_no_crea_cierra($consulta_campo17);
					if( count($resultado_c17)==0
					   && !is_array($resultado_c17)
					   && strlen($campo_n17)==4
					   )
					{
						//echo "<script>alert(' entro a separar registro ');</script>";
						$file_archivo_campo_17_no_existe = fopen($ruta_archivo_campo_17_no_existe, "a") or die("fallo la creacion del archivo");
						fwrite($file_archivo_campo_17_no_existe, "\n".trim($linea));
						fclose($file_archivo_campo_17_no_existe);
						
					}//fin if
					else if(
						count($resultado_c17)==0
					   && !is_array($resultado_c17)
					   && strlen($campo_n17)!=4
					   )
					{
						$file_archivo_campo_17_no_existe_no4ch = fopen($ruta_archivo_campo_17_no_existe_no4ch, "a") or die("fallo la creacion del archivo");
						fwrite($file_archivo_campo_17_no_existe_no4ch, "\n".trim($linea));
						fclose($file_archivo_campo_17_no_existe_no4ch);
					}
					else if(
						count($resultado_c17)==0
					...
					{
						$file_archivo_campo_17_no_existe_blanco = fopen($ruta_archivo_campo_17_no_existe_blanco, "a") or die("fallo la creacion del archivo");
						fwrite($file_archivo_campo_17_no_existe_blanco, "\n".trim($linea));
						fclose($file_archivo_campo_17_no_existe_blanco);
					}
					//fin parte separa campo 17 que no coincide
					...
					if($es_linea_valida_para_reparar==true)//si la linea cumple con las condiciones prevas es valida para reparar
					{
						...
						$array_fase_1_reparacion=reparador_campo_obligatorio_CANCER($campos,$cod_eapb,$cont_linea_para_indexador,$consecutivo_errores,
					...
					if(count($campos)==210) //redundante aca el numero de campos antes de entrar era 210, por la anterior validacion
					{
						//INDEXADOR DE DUPLICADOS
						//FASE 1 consulta por el campo 5,6 y 17 (tipo id,  numero id afiliado, diagnostico) si existe duplicado
						...
						$query_consultar_en_indexador.=" gioss_indexador_duplicados_del_reparador_0247 ";
						...
						$query_consultar_en_indexador.=" AND ";
						$query_consultar_en_indexador.="campo_cancer_de_numero_orden_5_tipo_id='".$campos[4]."'";// en cancer cuenta desde 1 en bd
						$query_consultar_en_indexador.=" AND ";
						$query_consultar_en_indexador.="campo_cancer_de_numero_orden_6_numero_id='".$campos[5]."'";
						$query_consultar_en_indexador.=" AND ";
						$query_consultar_en_indexador.="campo_cancer_de_numero_orden_17_diag_ciex='".$campos[16]."'";
						$query_consultar_en_indexador.=" ; ";
						$error_bd_seq="";		
						...
						if($error_bd_seq!="")
						{
						    $mensajes_error_bd.=" ERROR Al consultar en la tabla gioss_indexador_duplicados_del_reparador_0247 ".procesar_mensaje($error_bd_seq).".<br>";
						    
						    if($fue_cerrada_la_gui==false)
						    {
							    echo "<script>alert('ERROR Al consultar en la tabla gioss_indexador_duplicados_del_reparador_0247 ".procesar_mensaje($error_bd_seq)."');</script>";
						    }
						...
						if(is_array($resultado_esta_afiliado_en_indexador) && count($resultado_esta_afiliado_en_indexador)>0)
						{
						    ...
						    $acumulador_para_contar_duplicados+=1;
							    //echo "lista_lineas_duplicados recien consultada $lista_lineas_duplicados <br>";
						...
						if($existe_afiliado==false)
						{
						    ...
						    $query_insert_updt_en_indexador.=" gioss_indexador_duplicados_del_reparador_0247 ";				
						    $query_insert_updt_en_indexador.=" ( ";	
						    ...
						    $query_insert_updt_en_indexador.="'".$campos[16]."',";
						    ...
						    if($error_bd_seq!="")
						    {
							$mensajes_error_bd.=" ERROR Al subir en la tabla gioss_indexador_duplicados_del_reparador_0247 ".procesar_mensaje($error_bd_seq).".<br>";
							
							if($fue_cerrada_la_gui==false)
							{
								echo "<script>alert('ERROR Al subir en la tabla gioss_indexador_duplicados_del_reparador_0247  ".procesar_mensaje($error_bd_seq)."');</script>";
							}
						    }
						    else
						    {
						    ...
						    //sube a corregidos_sin_duplicados_cancer0247
						    $query_subir_registro_corregido="";
						    $query_subir_registro_corregido.=" INSERT INTO ";
						    $query_subir_registro_corregido.=" corregidos_sin_duplicados_cancer0247 ";				
						    $query_subir_registro_corregido.=" ( ";				
						    ...
						    //fin sube a corregidos_sin_duplicados_cancer0247
						...
						else if($existe_afiliado==true)
						{
						    ...
						    //borra el que estaba en corregidos_sin_duplicados_cancer0247
						    //entrea si el nuemro de filas es igual a dos
						    if(count($array_check_tiene_2_filas_coincidentes)==2)
						    {
							//BORRANDO el afiliado duplicado de corregidos_sin_duplicados_cancer0247
							$sql_delete_corregidos_temp="";
							$sql_delete_corregidos_temp.=" DELETE FROM corregidos_sin_duplicados_cancer0247  ";
							...
							$sql_delete_corregidos_temp.=" nick_usuario='$nick_user' ";
							$sql_delete_corregidos_temp.=" AND ";
							$sql_delete_corregidos_temp.=" campo_cancer_de_numero_orden_5='".$campos[4]."' ";
							$sql_delete_corregidos_temp.=" AND ";
							$sql_delete_corregidos_temp.=" campo_cancer_de_numero_orden_6='".$campos[5]."' ";							
							$sql_delete_corregidos_temp.=" AND ";
							$sql_delete_corregidos_temp.=" campo_cancer_de_numero_orden_17='".$campos[16]."'";
							$sql_delete_corregidos_temp.=" ; ";
							$error_bd_seq="";		
							$bool_hubo_error_query=$coneccionBD->insertar_no_warning_get_error_no_crea_cierra($sql_delete_corregidos_temp, $error_bd_seq);
							if($error_bd_seq!="")
							{
								$mensajes_error_bd.=" ERROR Al eliminar en la tabla corregidos_sin_duplicados_cancer0247 ".procesar_mensaje($error_bd_seq).".<br>";
								
							}
							//FIN BORRANDO el afiliado duplicado de corregidos_sin_duplicados_cancer0247
						    }
						    //fin borra el que estaba en corregidos_sin_duplicados_cancer0247
						    
							    //echo "lista_lineas_duplicados antes de update $lista_lineas_duplicados <br>";
						    ...
						    $query_insert_updt_en_indexador.=" AND ";
						    $query_insert_updt_en_indexador.="campo_cancer_de_numero_orden_5_tipo_id='".$campos[4]."'";
						    $query_insert_updt_en_indexador.=" AND ";
						    $query_insert_updt_en_indexador.="campo_cancer_de_numero_orden_6_numero_id='".$campos[5]."'";
						    $query_insert_updt_en_indexador.=" AND ";
						    $query_insert_updt_en_indexador.="campo_cancer_de_numero_orden_17_diag_ciex='".$campos[16]."'";						    
						    $query_insert_updt_en_indexador.=" ; ";
						    $error_bd_seq="";		
						    $bool_hubo_error_query=$coneccionBD->insertar_no_warning_get_error_no_crea_cierra($query_insert_updt_en_indexador, $error_bd_seq);
						    if($error_bd_seq!="")
						    {
							$mensajes_error_bd.=" ERROR Al actualizar en la tabla gioss_indexador_duplicados_del_reparador_0247 ".procesar_mensaje($error_bd_seq).".<br>";
							
							if($fue_cerrada_la_gui==false)
							{
								echo "<script>alert('ERROR Al actualizar en la tabla gioss_indexador_duplicados_del_reparador_0247  ".procesar_mensaje($error_bd_seq)."');</script>";
							}
						    }
						...
						//SUBE A EL REGISTRO A LA TABLA corregidos_con_duplicados_cancer0247
						$query_subir_registro_corregido="";
						$query_subir_registro_corregido.=" INSERT INTO ";
						$query_subir_registro_corregido.=" corregidos_con_duplicados_cancer0247 ";				
						$query_subir_registro_corregido.=" ( ";				
						$numero_actual_campo_registro_corregido=1;
						while($numero_actual_campo_registro_corregido<=210)
						{
						...
						if($error_bd_seq!="")
						{
							$mensajes_error_bd.=" ERROR Al subir en la tabla temporal de registros corregidos pre correccion duplicados  para corrector .<br>";
							
						}
						}//fin if cuenta 210 campos para indexador
					//SUBE A EL REGISTRO A LA TABLA corregidos_con_duplicados_cancer0247
					...
					$file_reparado_r0247_CANCER = fopen($ruta_archivo_reparado_CANCER, "a") or die("fallo la creacion del archivo");
					if($es_primera_linea==false)
					{
						fwrite($file_reparado_r0247_CANCER,$linea_reparada);
						$es_primera_linea=true;
					}
					else
					{
						fwrite($file_reparado_r0247_CANCER,"\n".$linea_reparada);
					}
					fclose($file_reparado_r0247_CANCER);
					//FIN ESCRIBE LINEA REPARADA
					
					if($hubo_inconsistencias_en_CANCER==false)
					{
						$hubo_inconsistencias_en_CANCER=$array_fase_1_reparacion["error"];
						}//fin if
					if($hubo_inconsistencias_en_CANCER==false)
					{
						$hubo_inconsistencias_en_CANCER=$array_fase_2_reparacion["error"];
						}//fin if
				...
				else /// else para registros con numero de campos invalidos
				{
					...
					$file_excluido_r0247_CANCER = fopen($ruta_archivo_excluido_CANCER, "a") or die("fallo la creacion del archivo");
					if($es_primera_linea_exc==false)
					{
						fwrite($file_excluido_r0247_CANCER,$linea_original);
						$es_primera_linea_exc=true;
					}
					else
					{
						fwrite($file_excluido_r0247_CANCER,"\n".$linea_original);
					}
					fclose($file_excluido_r0247_CANCER);
					...
					if($es_primera_linea==false)
					{
						fwrite($file_reparado_r0247_CANCER,"NO_SUBIO");
						$es_primera_linea=true;
					}
					else
					{
		...
		$nombre_vista_index_duplicados="indxd0247".$nombre_archivo_registrado.$nick_user.$fecha_y_hora_para_view;
		...
		$sql_vista_duplicados_reporte_obligatorio .="SELECT * from gioss_indexador_duplicados_del_reparador_0247  ";	
		...
		$sql_vista_duplicados_reporte_obligatorio.=" ORDER BY campo_cancer_de_numero_orden_5_tipo_id asc,campo_cancer_de_numero_orden_6_numero_id asc, campo_cancer_de_numero_orden_17_diag_ciex asc  ";
		$sql_vista_duplicados_reporte_obligatorio.=";";
		$error_bd_seq="";		
		...
		{								
			...
			if(count($resultados_query_cancer0247_duplicados)>0 && is_array($resultados_query_cancer0247_duplicados))
			{
				...
				{
					//echo "<script>alert('entro ".count($resultados_query_cancer0247_duplicados)."');</script>";
				    ...
				    $diagnostico_ciex_duplicado_actual=trim($duplicado_actual["campo_cancer_de_numero_orden_17_diag_ciex"]);
					...
					$fileHandler = new SplFileObject($ruta_archivo_reparado_CANCER);		
					...
					if(count($array_campos_del_duplicado_del_afiliado)==210)
					{
					    ...
					    {
						//sube a gioss_temp_dupl_afiliado_actual_reparador_cancer0247
						//para agrupar solo los registros duplicados para dicho afiliado
						$query_subir_registro_corregido="";
						$query_subir_registro_corregido.=" INSERT INTO ";
						$query_subir_registro_corregido.=" gioss_temp_dupl_afiliado_actual_reparador_cancer0247 ";				
						$query_subir_registro_corregido.=" ( ";				
						$numero_actual_campo_registro_corregido=1;
						while($numero_actual_campo_registro_corregido<=210)
						{
						...
						while($numero_actual_campo_registro_corregido<210)
						{
						    ...
						    {
							$query_subir_registro_corregido.="'".$diagnostico_ciex_duplicado_actual."',";
							
							$diag_ciex_temp_del_array=trim(procesar_mensaje3($array_campos_del_duplicado_del_afiliado[$numero_actual_campo_registro_corregido]));
							if($diag_ciex_temp_del_array!=$diagnostico_ciex_duplicado_actual)
							{
							    echo "<script>alert('numero registro: ".$linea_act." los diagnostico ciex  son diferentes del array: $num_id_temp_del_array de la tabla indexador:$numero_id_duplicado_actual lista: $lista_string_filas_donde_esta_duplicado');</script>";
							}
						    }
						...
						if($error_bd_seq!="")
						{
						    ...
						    $mensajes_error_bd.=" ERROR Al subir en la tabla gioss_temp_dupl_afiliado_actual_reparador_cancer0247 ".procesar_mensaje($error_bd_seq).".<br>";
							
						}
						//fin sube a gioss_temp_dupl_afiliado_actual_reparador_cancer0247
						
						
						//sube a corregidos_solo_duplicados_cancer0247 para reportes futuros
						$query_subir_registro_corregido="";
						$query_subir_registro_corregido.=" INSERT INTO ";
						$query_subir_registro_corregido.=" corregidos_solo_duplicados_cancer0247 ";				
						$query_subir_registro_corregido.=" ( ";				
						$numero_actual_campo_registro_corregido=1;
						while($numero_actual_campo_registro_corregido<=210)
						{
						...
						if($error_bd_seq!="")
						{
						    echo "<script>alert('ERROR Al subir en la tabla corregidos_solo_duplicados_cancer0247 ".procesar_mensaje($error_bd_seq)." ');</script>";
						    $mensajes_error_bd.=" ERROR Al subir en la tabla corregidos_solo_duplicados_cancer0247 ".procesar_mensaje($error_bd_seq).".<br>";
							
						}
						//fin sube a corregidos_solo_duplicados_cancer0247 para reportes futuros
				    ...
				    {
					$nombre_vista_con_los_duplicados_del_afiliado_actual="duppa0247".$nombre_archivo_registrado.$nick_user.$fecha_y_hora_para_view;
					...
					$sql_vista_duplicados_de_la_persona_actual .="SELECT * from gioss_temp_dupl_afiliado_actual_reparador_cancer0247  ";	
					...
					$sql_vista_duplicados_de_la_persona_actual.=" AND ";
					$sql_vista_duplicados_de_la_persona_actual.=" nombre_archivo_cancer='".$nombre_archivo_registrado."'  ";
					$sql_vista_duplicados_de_la_persona_actual.=" AND ";
					...
					$sql_vista_duplicados_de_la_persona_actual.=" AND ";
					$sql_vista_duplicados_de_la_persona_actual.=" campo_cancer_de_numero_orden_17='$diagnostico_ciex_duplicado_actual' ";
					...
					$array_campos_procesados_de_los_duplicados_del_duplicado=reparacion_campos_duplicados($tipo_id_duplicado_actual,
															  ...
															  $diagnostico_ciex_duplicado_actual,
															  $fecha_actual,
															  $tiempo_actual,
					...
					{						
						if(count($array_campos_procesados_de_los_duplicados_del_duplicado)!=210)
						{
						...
						while($cont_orden_campo_a_string<210)
						{
						...
						*/
						...
						$sql_insert_procesado_en_reporte_obligatorio.=" corregidos_sin_duplicados_cancer0247 ";									    
						$sql_insert_procesado_en_reporte_obligatorio.=" ( ";				
					...
					$sql_delete_corregidos_temp="";
					$sql_delete_corregidos_temp.=" DELETE FROM gioss_temp_dupl_afiliado_actual_reparador_cancer0247  ";
					$sql_delete_corregidos_temp.=" WHERE fecha_de_generacion='$fecha_actual'  AND hora_generacion='$tiempo_actual' ";
					$sql_delete_corregidos_temp.=" AND ";
					$sql_delete_corregidos_temp.=" codigo_entidad_prestadora='".$cod_prestador."'  ";
					$sql_delete_corregidos_temp.=" AND ";
		...
		//PARTE ESCRIBE CSV
		$nombre_vista_consulta_definitiva_corregidos="cro0247".$nombre_archivo_registrado.$nick_user.$fecha_y_hora_para_view;
		...
		$sql_vista_consulta_reporte_obligatorio.=" AS SELECT * FROM corregidos_sin_duplicados_cancer0247 ";
		...
		$sql_vista_consulta_reporte_obligatorio.=" AND ";
		$sql_vista_consulta_reporte_obligatorio.=" nombre_archivo_cancer='".$nombre_archivo_registrado."'  ";
		$sql_vista_consulta_reporte_obligatorio.=" AND ";
		...
		while($contador_offset<$numero_registros)
		{
			...
			if(count($resultado_query_reporte_obligatoria)>0)
			{
				...
				$file_reparado_r0247_sin_dupl_CANCER= fopen($ruta_archivo_reparado_sin_duplicados_CANCER, "a") or die("fallo la creacion del archivo");
				...
				foreach($resultado_query_reporte_obligatoria as $resultado)
				{
					...
					while($cont_orden_campo_cancer<=210)
					{
						...
						$cadena_escribir_linea.=$resultado["campo_cancer_de_numero_orden_".$cont_orden_campo_cancer];
						
						$cont_orden_campo_cancer++;
					}
					if($flag_para_salto_linea_inicial==false)
					{
						fwrite($file_reparado_r0247_sin_dupl_CANCER, $cadena_escribir_linea);
						$flag_para_salto_linea_inicial=true;
					}
					else
					{
						fwrite($file_reparado_r0247_sin_dupl_CANCER, "\n".$cadena_escribir_linea);
					}
		...
		if($es_valido_nombre_archivo)
		{
			if(connection_aborted()==false)
			{
				...
				echo "<script>document.getElementById('mensaje').innerHTML='$mensaje_perm_estado $mensaje_perm_estado_reg_dupl $mensaje_perm_estado_reg_recuperados Se ha terminado de reparar el archivo CANCER <br> lineas con numero de campos incorrectos, que no permitieron corregir: $numero_lineas_campos_incorrectos';</script>";
			...
			//UPDATE MENSAJE
			$mensaje_estado_registros="";
			$mensaje_estado_registros.="$mensaje_perm_estado $mensaje_perm_estado_reg_dupl $mensaje_perm_estado_reg_recuperados Se ha terminado de reparar el archivo CANCER <br> lineas con numero de campos incorrectos,";
			$mensaje_estado_registros.=" que no permitieron corregir: $numero_lineas_campos_incorrectos ";
		...
		if($hubo_inconsistencias_en_CANCER)
		{
			$se_genero_archivo_de_inconsistencias=true;		
			$errores.="Hubo inconsistencias en el archivo CANCER.<br>";
			
		}
		
		//CREAR ZIP
		$archivos_a_comprimir=array();
		$archivos_a_comprimir[]=$ruta_archivo_reparado_CANCER;
		$archivos_a_comprimir[]=$ruta_archivo_reparado_sin_duplicados_CANCER;
		$archivos_a_comprimir[]=$ruta_archivo_excluido_CANCER;
		$archivos_a_comprimir[]=$ruta_cambios_duplicados_campos;
		$archivos_a_comprimir[]=$ruta_archivo_campo_17_no_existe;
		$archivos_a_comprimir[]=$ruta_archivo_campo_17_no_existe_no4ch;
		$archivos_a_comprimir[]=$ruta_archivo_campo_17_no_existe_blanco;
		$archivos_a_comprimir[]=$ruta_cambios_campos_correccion_norma_pre;//agregado 21 09 2017
		$archivos_a_comprimir[]=$ruta_cambios_campos_correccion_norma_pos;//agregado 21 09 2017
		$archivos_a_comprimir[]=$ruta_archivo_afiliado_no_existe;//agregado 28 09 2017
		$archivos_a_comprimir[]=$ruta_archivo_afiliado_existe_cambio_sexo;//agregado 28 09 2017
		$archivos_a_comprimir[]=$ruta_archivo_afiliado_existe_cambio_fecha_nacimiento;//agregado 28 09 2017
		$archivos_a_comprimir[]=$ruta_archivo_registros_excluidos_no_afiliados;//agregado 28 09 2017
		$archivos_a_comprimir[]=$ruta_archivo_registros_excluidos_fecha_nacimiento_invalida;//agregado 28 09 2017
		$ruta_zip=$ruta_carpeta_del_reparado."/reparado0247CANCER_".$cod_prestador."_".$fecha_actual."_".$tiempo_actual_string.'.zip';
		...
		//BOTONES DESCARGA
		$botones="";
		$botones.=" <input type=\'button\' value=\'Descargar archivo reparado para CANCER\'  class=\'btn btn-success color_boton\' onclick=\"download_inconsistencias_campos(\'$ruta_zip\');\"/> ";
		...
		if($errores!="")
		{
			if(connection_aborted()==false)
			{
				...
				echo "<script>document.getElementById('parrafo_mensaje_error').innerHTML='$errores <br> Se ha reparado el archivo <br> $botones';</script>";
				ob_flush();
				flush();
			}
		}
		else
		{
			if(connection_aborted()==false)
			{
				...
				echo "<script>document.getElementById('parrafo_mensaje_exito').innerHTML='$exitos <br> Se ha reparado el archivo <br> $botones';</script>";
				ob_flush();
				flush();
			}
		}
		...
		try
		{
			...
			{	
				...
				$mail->FromName = "GIOSS";
				$mail->Subject = "Corrector CANCER 0247 ";
				$mail->AltBody = "Cordial saludo,\n El sistema ha reparado el archivo";
		    
				$mail->MsgHTML("Cordial saludo,\n El sistema ha adjuntado el archivo de cancer reparado para la EAPB $cod_eapb.<strong>GIOSS</strong>.");
				...
				else 
				{
				    ...
				    if(connection_aborted()==false)
				    {
					echo "<script>alert('Se ha enviado una copia del archivo reparado a su correo $correo_electronico')</script>";
				    }
				}
	...
	else
	{
		if($errores!="")
		{
			...
			{
				echo "<script>document.getElementById('div_mensaje_error').style.display='inline';</script>";
				echo "<script>document.getElementById('parrafo_mensaje_error').innerHTML='$errores <br> No se ha reparado el archivo <br> $botones';</script>";
				ob_flush();
				flush();
			}
		}
		...
		if($bool_esta_siendo_reparado)
		{
			...
			{
				echo "<script>document.getElementById('div_mensaje_error').style.display='inline';</script>";
				echo "<script>document.getElementById('parrafo_mensaje_error').innerHTML='<br>El archivo 0247 esta siendo reparado actualmente.';</script>";
				ob_flush();
				flush();
			}
		}
