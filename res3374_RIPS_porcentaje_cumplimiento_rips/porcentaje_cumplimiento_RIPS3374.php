...
$smarty->display('porcentaje_cumplimiento_RIPS3374.html.tpl');
...
if(isset($_POST["year_de_validacion"]) && isset($_POST["eapb"]) && $_POST["year_de_validacion"]!="" && $_POST["eapb"]!="none")
{
	$cod_eapb=$_POST["eapb"];
	...
	$nombre_vista_porcentaje_cumplimiento_rips="vpcfr_".$nick_user."_".$tipo_id."_".$identificacion;
	
	$sql_vista_cumplimiento_facturas_rips="";
	$sql_vista_cumplimiento_facturas_rips.="CREATE OR REPLACE VIEW $nombre_vista_porcentaje_cumplimiento_rips ";
	$sql_vista_cumplimiento_facturas_rips.=" AS SELECT DISTINCT codigo_prestador ";
	$sql_vista_cumplimiento_facturas_rips.=" FROM gioss_facturas_radicadas ft ";
	//LEER se hace left join para poder permitir la busqueda por departamento y ciudad
	$sql_vista_cumplimiento_facturas_rips.=" LEFT JOIN  gios_prestador_servicios_salud ps ";
	$sql_vista_cumplimiento_facturas_rips.=" ON (ft.codigo_prestador=ps.cod_registro_especial_pss) WHERE  ";
	$sql_vista_cumplimiento_facturas_rips.=" (fecha_radicacion BETWEEN '$fecha_inicio' AND '$fecha_fin' ) ";
	$sql_vista_cumplimiento_facturas_rips.=" AND ";
	$sql_vista_cumplimiento_facturas_rips.=" codigo_eapb='$cod_eapb' ";
	if($codigo_departamento!="")
	{
		$sql_vista_cumplimiento_facturas_rips.=" AND ";
		$sql_vista_cumplimiento_facturas_rips.=" cod_depto='$codigo_departamento' ";
	}
	if($codigo_departamento!="" && $codigo_municipio!="")
	{
		$sql_vista_cumplimiento_facturas_rips.=" AND ";
		$sql_vista_cumplimiento_facturas_rips.=" cod_municipio='$codigo_municipio' ";
	}
	if($cod_prestador!="")
	{
		$sql_vista_cumplimiento_facturas_rips.=" AND ";
		$sql_vista_cumplimiento_facturas_rips.=" codigo_prestador='$cod_prestador' ";
	}
	$sql_vista_cumplimiento_facturas_rips.="  ; ";
	$error_bd_seq="";
	$bool_hubo_error_query=$coneccionBD->insertar_no_warning_get_error($sql_vista_cumplimiento_facturas_rips, $error_bd_seq);
	if($error_bd_seq!="")
	{
		echo "<script>alert('error al crear vista(s) ".procesar_mensaje($error_bd_seq)." ".procesar_mensaje($sql_vista_cumplimiento_facturas_rips)."');</script>";
	}
	
	//echo "<script>alert(' ".procesar_mensaje($sql_vista_cumplimiento_facturas_rips)."');</script>";
	
	$numero_registros_vista=0;
	$sql_numero_registros_vista="";
	$sql_numero_registros_vista.="SELECT count(*) as contador FROM $nombre_vista_porcentaje_cumplimiento_rips  ";	
	$sql_numero_registros_vista.=" ; ";
	...
	while($contador_offset<$numero_registros_vista)
	{
		...
		$sql_query_busqueda="";
		$sql_query_busqueda.="SELECT * FROM $nombre_vista_porcentaje_cumplimiento_rips LIMIT $limite OFFSET $contador_offset;  ";
		...
		if(count($resultado_para_calidad_de_datos_desde_estado_informacion)>0)
		{
			...
			$nombre_archivo_calidad_de_datos=$cod_eapb."_porcentaje_cumplimiento_rips.csv";
			$ruta_archivo_porcentaje_cumplimiento=$rutaTemporal.$nombre_archivo_calidad_de_datos;
			
			//PARTE INDICAR HUBO RESULTADOS UNA SOLA VEZ Y CREAR EL ARCHIVO EN BLANCO
			if($hubo_resultados==false)
			{
				$file_cumplimiento= fopen($ruta_archivo_porcentaje_cumplimiento, "w") or die("fallo la creacion del archivo");
				fclose($file_cumplimiento);
			
				$html_abrir_ventana="";
				$html_abrir_ventana.="<script>";
				$html_abrir_ventana.="ventana_detalle=window.open ('','ventana_info_consolidado_cumplimiento_fact', config='height=600,width=1280, toolbar=no, menubar=no, scrollbars=yes, resizable=yes,location=no, directories=no, status=no');";
				$html_abrir_ventana.="</script>";
				...
				$html_nueva_ventana.="<title>Cumplimiento Facturas Consolidado</title>";
				...
				$file_cumplimiento= fopen($ruta_archivo_porcentaje_cumplimiento, "a") or die("fallo la creacion del archivo");
				...
				fwrite($file_cumplimiento, $linea_encabezado."\n");
				//encabezado
				
				$titulos="";
				$titulos.="Cod. Depto. Prestador,Nombre Depto. Prestador,Cod. Mpio. Prestador,Nombre Mpio. Prestador,Tipo ID Prestador,";
				$titulos.="N. ID Prestador,Cod. Prestador de Servicios,Nombre Prestador de Servicios,Facturas Radicadas,Facturas con RIPS Aceptados,";
				$titulos.="Factura Con RIPS Rechazados,Facturas Radicadas sin RIPS,Valor Facturas Radicadas,Valor Facturas con RIPS Aceptados,";
				$titulos.="Valor Facturas con RIPS Rechazados,Valor Facturas SIN RIPS,Porcentaje de Cumplimiento RIPS,Porcentaje de NO cumplimiento RIPS,";
				$titulos.="Porcentaje de Valor con Factura RIPS,Porcentaje de Valor con Factura SIN RIPS";				
				fwrite($file_cumplimiento, $titulos."\n");
			...
			foreach($resultado_para_calidad_de_datos_desde_estado_informacion as $resultado)
			{
				...
				//cuenta facturas radicadas con facturas rips validados con exito				
				$sql_contador_facturas_radicadas_rips_aceptados="";
				$sql_contador_facturas_radicadas_rips_aceptados.=" SELECT count(*) AS cantidad_facturas_con_rips_aceptados, ";
				$sql_contador_facturas_radicadas_rips_aceptados.=" SUM(COALESCE(valor_factura_rips::numeric,'0')) AS valor_facturas_con_rips_aceptados ";
				$sql_contador_facturas_radicadas_rips_aceptados.=" FROM gioss_facturas_radicadas ft ";
				//LEER se hace left join para poder permitir la busqueda por departamento y ciudad
				$sql_contador_facturas_radicadas_rips_aceptados.=" LEFT JOIN  gios_prestador_servicios_salud ps ";
				$sql_contador_facturas_radicadas_rips_aceptados.=" ON (ft.codigo_prestador=ps.cod_registro_especial_pss) WHERE  ";
				$sql_contador_facturas_radicadas_rips_aceptados.=" (fecha_radicacion BETWEEN '$fecha_inicio' AND '$fecha_fin' ) ";
				$sql_contador_facturas_radicadas_rips_aceptados.=" AND ";
				$sql_contador_facturas_radicadas_rips_aceptados.=" codigo_eapb='$cod_eapb' ";
				$sql_contador_facturas_radicadas_rips_aceptados.=" AND ";
				$sql_contador_facturas_radicadas_rips_aceptados.=" codigo_prestador='".$resultado["codigo_prestador"]."' ";
				$sql_contador_facturas_radicadas_rips_aceptados.=" AND ";
				$sql_contador_facturas_radicadas_rips_aceptados.=" estado_verificacion_factura='1' ";
				if($codigo_departamento!="")
				{
					$sql_contador_facturas_radicadas_rips_aceptados.=" AND ";
					$sql_contador_facturas_radicadas_rips_aceptados.=" cod_depto='$codigo_departamento' ";
				}
				if($codigo_departamento!="" && $codigo_municipio!="")
				{
					$sql_contador_facturas_radicadas_rips_aceptados.=" AND ";
					$sql_contador_facturas_radicadas_rips_aceptados.=" cod_municipio='$codigo_municipio' ";
				}								
				$sql_contador_facturas_radicadas_rips_aceptados.="  ; ";
				$resultado_para_facturas_radicadas_rips_aceptados=$coneccionBD->consultar2($sql_contador_facturas_radicadas_rips_aceptados);
				$total_facturas_radicadas_rips_aceptados="";
				$valor_facturas_radicadas_rips_aceptados=0;
				if(is_array($resultado_para_facturas_radicadas_rips_aceptados) && count($resultado_para_facturas_radicadas_rips_aceptados)>0)
				{
					$total_facturas_radicadas_rips_aceptados=$resultado_para_facturas_radicadas_rips_aceptados[0]["cantidad_facturas_con_rips_aceptados"];
					$valor_facturas_radicadas_rips_aceptados=floatval($resultado_para_facturas_radicadas_rips_aceptados[0]["valor_facturas_con_rips_aceptados"]);
				}
				//fin cuenta facturas radicadas con facturas rips validados con exito
				
				//cuenta facturas radicadas con facturas rips rechazadas	
				$sql_contador_facturas_radicadas_rips_rechazados="";
				$sql_contador_facturas_radicadas_rips_rechazados.=" SELECT count(*) AS cantidad_facturas_con_rips_rechazados, ";
				$sql_contador_facturas_radicadas_rips_rechazados.=" SUM(COALESCE(valor_factura_rips::numeric,'0')) AS valor_facturas_con_rips_rechazados ";
				$sql_contador_facturas_radicadas_rips_rechazados.=" FROM gioss_facturas_radicadas ft ";
				//LEER se hace left join para poder permitir la busqueda por departamento y ciudad
				$sql_contador_facturas_radicadas_rips_rechazados.=" LEFT JOIN  gios_prestador_servicios_salud ps ";
				$sql_contador_facturas_radicadas_rips_rechazados.=" ON (ft.codigo_prestador=ps.cod_registro_especial_pss) WHERE  ";
				$sql_contador_facturas_radicadas_rips_rechazados.=" (fecha_radicacion BETWEEN '$fecha_inicio' AND '$fecha_fin' ) ";
				$sql_contador_facturas_radicadas_rips_rechazados.=" AND ";
				$sql_contador_facturas_radicadas_rips_rechazados.=" codigo_eapb='$cod_eapb' ";
				$sql_contador_facturas_radicadas_rips_rechazados.=" AND ";
				$sql_contador_facturas_radicadas_rips_rechazados.=" codigo_prestador='".$resultado["codigo_prestador"]."' ";
				$sql_contador_facturas_radicadas_rips_rechazados.=" AND ";
				$sql_contador_facturas_radicadas_rips_rechazados.=" estado_verificacion_factura='2' ";
				if($codigo_departamento!="")
				{
					$sql_contador_facturas_radicadas_rips_rechazados.=" AND ";
					$sql_contador_facturas_radicadas_rips_rechazados.=" cod_depto='$codigo_departamento' ";
				}
				if($codigo_departamento!="" && $codigo_municipio!="")
				{
					$sql_contador_facturas_radicadas_rips_rechazados.=" AND ";
					$sql_contador_facturas_radicadas_rips_rechazados.=" cod_municipio='$codigo_municipio' ";
				}								
				$sql_contador_facturas_radicadas_rips_rechazados.="  ; ";
				$resultado_para_facturas_radicadas_rips_rechazados=$coneccionBD->consultar2($sql_contador_facturas_radicadas_rips_rechazados);
				$total_facturas_radicadas_rips_rechazados="";
				$valor_facturas_radicadas_rips_rechazados=0;
				if(is_array($resultado_para_facturas_radicadas_rips_rechazados) && count($resultado_para_facturas_radicadas_rips_rechazados)>0)
				{
					$total_facturas_radicadas_rips_rechazados=$resultado_para_facturas_radicadas_rips_rechazados[0]["cantidad_facturas_con_rips_rechazados"];
					$valor_facturas_radicadas_rips_rechazados=floatval($resultado_para_facturas_radicadas_rips_rechazados[0]["valor_facturas_con_rips_rechazados"]);
				}
				//cuenta facturas radicadas con facturas rips rechazadas
				
				//cuenta facturas radicadas sin facturas rips
				$sql_contador_facturas_radicadas_sin_rips="";
				$sql_contador_facturas_radicadas_sin_rips.=" SELECT count(*) AS cantidad_facturas_sin_rips, ";
				$sql_contador_facturas_radicadas_sin_rips.=" SUM(COALESCE(valor_factura::numeric,'0')) AS valor_facturas_sin_rips ";
				$sql_contador_facturas_radicadas_sin_rips.=" FROM gioss_facturas_radicadas ft ";
				//LEER se hace left join para poder permitir la busqueda por departamento y ciudad
				$sql_contador_facturas_radicadas_sin_rips.=" LEFT JOIN  gios_prestador_servicios_salud ps ";
				$sql_contador_facturas_radicadas_sin_rips.=" ON (ft.codigo_prestador=ps.cod_registro_especial_pss) WHERE  ";
				$sql_contador_facturas_radicadas_sin_rips.=" (fecha_radicacion BETWEEN '$fecha_inicio' AND '$fecha_fin' ) ";
				$sql_contador_facturas_radicadas_sin_rips.=" AND ";
				$sql_contador_facturas_radicadas_sin_rips.=" codigo_eapb='$cod_eapb' ";
				$sql_contador_facturas_radicadas_sin_rips.=" AND ";
				$sql_contador_facturas_radicadas_sin_rips.=" codigo_prestador='".$resultado["codigo_prestador"]."' ";
				$sql_contador_facturas_radicadas_sin_rips.=" AND ";
				$sql_contador_facturas_radicadas_sin_rips.=" estado_verificacion_factura='3' ";
				if($codigo_departamento!="")
				{
					$sql_contador_facturas_radicadas_sin_rips.=" AND ";
					$sql_contador_facturas_radicadas_sin_rips.=" cod_depto='$codigo_departamento' ";
				}
				if($codigo_departamento!="" && $codigo_municipio!="")
				{
					$sql_contador_facturas_radicadas_sin_rips.=" AND ";
					$sql_contador_facturas_radicadas_sin_rips.=" cod_municipio='$codigo_municipio' ";
				}								
				$sql_contador_facturas_radicadas_sin_rips.="  ; ";
				$resultado_para_facturas_radicadas_sin_rips=$coneccionBD->consultar2($sql_contador_facturas_radicadas_sin_rips);
				$total_facturas_radicadas_sin_rips="";
				$valor_facturas_radicadas_sin_rips=0;
				if(is_array($resultado_para_facturas_radicadas_sin_rips) && count($resultado_para_facturas_radicadas_sin_rips)>0)
				{
					$total_facturas_radicadas_sin_rips=$resultado_para_facturas_radicadas_sin_rips[0]["cantidad_facturas_sin_rips"];
					$valor_facturas_radicadas_sin_rips=floatval($resultado_para_facturas_radicadas_sin_rips[0]["valor_facturas_sin_rips"]);
				}
				//fin cuenta facturas radicadas sin facturas
				
				$total_facturas_radicadas=0;
				$total_facturas_radicadas=intval($total_facturas_radicadas_rips_aceptados)+intval($total_facturas_radicadas_rips_rechazados)+intval($total_facturas_radicadas_sin_rips);
				$linea_cumplimiento_factura.=$total_facturas_radicadas.",";
				
				
				$linea_cumplimiento_factura.=$total_facturas_radicadas_rips_aceptados.",";
				
				$linea_cumplimiento_factura.=$total_facturas_radicadas_rips_rechazados.",";
				
				$linea_cumplimiento_factura.=$total_facturas_radicadas_sin_rips.",";
				
				$valor_facturas_fisicas=0;
				$sql_valor_facturas_fisicas="";
				$sql_valor_facturas_fisicas.=" SELECT  ";
				$sql_valor_facturas_fisicas.=" SUM(COALESCE(valor_factura::numeric,'0')) AS valor_facturas_fisicas ";
				$sql_valor_facturas_fisicas.=" FROM gioss_facturas_radicadas ft ";
				//LEER se hace left join para poder permitir la busqueda por departamento y ciudad
				$sql_valor_facturas_fisicas.=" LEFT JOIN  gios_prestador_servicios_salud ps ";
				$sql_valor_facturas_fisicas.=" ON (ft.codigo_prestador=ps.cod_registro_especial_pss) WHERE  ";
				$sql_valor_facturas_fisicas.=" (fecha_radicacion BETWEEN '$fecha_inicio' AND '$fecha_fin' ) ";
				$sql_valor_facturas_fisicas.=" AND ";
				$sql_valor_facturas_fisicas.=" codigo_eapb='$cod_eapb' ";
				$sql_valor_facturas_fisicas.=" AND ";
				$sql_valor_facturas_fisicas.=" codigo_prestador='".$resultado["codigo_prestador"]."' ";
				if($codigo_departamento!="")
				{
					$sql_valor_facturas_fisicas.=" AND ";
					$sql_valor_facturas_fisicas.=" cod_depto='$codigo_departamento' ";
				}
				if($codigo_departamento!="" && $codigo_municipio!="")
				{
					$sql_valor_facturas_fisicas.=" AND ";
					$sql_valor_facturas_fisicas.=" cod_municipio='$codigo_municipio' ";
				}								
				$sql_valor_facturas_fisicas.="  ; ";
				$resultado_para_valor_facturas_fisicas=$coneccionBD->consultar2($sql_valor_facturas_fisicas);
				if(is_array($resultado_para_valor_facturas_fisicas) && count($resultado_para_valor_facturas_fisicas)>0)
				{
					$valor_facturas_fisicas=floatval($resultado_para_valor_facturas_fisicas[0]["valor_facturas_fisicas"]);
				}
				$linea_cumplimiento_factura.=$valor_facturas_fisicas.",";
				
				
				$linea_cumplimiento_factura.=$valor_facturas_radicadas_rips_aceptados.",";
				
				$linea_cumplimiento_factura.=$valor_facturas_radicadas_rips_rechazados.",";
				
				$linea_cumplimiento_factura.=$valor_facturas_radicadas_sin_rips.",";
				
				$porcentaje_cumplimiento_facturas=0;
				if(floatval($total_facturas_radicadas)>0)
				{
					$porcentaje_cumplimiento_facturas=round((floatval($total_facturas_radicadas_rips_aceptados)/floatval($total_facturas_radicadas))*100,1,PHP_ROUND_HALF_UP);
				}
				$linea_cumplimiento_factura.=$porcentaje_cumplimiento_facturas.",";
				
				$porcentaje_no_cumplimiento_facturas=0;
				if(floatval($total_facturas_radicadas)>0)
				{
					$porcentaje_no_cumplimiento_facturas=round(((floatval($total_facturas_radicadas_rips_rechazados)+floatval($total_facturas_radicadas_sin_rips))/floatval($total_facturas_radicadas))*100,1,PHP_ROUND_HALF_UP);
				}
				$linea_cumplimiento_factura.=$porcentaje_no_cumplimiento_facturas.",";
				
				$porcentaje_valor_cumplimiento_facturas=0;
				if(floatval($valor_facturas_fisicas)>0)
				{
					$porcentaje_valor_cumplimiento_facturas=round((floatval($valor_facturas_radicadas_rips_aceptados)/floatval($valor_facturas_fisicas))*100,1,PHP_ROUND_HALF_UP);
				}
				$linea_cumplimiento_factura.=$porcentaje_valor_cumplimiento_facturas.",";
				
				$porcentaje_valor_no_cumplimiento_facturas=0;
				if(floatval($valor_facturas_fisicas)>0)
				{
					$porcentaje_valor_no_cumplimiento_facturas=round(((floatval($valor_facturas_radicadas_rips_rechazados)+floatval($valor_facturas_radicadas_sin_rips))/floatval($valor_facturas_fisicas))*100,1,PHP_ROUND_HALF_UP);
				}
				$linea_cumplimiento_factura.=$porcentaje_valor_no_cumplimiento_facturas;
				
				fwrite($file_cumplimiento, $linea_cumplimiento_factura."\n");
	...
	if($hubo_resultados)
	{
		...
		$html_reabrir_ventana.="{";
		$html_reabrir_ventana.="ventana_detalle=window.open ('','ventana_info_consolidado_cumplimiento_fact', config='height=600,width=1280, toolbar=no, menubar=no, scrollbars=yes, resizable=yes,location=no, directories=no, status=no');";
		...
		$boton_descarga=" <input type=\'button\' value=\'Descargar archivo con el reporte de calidad de datos para RIPS\'  class=\'btn btn-success color_boton\' onclick=\"download_archivo(\'$ruta_archivo_porcentaje_cumplimiento\');\"/> ";
	...
	//borrando vistas
	$sql_borrar_vistas="";
	$sql_borrar_vistas.=" DROP VIEW $nombre_vista_porcentaje_cumplimiento_rips ; ";
