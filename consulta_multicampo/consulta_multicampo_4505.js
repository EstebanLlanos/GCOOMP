...
async function reasignar_valores_consulta_multicampo_campo_actual(selector_all_or_one_vp_param,selector_general_o_detallado_cross_param,array_numero_campo_para_reasignar_param,array_vp_campo_para_reasignar_param)
{
  document.getElementById('selector_all_or_one_vp').value=selector_all_or_one_vp_param;
  all_or_one_vp();
  document.getElementById('selector_general_o_detallado_cross').value=selector_general_o_detallado_cross_param;
  ...
  var cont_temp=0;
  while(cont_temp<array_numero_campo_para_reasignar_param.length)
  {
    id_numerico_campo=cont_temp+1;
    document.getElementById('campocross_'+id_numerico_campo).value=array_numero_campo_para_reasignar_param[cont_temp];
    consultar_valor_permitido_campo_cruzado('divcampocrossvp_'+id_numerico_campo,id_numerico_campo);
    cont_temp++;
  }//fin while
  await sleep(2000);
  var cont_vp_temp=0;
  while(cont_vp_temp<array_vp_campo_para_reasignar_param.length)
  {
    id_numerico_campo=cont_vp_temp+1;
    valor_actual=array_vp_campo_para_reasignar_param[cont_vp_temp];
    id_selector_vp_actual='campocrossvp_'+id_numerico_campo;
    document.getElementById(id_selector_vp_actual).value=valor_actual;
    cont_vp_temp++;
  }//fin while
}//fin function
...
async function adicionar_campo_n_veces(n_veces,selector_all_or_one_vp_param,selector_general_o_detallado_cross_param,array_numero_campo_para_reasignar_param,array_vp_campo_para_reasignar_param)
{
  /*
  n_cont=0;
  
  while(n_cont<n_veces-2)//debido a que por defecto tiene 2
  {
    adicionar_campo_para_cruce();
    
    n_cont++;
    await sleep(n_cont*1500);
  }//fin while
  */

  adicionar_campos_para_cruce_multicampo(array_numero_campo_para_reasignar_param,array_vp_campo_para_reasignar_param);
  await sleep(1500);//n_cont no es necesario
  reasignar_valores_consulta_multicampo_campo_actual(selector_all_or_one_vp_param,selector_general_o_detallado_cross_param,array_numero_campo_para_reasignar_param,array_vp_campo_para_reasignar_param);
}//fin function
...
function adicionar_campo_para_cruce()
{
   ...
   numero_maximo_campos_para_cruce=30;
   while(document.getElementById('campocross_'+numero_campocross_actual) && numero_campocross_actual<=numero_maximo_campos_para_cruce)
   {
   ...
   if(numero_campocross_actual>=2  && numero_campocross_actual<=numero_maximo_campos_para_cruce)
   {
...
function adicionar_campos_para_cruce_multicampo(array_numero_campo_para_reasignar_param,array_vp_campo_para_reasignar_param)
{
    //alert('pre 1: '+array_numero_campo_para_reasignar_param.toString()+'\npre 2:'+array_vp_campo_para_reasignar_param.toString() );

   numero_campocross_actual=0;//inicia desde cero porque es el array y no el dom
   string_valores_seleccionados_campo="";
   string_valores_seleccionados_valor_permitido_campo="";
   numero_maximo_campos_para_cruce=30;
   while(numero_campocross_actual<array_numero_campo_para_reasignar_param.length && numero_campocross_actual<=numero_maximo_campos_para_cruce)
   {
      if(string_valores_seleccionados_campo!=""){string_valores_seleccionados_campo+="ZsepZ";}
      string_valores_seleccionados_campo+=array_numero_campo_para_reasignar_param[numero_campocross_actual];
      if(typeof array_vp_campo_para_reasignar_param[numero_campocross_actual] !== 'undefined' )
      {
        if(string_valores_seleccionados_valor_permitido_campo!=""){string_valores_seleccionados_valor_permitido_campo+="ZsepZ";}
        string_valores_seleccionados_valor_permitido_campo+=array_vp_campo_para_reasignar_param[numero_campocross_actual];
      }//fin if
      numero_campocross_actual++;
   }//fin while
   ...
   if(numero_campocross_actual>=2  && numero_campocross_actual<=numero_maximo_campos_para_cruce)
   {
      ConsultaAJAX_Async("numero_campocross_actual="+numero_campocross_actual+"&str_array_campos="+string_valores_seleccionados_campo+"&str_array_vpcampos="+string_valores_seleccionados_valor_permitido_campo,"selectores_campos_cruzados_multicampo.php","div_para_consultas_cruzadas");
    }
    else
    {
      alert('El limite es de '+(numero_campocross_actual-1)+' campos para cruzar.');
...
function reset_adicionar_campos()
{
 numero_campocross_actual=1;//inicia desde uno
 string_valores_seleccionados_campo="";
 string_valores_seleccionados_valor_permitido_campo="";
 numero_maximo_campos_para_cruce=30;
 while(document.getElementById('campocross_'+numero_campocross_actual) && numero_campocross_actual<2)
 {
    if(string_valores_seleccionados_campo!=""){string_valores_seleccionados_campo+="ZsepZ";}
    string_valores_seleccionados_campo+=document.getElementById('campocross_'+numero_campocross_actual).value;
    if(document.getElementById('campocrossvp_'+numero_campocross_actual) )
...
function reset_numero_campos()
{
  reset_adicionar_campos();
  /*
  var original= '<table style=\'width: 50%;\'>'+
'                     '+
'                     <tr>'+
...
'                   <tr>'+
'                     <td colspan=\'2\' style=\'text-align:left;\'>'+
'                       <h6  id=\'sub_titulo_conteo_detallado_cross\' style=\'color:#0000FF;text-shadow: 2px 2px 5px #A8A8FF;\'>Seleccione si se mostrara el detallado del campo seleccionado,<br>(registros con el valor permitido seleccionado para dicho campo, datos de identificacion y valor del campo en cuestión )<br> o solo su conteo:</h6><br>'+
...
'                   <input type=\'button\' id=\'boton_reset_campos\' name=\'boton_reset_campos\' class=\'btn btn-success color_boton\' value=\'Reset Numero Campos\' onclick=\'\' style=\'display:inline;\'/>';
document.getElementById('div_para_consultas_cruzadas').innerHTML=original;
*/
}//fin function
