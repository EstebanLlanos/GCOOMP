...
if(isset($_REQUEST["prestador"]) && isset($_REQUEST["periodo"]) && isset($_REQUEST["year"]) && $_REQUEST["year"]!="" && strlen($_REQUEST["year"])==4 && $_REQUEST["prestador"]!="none" )
{
	...
	//parte base de datos
	$query_consulta="";
	$query_consulta.="SELECT nombre_archivo,numero_secuencia FROM gioss_numero_de_secuencia_r0247_cancer WHERE ";
	if($periodo!="none")
	{
	...
	{		
		...
		$selector_numeros_remision.="<select name='nombres_archivos_varios' id='nombres_archivos_varios' class='campo_azul' size='$size_selector' style='width:600px;' >";
		$selector_numeros_remision.="<option value='none' selected>Seleccione un archivo CANCER 0247</option>";
		foreach($resultado_query as $fila_resultado)
		{
			$selector_numeros_remision.="<option value='".$fila_resultado["numero_secuencia"]."'>Numero Secuencia: ".$fila_resultado["numero_secuencia"].", Nombre Archivo: ".$fila_resultado["nombre_archivo"]."</option>";
		}
		$selector_numeros_remision.="</select>";
	}
	else
	{
		$selector_numeros_remision.="No se encontraron archivos 0247 para el periodo y/o a&ntildeo establecidos.";
	}
