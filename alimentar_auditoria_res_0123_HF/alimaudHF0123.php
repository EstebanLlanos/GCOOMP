...
$smarty->display('alimaudHF0123.html.tpl');

/*
INSERT INTO gios_menus_opciones_sistema(id_principal,id_padre,nombre_opcion,descripcion_ayuda,tiene_submenus,ruta_interfaz,prioridad_jerarquica) VALUES ('235','171','Alimentar Auditoria','',FALSE,'..|alimentar_auditoria_res_0123_HF|alimaudHF0123.php','50.02');

INSERT INTO gios_menus_perfiles(id_menu,id_perfil) VALUES ('235','5'); --admin sistema
*/
...
)//fin if
{
	...
	--PRIMARY KEY tabla_auditoria_usuarios_reportados_hemofilia

	ALTER TABLE tabla_auditoria_usuarios_reportados_hemofilia DROP CONSTRAINT tabla_auditoria_usuarios_reportados_hemofilia_pkey;
	ALTER TABLE tabla_auditoria_usuarios_reportados_hemofilia ADD CONSTRAINT tabla_auditoria_usuarios_reportados_hemofilia_pkey PRIMARY KEY (consecutivo_registro, periodo_reportado, fecha_de_corte,  codigo_entidad_reporta);

	--insert tipo cohorte y update
	UPDATE tabla_auditoria_tipo_cohortes set descripcion='Cambio de DX' where codigo='5';
	INSERT INTO tabla_auditoria_tipo_cohortes(codigo,descripcion) VALUES('6','Cambio de Severidad');

	--TABLA INDICE PERIODOS USUARIOS REPORTADOS HEMOFILIA
	CREATE TABLE tabla_aud_us_rep_hf_indice
	(
	...
	--insert del periodo ya subido para evitar que se reescriba por accidente
	INSERT INTO tabla_aud_us_rep_hf_indice(periodo_reportado,fecha_de_corte,codigo_entidad_reporta,numero_total_registros) VALUES('2016','2017-01-31','EMP028','43');
	 
	*/

	//CONSULTA MAXIMA FECHA DE CORTE EN REPORTE OBLIGATORIO
	$query_busca_ultima_fecha_reporte_obligatorio="SELECT max(fecha_corte_reporte) as maxima_fecha_corte from gioss_consulta_reporte_obligatorio_hf0123_exitoso WHERE codigo_entidad_eapb_generadora='$codigo_entidad_reporta';";
	...
	$query_buscar_ultimo_periodo_en_tabla_usuarios_reportados_cohorte="SELECT max(periodo_reportado::numeric) as maximo_periodo from tabla_auditoria_usuarios_reportados_hemofilia WHERE codigo_entidad_reporta='$codigo_entidad_reporta' ;";
	...
	{
		$query_delete_registros_proceso_incompleto="DELETE * FROM tabla_auditoria_usuarios_reportados_hemofilia WHERE periodo_reportado='$periodo_indicado' AND codigo_entidad_reporta='$codigo_entidad_reporta' ;";
		$error_bd_seq="";
	...
	if($fecha_corte_ultimo_periodo!="" && $periodo_anterior!="" && $bool_fue_diligenciado==false)
	{
		...
		$nombre_view_ultimo_reporte_obligatorio_generado="valim0123aud_".$nick_user.str_replace("-", "", $fecha_corte_ultimo_periodo);
		$query_view_ultimo_reporte_obligatorio="CREATE OR REPLACE VIEW $nombre_view_ultimo_reporte_obligatorio_generado AS 
		( 
			SELECT * FROM gioss_consulta_reporte_obligatorio_hf0123_exitoso WHERE fecha_corte_reporte='$fecha_corte_ultimo_periodo' ORDER BY numero_registro ASC
		);
		";
		$error_bd_seq="";
		...
		$inicializa_contador_campos_en=0;//esta variable almacen el valor en que se deben inicializar lso contadores de campos para bd
		$numero_total_campos_norma=96;
		 ...
		 )//fin if
		{
			...
			while($contador_offset<$numero_registros)
			{
				...
				{
					$valor_campo_23_hemofilia=trim($registro['campo_hf_de_numero_orden_22']);
					$valor_campo_24_hemofilia=trim($registro['campo_hf_de_numero_orden_23']);
					$tipo_identificacion_campo_5_hemofilia=trim($registro['campo_hf_de_numero_orden_4']);
					$numero_identificacion_campo_6_hemofilia=trim($registro['campo_hf_de_numero_orden_5']);

					$array_campos_registro=array();
					$contador_campos=0;
					while(isset($registro['campo_hf_de_numero_orden_'.$contador_campos])==true)
					{
						$array_campos_registro[$contador_campos]=trim($registro['campo_hf_de_numero_orden_'.$contador_campos]);
						$contador_campos++;
					}//fin while

					//aqui va query busqueda si fue reportado previamente el usuario en tabla_auditoria_usuarios_reportados_hemofilia
					$query_buscar_paciente="SELECT * FROM tabla_auditoria_usuarios_reportados_hemofilia WHERE campo_hemofilia_4='$tipo_identificacion_campo_5_hemofilia' AND campo_hemofilia_5='$numero_identificacion_campo_6_hemofilia' AND periodo_reportado='$periodo_anterior' ;";
					$resultados_buscar_paciente=$coneccionBD->consultar2_no_crea_cierra($query_buscar_paciente);


					

					//EVALUA CRITERIOS PARA CUANDO EXISTE Y CUANDO NO EXISTE EN EL REPORTE DE USUARIOS HEMOFILIA
					if(count($resultados_buscar_paciente)>0 && is_array($resultados_buscar_paciente)==true)
					{
						//ANTERIORES
						if($valor_campo_23_hemofilia=="0" || $valor_campo_23_hemofilia=="1")
						{
							//COHORETE HEMOFILIA
							$tipo_cohorte_auditoria_para_insert="2";
						}//fin if
						else if(
							$valor_campo_23_hemofilia=="2"
							|| $valor_campo_23_hemofilia=="3"
							|| $valor_campo_23_hemofilia=="4"
							|| $valor_campo_23_hemofilia=="5"
							|| $valor_campo_23_hemofilia=="6"
							|| $valor_campo_23_hemofilia=="7"
							|| $valor_campo_23_hemofilia=="8"
							|| $valor_campo_23_hemofilia=="9"
							|| $valor_campo_23_hemofilia=="10"
							|| $valor_campo_23_hemofilia=="11"
						)//fin if
						{
							//OTRAS COAGULOPATIAS
						...
						$valor_campo_23_usuarios_reportados_hemofilia=trim($resultados_buscar_paciente['campo_hemofilia_22']);
						$valor_campo_24_usuarios_reportados_hemofilia=trim($resultados_buscar_paciente['campo_hemofilia_23']);
						if($valor_campo_23_usuarios_reportados_hemofilia!=$valor_campo_23_hemofilia)
						{
							//I. definicion tecnica cambio de diagnostico
							$tipo_cohorte_auditoria_para_insert="5";
						}//fin if
						else if(
							$valor_campo_23_usuarios_reportados_hemofilia==$valor_campo_23_hemofilia 
							&& $valor_campo_24_hemofilia!=$valor_campo_24_usuarios_reportados_hemofilia
						)//fin condicion
						{
							if($valor_campo_23_hemofilia=="0" || $valor_campo_23_hemofilia=="1")
							{
					...
					//FIN EVALUA CRITERIOS PARA CUANDO EXISTE Y CUANDO NO EXISTE EN EL REPORTE DE USUARIOS HEMOFILIA
					...
					)
					{
						...
						$query_preparacion_insert.="INSERT INTO tabla_auditoria_usuarios_reportados_hemofilia
							(
							";
						$cont_campos_ins_hf=$inicializa_contador_campos_en;
						while ($cont_campos_ins_hf<$numero_total_campos_norma) 
						{
							$query_preparacion_insert.="campo_hemofilia_".$cont_campos_ins_hf.",";
							$cont_campos_ins_hf++;
						}//fin while
						$query_preparacion_insert.="
						periodo_reportado,
						...
						while ($cont_campos_ins_hf<$numero_total_campos_norma) 
						{
							$query_preparacion_insert.="'".trim($registro['campo_hf_de_numero_orden_'.$cont_campos_ins_hf])."',";
							$cont_campos_ins_hf++;
						}//fin while
						$query_preparacion_insert.="
						'".$periodo_reportado_para_insert."',
