...
function corrige_longitud_fecha($array_fecha_corregida,$fecha_corte,$fase=0)
{
    ...
    {    
	    ...
	    //FIN PARTE YEAR
            ...
            {
                $array_fecha_corregida[1]=substr("".intval($array_fecha_corregida[1]),0,2);
            }//fin else if
	    //FIN PARTE MES
            ...
            {
                $array_fecha_corregida[2]=substr("".intval($array_fecha_corregida[2]),0,2);
            }//fin else if
	    //FIN PARTE DIA
...
$selector_periodo.="<select id='periodo' name='periodo' class='campo_azul' onchange='validar_antes_seleccionar_archivos();' style='width:230px;' >";
...
$ruta_archivo_inconsistencias_CANCER="";
$ruta_archivo_separados_diagnostico_no_corresponde="";
...
if(isset($_POST["accion"]) && $_POST["accion"]=="validar" && isset($_FILES["0247_cancer_file"]) && $_FILES["0247_cancer_file"]["error"]==0)
{
	...
	$ruta_archivo_separados_diagnostico_no_corresponde=$rutaTemporal."reg_no_son_cancer_".$cod_prestador."_".$fecha_actual."_".$tiempo_actual_string.".txt";
	if(file_exists($ruta_archivo_separados_diagnostico_no_corresponde))
	{
		unlink($ruta_archivo_separados_diagnostico_no_corresponde);
	}
	
	$file_archivo_separados_diagnostico_no_corresponde = fopen($ruta_archivo_separados_diagnostico_no_corresponde, "w") or die("fallo la creacion del archivo");
	$descripcion_separado_por_campo17="REGISTROS EXCLUIDOS DEBIDO A QUE EL CODIGO CIEX NO CORRESPONDE A CANCER";
	fwrite($file_archivo_separados_diagnostico_no_corresponde,$descripcion_separado_por_campo17);
	//fin archivo donde se separa los que no estan en ciex del campo 17
	...
	$ruta_inconsistencias_registros_malos=$rutaTemporal."INCONSISTENCIAS_ENCONTRADAS_CANCER_RM_".$fecha_actual."_".$tiempo_actual_string.".csv";
	...
	$titulos.="codigo detalle inconsistencia,detalle inconsistencia, numero de linea, numero de campo, tipo identificacion, numero identificacion, diagnoxtico c17";
	...
	$ruta_archivo_inconsistencias_CANCER=$rutaTemporal."INCONSISTENCIAS_ENCONTRADAS_CANCER_RT_".$fecha_actual."_".$tiempo_actual_string.".csv";
	if(file_exists($ruta_archivo_inconsistencias_CANCER) )
	{
		unlink($ruta_archivo_inconsistencias_CANCER);
	}
	$file_inconsistencias_r4725_CANCER = fopen($ruta_archivo_inconsistencias_CANCER, "w") or die("fallo la creacion del archivo");
	$titulos="";
	$titulos.="consecutivo,nombre archivo,codigo tipo inconsistencia,tipo inconsistencia,codigo grupo inconsistencia,grupo inconsistencia,";
	$titulos.="codigo detalle inconsistencia,detalle inconsistencia, numero de linea, numero de campo, tipo identificacion, numero identificacion, diagnoxtico c17";
	//PARTE REESCRIBE TITULOS INCONSISTENCIAS PARA CSV SIMPLE DE ABRIR
    ...
    $error_titulos_procesado_csv_excel.="=\"".implode("\";=\"", $columnas_titulos_inconsistencias_para_bd)."\"";
    //FIN PARTE REESCRIBE TITULOS INCONSISTENCIAS PARA CSV SIMPLE DE ABRIR
	fwrite($file_inconsistencias_r4725_CANCER,$error_titulos_procesado_csv_excel."\n");
	//fin abre o crea el archivo dodne se escribiran las inconsistencias
	...
	//los campos 16,128,130 pueden estar en blanco
	...
	else
	{
		if($nombre_archivo_file!="")
		{
			...
			{
				$es_valido_nombre_archivo=false;
				$errores.="El encabezado del archivo $nombre_archivo_file no corresponde a un archivo CANCER. <br>";
			}
			else
			{
	...
	//VERIFICA SI EL ARCHIVO YA ESTA SIENDO VALIDADO ACTUALMENTE
        ...
        $query_verificacion_esta_siendo_procesado.=" SELECT * FROM gioss_0247_esta_validando_actualmente ";
	...
	if($verificacion_ya_se_valido_con_exito==false && $bool_esta_siendo_validado==false)
	{
		//en la parte de arriba esta el original, se pone aqui para poner el nomrbe del archivo
		$mensaje_advertencia_tiempo="";
		...
		$sql_query_delete="";
		$sql_query_delete.=" DELETE FROM gioss_tabla_registros_no_cargados_rechazados_r0247_cancer ";
		...
		$numero_secuencia_actual=$utilidades->obtenerSecuencia("gioss_numero_secuencia_r0247_cancer");
					
		$sql_query_inserta_seq="";
		$sql_query_inserta_seq.=" INSERT INTO gioss_numero_de_secuencia_r0247_cancer ";
		$sql_query_inserta_seq.=" ( ";
		...
		$query_insert_esta_siendo_procesado.=" INSERT INTO gioss_0247_esta_validando_actualmente ";
		$query_insert_esta_siendo_procesado.=" ( ";
		...
		if($es_valido_nombre_archivo)
		{
			...
			$lineas_del_archivo = count(file($ruta_archivo_cancer)); 
			...
			while (!feof($file_CANCER)) 
			{
				...
				$verificar_si_ejecucion_fue_cancelada.=" SELECT esta_ejecutando FROM gioss_0247_esta_validando_actualmente ";
				...
				if(count($campos)==210)
				{
					//PARTE SE CONSULTA SI ES CANCER PARA MENSAJE, ARCHIVO EXCLUIDOS 
					...
					$resultado=$coneccionBD->consultar2_no_crea_cierra($consulta_campo17);
					//FIN PARTE SE CONSULTA SI ES CANCER PARA MENSAJE, ARCHIVO EXCLUIDOS
					...
					$campo_n5_ti=trim($campos[4]);//TIPO IDENTIFICACION
					$campo_n6_ni=trim($campos[5]);//NUMERO IDENTIFICACION	
					...
					if($muestra_mensaje_nuevo)
					{
						...
						$mensaje_contador_errores="revisando linea ".($nlinea+1)." de $lineas_del_archivo del archivo CANCER. <br> registros buenos $registros_buenos , registros malos $registros_malos .<br>Porcentaje actual $porcentaje % ";
						//PARTE MENSAJE SI CAMPO 17 NO EXISTE EN BD
						if( count($resultado)==0 && !is_array($resultado) )
						{
							$numero_registros_no_existe_campo_17++;
							$mensaje_contador_errores.="<br>Numero de registros del archivo que el codigo en el campo 17 no existe en la base de datos: ".$numero_registros_no_existe_campo_17;
						}//fin if
						else if($numero_registros_no_existe_campo_17>0)
						{
							$mensaje_contador_errores.="<br>";
							$mensaje_contador_errores.="<br>Numero de registros del archivo que el codigo en el campo 17 no existe en la base de datos: ".$numero_registros_no_existe_campo_17;
						}//fin else 
						//PARTE MENSAJE SI CAMPO 17 NO EXISTE EN BD

						//PARTE AFILIADO NO EXISTE
						if(
							$num_filas_resultado_existe_tablas_regimen==0 
							|| !is_array($resultados_query_existe_afiliado_tablas_regimen)
						)//fin parentesis condicion
						{
							$mensaje_contador_errores.="<br>";
							$mensaje_contador_errores.="<br>Numero de registros donde el afiliado no existe en la base de datos: ".$numero_registros_afiliado_no_existe;
							if($numero_registros_a_mostrar_max_afiliado_no_existe>0)
							{
								$mensaje_contador_errores.="<br>";
								$mensaje_contador_errores.="<br>Se muestra primeros 10 afiliados inexistentes(revise el log al finalizar para ver todos):<br>";
								$mensaje_contador_errores.=$cadena_primeros_afiliados_inexistentes;
							}//fin if hay afiliados para mostrar maximo
						}//fin if
						else if($numero_registros_afiliado_no_existe>0)
						{
							$mensaje_contador_errores.="<br>";
							$mensaje_contador_errores.="<br>Numero de registros donde el afiliado no existe en la base de datos: ".$numero_registros_afiliado_no_existe;
							if($numero_registros_a_mostrar_max_afiliado_no_existe>0)
						...
						//FIN PARTE AFILIADO NO EXISTE

						//PARTE AFILIADO SI EXISTE
						if(
							$num_filas_resultado_existe_tablas_regimen>0 
							&& is_array($resultados_query_existe_afiliado_tablas_regimen)
						)//fin parentesis condicion
						{
							$mensaje_contador_errores.="<br>";
							$mensaje_contador_errores.="<br>Numero de registros donde el afiliado si existe en la base de datos: ".$numero_registros_afiliado_si_existen;
							if($numero_registros_a_mostrar_max_afiliado_si_existen>0)
							{
								$mensaje_contador_errores.="<br>";
								$mensaje_contador_errores.="<br>Se muestra primeros 10 afiliados existentes(solo se muestran 10 primeros encontrados):<br>";
								$mensaje_contador_errores.=$cadena_primeros_afiliados_existentes;
							}//fin if hay afiliados para mostrar maximo
						}//fin if
						else if($numero_registros_afiliado_si_existen>0)
						{
							$mensaje_contador_errores.="<br>";
							$mensaje_contador_errores.="<br>Numero de registros donde el afiliado si existe en la base de datos: ".$numero_registros_afiliado_si_existen;
							if($numero_registros_a_mostrar_max_afiliado_si_existen>0)
						...
						*/
						...
						$query_update_esta_siendo_procesado.=" UPDATE gioss_0247_esta_validando_actualmente ";
					...
					}//fin if muestra mensaje

					//PARTE VALIDACION CANCER AFILIADO ESTA EN BD
					...
					$tipo_de_regimen_de_la_informacion_reportada=trim($campos[9]);
			    	...
			    	else if	($si_existe==false )
			    	{
						...
						//se pone true debido a que es obligatoria, ademas impide que prosiga con la validacion 
						//de este registro para agilizar el proceso, esto aplica para las versiones prepagada
					...
					//FIN PARTE VALIDACION CANCEr AFILIADO ESTA EN BD
					
					$linea_mod_depende_afiliados=implode("\t", $campos);
				    					
					//parte separa campo 17 que no coincide
			    	$existe_codigo_campo_17=true;
					if( count($resultado)==0 && !is_array($resultado) )
					{
						//echo "<script>alert(' entro a separar registro ');</script>";
						fwrite($file_archivo_separados_diagnostico_no_corresponde, "\n".trim($linea_mod_depende_afiliados));

						$existe_codigo_campo_17=false;
						
						$errores_campo_17_codigo_no_existe="";

						//INCONSISTENCIAS A MOSTRAR c17
						//campo obligatorio
						//echo "<br>Codigo CIEX campo 17: ".trim($campos[16])."<br>";
						if(trim($campos[16])=="")
						{
							//echo "<br>esta en blanco <br>";
							if($errores_campo_17_codigo_no_existe!="")
							{
								$errores_campo_17_codigo_no_existe.="|";
							}		
							...
							$errores_campo_17_codigo_no_existe.=$consecutivo_errores.",".$nombre_archivo_registrado.",01,".$array_tipo_validacion["01"].",0104,".$array_grupo_validacion["0104"].",$var_numero_codigo,$cadena_descripcion_inconsistencia ...VR:".$campos[16]." ,".($nlinea+1).",17";//campo 16 es el numeros orden para el campo 17
							$consecutivo_errores++;
						...
						if(strlen($campos[16])!=4)
						{
							...
							$cadena_descripcion_inconsistencia=explode(";;",$array_detalle_validacion[$var_numero_codigo])[1];
							$errores_campo_17_codigo_no_existe.=$consecutivo_errores.",".$nombre_archivo_registrado.",01,".$array_tipo_validacion["01"].",0101,".$array_grupo_validacion["0101"].",$var_numero_codigo,$cadena_descripcion_inconsistencia ...VR:".$campos[16]." ,".($nlinea+1).",17";//campo 16 es el numeros orden para el campo 17
							$consecutivo_errores++;
						...
						if(!ctype_alnum($campos[16]))
						{
							...
							$cadena_descripcion_inconsistencia=explode(";;",$array_detalle_validacion[$var_numero_codigo])[1];
							$errores_campo_17_codigo_no_existe.=$consecutivo_errores.",".$nombre_archivo_registrado.",01,".$array_tipo_validacion["01"].",0102,".$array_grupo_validacion["0102"].",$var_numero_codigo,$cadena_descripcion_inconsistencia ...VR:".$campos[16]." ,".($nlinea+1).",17";//campo 16 es el numeros orden para el campo 17
							$consecutivo_errores++;
							
							//$hubo_errores=true;
						}//fin if
						
						$tiene_letra_principio=substr($campos[16],0,1);
						if(ctype_lower($tiene_letra_principio))
						{
							if($errores_campo_17_codigo_no_existe!="")
							{
								$errores_campo_17_codigo_no_existe.="|";
						...
						//FIN INCONSISTENCIAS A MOSTRAR C17

						//inconsistencia no existe codigo campo 17 este entra por defecto ya que no se encontro en bd
						if($errores_campo_17_codigo_no_existe!="")
						{
							$errores_campo_17_codigo_no_existe.="|";
						}
						...
						$cadena_descripcion_inconsistencia=explode(";;",$array_detalle_validacion[$var_numero_codigo])[1];
						$errores_campo_17_codigo_no_existe.=$consecutivo_errores.",".$nombre_archivo_registrado.",01,".$array_tipo_validacion["01"].",0103,".$array_grupo_validacion["0103"].",$var_numero_codigo,$cadena_descripcion_inconsistencia ...VR:".$campos[16]." ,".($nlinea+1).",17";//campo 16 es el numeros orden para el campo 17
						$consecutivo_errores++;
						//fin inconsistencia no existe codigo campo 17

						$array_resultados_validacion=array("error"=>true,"mensaje"=>$errores_campo_17_codigo_no_existe);

					}//fin if
					else
					{							
						//no hace nada ya que aca es cuando si existe y el valor por defecto ya es true
					}
					...
					}//fin if si existe como afiliado en la base de datos
										
					if($hubo_inconsistencias_en_CANCER==false)
					{
						$hubo_inconsistencias_en_CANCER=$array_resultados_validacion["error"];
					}
					...
					if($array_resultados_validacion["error"]==false)
					{
						...
						if($se_creo_tabla_indice==false)
						{
							...
							$sql_insertar_en_tabla_indice_analisis_coherencia.="insert into gioss_indice_archivo_para_analisis_0247";
							$sql_insertar_en_tabla_indice_analisis_coherencia.="(";
						...
						if($se_creo_tabla_indice==true)
						{
							...
							$sql_insertar_en_tabla_analisis_coherencia.="insert into gioss_archivo_para_analisis_0247";
							$sql_insertar_en_tabla_analisis_coherencia.="(";
							...
							$cont_campo_ins=1;
							while($cont_campo_ins<=210)
							{
								
								$sql_insertar_en_tabla_analisis_coherencia.="campo_cancer_de_numero_orden_".$cont_campo_ins.",";
					...
					$query_insertar_campos_a_bd.="INSERT INTO gioss_tabla_registros_no_cargados_rechazados_r0247_cancer ";
					$query_insertar_campos_a_bd.="(";				
					$cont_campo_ins=1;
					while($cont_campo_ins<=210)
					{
						$query_insertar_campos_a_bd.="campo_cancer_de_numero_orden_".$cont_campo_ins.",";
						$cont_campo_ins++;
					}
					...
					while($cont_campo_ins<=210)
					{
						$query_insertar_campos_a_bd.="'".procesar_mensaje($campos[$cont_campo_ins-1])."',";
						$cont_campo_ins++;
					}
					...
					//escribe los errores
					$mensaje_errores_CANCER=$array_resultados_validacion["mensaje"];
					$array_mensajes_errores_campos=explode("|",$mensaje_errores_CANCER);
					
					foreach($array_mensajes_errores_campos as $msg_error)
					{
						//PARA CAMBIAR LAS COMAS POR OTRO SEPARADOR DE CAMPOS SIN ALTERAR EL CODIGO PRINCIPAL
						/*
						//se comenta para evitar enredos por el momento,
						//por lo tanto se conserva la coma (,) como separador de columnas
						$cantidad_a_modificar_desde_el_inicio=7;
						$cantidad_a_modificar_desde_el_final=2;
						$mensaje_error_actual_a_modificar_separador="";
						$mensaje_error_actual_a_modificar_separador=custom_replace_str(",","||",$msg_error,false,$cantidad_a_modificar_desde_el_inicio,$cantidad_a_modificar_desde_el_final);
						//echo $msg_error."<br>".$mensaje_error_actual_a_modificar_separador."<br>";
						*/
						//FIN PARA CAMBIAR LAS COMAS POR OTRO SEPARADOR DE CAMPOS SIN ALTERAR EL CODIGO PRINCIPAL
						...
						if($msg_error!="")
						{
							...
							$columnas_inconsistencias_para_bd[]=$campo_n17;
							...
							fwrite($file_inconsistencias_r4725_CANCER, $error_procesado_csv_excel."\n");
						...
						if(count($columnas_inconsistencias_para_bd)==10)
						{
							...
							$sql_insertar_inconsistencia_cancer="";
							$sql_insertar_inconsistencia_cancer.=" INSERT INTO gioss_reporte_inconsistencias_r0247_cancer ";
							$sql_insertar_inconsistencia_cancer.=" ( ";
							$sql_insertar_inconsistencia_cancer.=" numero_orden, ";
							$sql_insertar_inconsistencia_cancer.=" nombre_archivo, ";
							$sql_insertar_inconsistencia_cancer.=" cod_tipo_inconsitencia, ";
							$sql_insertar_inconsistencia_cancer.=" nombre_tipo_inconsistencia, ";
							$sql_insertar_inconsistencia_cancer.=" cod_grupo_inconsistencia, ";
							$sql_insertar_inconsistencia_cancer.=" nombre_grupo_inconsistencia, ";
							$sql_insertar_inconsistencia_cancer.=" cod_detalle_inconsistencia, ";
							$sql_insertar_inconsistencia_cancer.=" detalle_inconsistencia, ";
							$sql_insertar_inconsistencia_cancer.=" numero_linea, ";
							$sql_insertar_inconsistencia_cancer.=" numero_campo, ";
							$sql_insertar_inconsistencia_cancer.=" fecha_validacion, ";
							$sql_insertar_inconsistencia_cancer.=" hora_validacion ";
							$sql_insertar_inconsistencia_cancer.=" ) ";
							$sql_insertar_inconsistencia_cancer.=" VALUES ";
							$sql_insertar_inconsistencia_cancer.=" ( ";
							$sql_insertar_inconsistencia_cancer.=" '".$numero_secuencia_actual."', ";
							$sql_insertar_inconsistencia_cancer.=" '".$nombre_archivo_registrado."', ";
							$sql_insertar_inconsistencia_cancer.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[2]) )."', ";
							$sql_insertar_inconsistencia_cancer.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[3]) )."', ";
							$sql_insertar_inconsistencia_cancer.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[4]) )."', ";
							$sql_insertar_inconsistencia_cancer.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[5]) )."', ";
							$sql_insertar_inconsistencia_cancer.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[6]) )."', ";
							$sql_insertar_inconsistencia_cancer.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[7]) )."', ";
							$sql_insertar_inconsistencia_cancer.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[8]) )."', ";
							$sql_insertar_inconsistencia_cancer.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[9]) )."', ";
							$sql_insertar_inconsistencia_cancer.=" '".$fecha_actual."', ";
							$sql_insertar_inconsistencia_cancer.=" '".$tiempo_actual."' ";
							$sql_insertar_inconsistencia_cancer.=" ); ";
							$error_bd_ins="";
							$bool_hubo_error_query=$coneccionBD->insertar_no_warning_get_error_no_crea_cierra($sql_insertar_inconsistencia_cancer, $error_bd_ins);
							if($error_bd_ins!="")
							{
				    ...
				    if(count($campos)>=210)
				    {
					    try
					    {
							...
							FROM gioss_index_reporte_ips_0247 
							WHERE 
							...
							$sql_update_en_reporte_ips.="UPDATE gioss_index_reporte_ips_0247 SET ";
							...
							if($filas_afectadas==0)
							{
								...
								$sql_insertar_en_reporte_ips.="insert into gioss_index_reporte_ips_0247";
								$sql_insertar_en_reporte_ips.="(";
				...
				else
				{
					...
					$mensaje_contador_errores="revisando linea ".($nlinea+1)." de $lineas_del_archivo del archivo CANCER. <br> registros buenos $registros_buenos , registros malos $registros_malos .<br> ";
					$html_del_mensaje="";
					$html_del_mensaje.="<table>";
					$html_del_mensaje.="<tr>";
					$html_del_mensaje.="<td colspan=\'2\'>";
					...
					$error_longitud=$consecutivo_errores.",".$nombre_archivo_registrado.",01,".$array_tipo_validacion["01"].",0104,".$array_grupo_validacion["0104"].",0104099,$cadena_descripcion_inconsistencia ".count($campos).",".($nlinea+1).","."-1";
					$consecutivo_errores++;
					
					if($hubo_inconsistencias_en_CANCER==false)
					{
						$hubo_inconsistencias_en_CANCER=true;
					}
		...
		fclose($file_inconsistencias_r4725_CANCER);
		fclose($file_archivo_separados_diagnostico_no_corresponde);
		...
		if($es_valido_nombre_archivo)
		{
			...
			if($hubo_inconsistencias_en_CANCER==false)
			{
				...
				$sql_query_delete="";
				$sql_query_delete.=" DELETE FROM gioss_tabla_registros_cargados_exito_r0247_cancer ";
				$sql_query_delete.=" WHERE fecha_corte='".$fecha_de_corte."' ";
				$sql_query_delete.=" AND codigo_eapb_a_reportar='".$cod_eapb."' ";
				$sql_query_delete.=" AND codigo_prestador_reportante='".$cod_prestador."' ";
				$sql_query_delete.=" AND nombre_archivo='".$nombre_archivo_registrado."' ; ";
				...
				while (!feof($file_CANCER)) 
				{
					...
					{
						$query_insertar_campos_a_bd="";
						$query_insertar_campos_a_bd.="INSERT INTO gioss_tabla_registros_cargados_exito_r0247_cancer ";
						$query_insertar_campos_a_bd.="(";
						...
						$mensaje_contador_errores="Subiendo a cargados con exito el registro ".($nlinea+1)." de $lineas_del_archivo del archivo CANCER ";
						$html_del_mensaje="";
						$html_del_mensaje.="<table>";
						$html_del_mensaje.="<tr>";
						$html_del_mensaje.="<td colspan=\'2\'>";
				...
				$query_registrar_resumen_cargado_con_exito.="INSERT INTO gioss_resumen_cargados_exito_0247 ";
				$query_registrar_resumen_cargado_con_exito.="(";
			...
			//SUBIR A TABLA CONSOLIDADOS PARA CANCER
			$estado_validacion_cancer=2;
			if($hubo_inconsistencias_en_CANCER==false)
			{
				$estado_validacion_cancer=1;
			}
			...
			$query_delete_estado_validacion.="DELETE FROM gioss_tabla_consolidacion_registros_validados_r0247_cancer ";
			...
			$query_registrar_estado_validacion.="INSERT INTO gioss_tabla_consolidacion_registros_validados_r0247_cancer ";
			$query_registrar_estado_validacion.="(";
			...
			$query_registrar_estado_validacion.="(";
			$query_registrar_estado_validacion.="'".$estado_validacion_cancer."',";
			...
			//FIN SUBIR A TABLA CONSOLIDADOS PARA CANCER
			
			
			//SUBIR A TABLA DE ESTADO INFORMACION PARA CANCER
			$estado_validacion_cancer=2;
			if($hubo_inconsistencias_en_CANCER==false)
			{
				$estado_validacion_cancer=1;
			...
			$query_descripcion_estado_validacion.=" SELECT * FROM gioss_estado_validacion_archivos WHERE codigo_estado_validacion='$estado_validacion_cancer' ; ";
			...
			$query_delete_estado_informacion.=" DELETE FROM gioss_tabla_estado_informacion_r0247_cancer ";
			...
			$query_registrar_estado_informacion.="INSERT INTO gioss_tabla_estado_informacion_r0247_cancer ";
			$query_registrar_estado_informacion.="(";
			...
			$query_registrar_estado_informacion.="(";
			$query_registrar_estado_informacion.="'".$estado_validacion_cancer."',";//1
			...
			//FIN SUBIR A TABLA DE ESTADO INFORMACION PARA CANCER
			
			
			
			if(connection_aborted()==false)
			{
				echo "<script>document.getElementById('mensaje').innerHTML='Se ha terminado de revisar y validar el archivo CANCER <b>$nombre_archivo_registrado</b> <br> Registros buenos $registros_buenos , Registros malos $registros_malos . ';</script>";
				ob_flush();
				flush();
			}
		...
		if($numero_registros_para_reporte_por_ips>0)
		{
			...
			$parte_inicial_reporte_registros_por_ips.="RESOLUCION 0247\n";
		...
		$parte_inicial_reporte_evaluacion_resultado_validacion.="Resolucion 0247\n";
		...
		$archivos_a_comprimir[0]=$ruta_archivo_inconsistencias_CANCER;
		$archivos_a_comprimir[1]=$ruta_archivo_separados_diagnostico_no_corresponde;
		$archivos_a_comprimir[2]=$ruta_registros_malos;
		$archivos_a_comprimir[3]=$ruta_registros_buenos;
		$archivos_a_comprimir[4]=$ruta_inconsistencias_registros_malos;
		$archivos_a_comprimir[]=$ruta_registros_que_tienen_afiliados;
		$archivos_a_comprimir[]=$ruta_registros_que_no_tienen_afiliados;
		//$archivos_a_comprimir[]=$ruta_log_registros_modificados; //NO SE USA FUE REEMPLAZADO POR CAMBIO_SEXO Y CAMBIO_FECHA_NACIMIENTO
		...
		$ruta_zip=$rutaTemporal."inconsistencias0247CANCER_".$cod_prestador."_".$fecha_actual."_".$tiempo_actual_string.'.zip';
		...
		//BOTONES DESCARGA
		$botones="";
		$botones.=" <input type=\'button\' value=\'Descargar archivo de inconsistencias para CANCER\'  class=\'btn btn-success color_boton\' onclick=\"download_inconsistencias_campos(\'$ruta_zip\');\"/> ";
		...
		try
		{
			...
			{	
				...
				$mail->FromName = "GIOSS";
				$mail->Subject = "Inconsistencias CANCER 0247 ";
				$mail->AltBody = "Cordial saludo,\n El sistema ha determinado que su archivo(s) para la EAPB $cod_eapb contiene diversas inconsistencias,\n las cuales pueden ser: campos con información inconsistente, usuarios duplicados ó el uso de caracteres especiales(acentos,'Ñ' o É,Ý, ¥, ¤, ´)";
		    
				$mail->MsgHTML("Cordial saludo,\n El sistema ha determinado que su archivo(s) para la EAPB $cod_eapb  contiene diversos errores, con el numero de secuencia: $numero_secuencia_actual.<strong>GIOSS</strong>.");
	...
	else if($verificacion_ya_se_valido_con_exito)
	{
		if(connection_aborted()==false)
		{
			echo "<script>document.getElementById('div_mensaje_error').style.display='inline';</script>";
			echo "<script>document.getElementById('parrafo_mensaje_error').innerHTML='<br>El archivo 0247 ya fue validado con exito';</script>";
			ob_flush();
			flush();
		}
	}
	else if($bool_esta_siendo_validado)
	{
		if(connection_aborted()==false)
		{
			echo "<script>document.getElementById('div_mensaje_error').style.display='inline';</script>";
			echo "<script>document.getElementById('parrafo_mensaje_error').innerHTML='<br>El archivo 0247 esta siendo validado actualmente.';</script>";
			ob_flush();
			flush();
		}
	}
