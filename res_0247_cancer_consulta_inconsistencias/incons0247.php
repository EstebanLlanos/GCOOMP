...
$smarty->display('incons0247.html.tpl');
...
if(isset($_POST["tipo_busqueda"]))
{
	...
	if($_POST["tipo_busqueda"]=="1" && isset($_POST["numero_secuencia"]) )
	{
		...
		$query_estado_validacion.="SELECT * FROM gioss_tabla_consolidacion_registros_validados_r0247_cancer WHERE numero_secuencia='$numero_secuencia_para_bd' AND codigo_eapb='$codigo_eapb' AND codigo_entidad_reportadora='$codigo_entidad_reportadora'; ";
		...
		{
			$existe_validacion=false;
			//echo "<script>document.getElementById('mensaje_div').innerHTML='No se hallaron archivos 0247 con dicho numero de secuencia.';</script>";
	...
	if($_POST["tipo_busqueda"]=="2" && isset($_POST["nombre_archivo"]) && isset($_POST["numeros_secuencias_varios"])  && strlen($_POST["numeros_secuencias_varios"])>0 )
	{
		...
		{
			$existe_validacion=false;
			//echo "<script>document.getElementById('mensaje_div').innerHTML='No se hallaron archivos 0247 con dicho nombre de archivo.';</script>";
			echo "<script>document.getElementById('div_mensaje_error').style.display='inline';</script>";
			echo "<script>document.getElementById('parrafo_mensaje_error').innerHTML='No se puede efectuar la consulta, no existen datos asociados al parametro de busqueda digitado.';</script>";
		}
	}
	if($_POST["tipo_busqueda"]=="3" && isset($_POST["nombres_archivos_varios"])  && strlen($_POST["nombres_archivos_varios"])>0)
	{
		//ahora obtiene el numero de secuencia si hay un archivo validado varias veces en el mismo periodo de tiempo
		$numero_secuencia_para_bd=$_POST["nombres_archivos_varios"];
	...
	if($existe_validacion==true)
	{
		if($_POST["tipo_busqueda"]=="1" && isset($_POST["numero_secuencia"]) )
		{
			...
			$sql_numero_registros.="SELECT count(*) as contador FROM gioss_reporte_inconsistencias_r0247_cancer WHERE numero_orden='$numero_secuencia_para_bd';  ";
			...
			$sql_vista_inconsistencias.="CREATE OR REPLACE VIEW vista_inconsistencias_0247_".$nick_user."_".$tipo_id."_".$identificacion." ";
			$sql_vista_inconsistencias.=" AS SELECT * FROM gioss_reporte_inconsistencias_r0247_cancer WHERE numero_orden='$numero_secuencia_para_bd' order by numero_linea, numero_campo ; ";
			$error_bd_seq="";
			...
			while($contador_offset<$numero_registros)
			{
				...
				//Ejemplo: SELECT *  FROM gioss_reporte_inconsistencias_r0247_cancer WHERE numero_orden='68'  order by numero_linea, numero_campo limit 5 offset 0; 
				$sql_query_busqueda="";
				$sql_query_busqueda.="SELECT * FROM vista_inconsistencias_0247_".$nick_user."_".$tipo_id."_".$identificacion." LIMIT $limite OFFSET $contador_offset;  ";
				$resultado_query_inconsistencias=$coneccionBD->consultar2($sql_query_busqueda);
			
				if(count($resultado_query_inconsistencias)>0)
				{
					$nombre_archivo=$resultado_query_inconsistencias[0]["nombre_archivo"];
					...
					foreach($resultado_query_inconsistencias as $resultado)
					{
						...
						$linea_inconsistencia.=$cont_linea.",".$resultado["nombre_archivo"].",".$resultado["cod_tipo_inconsitencia"].",";
			...
			if($hubo_resultados)
			{
				...
				$boton_descarga=" <input type=\'button\' value=\'Descargar archivo con las inconsistencias consultadas para 0247\'  class=\'btn btn-success color_boton\' onclick=\"download_inconsistencias_campos(\'$ruta_zip\');\"/> ";
