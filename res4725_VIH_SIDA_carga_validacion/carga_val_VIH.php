...
$smarty->display('carga_val_VIH.html.tpl');
...
if(isset($_POST["accion"]) && $_POST["accion"]=="validar" && isset($_FILES["r4725_file"]) && $_FILES["r4725_file"]["error"]==0)
{
	...
	$nombre_archivo_file=explode(".",$_FILES["r4725_file"]["name"])[0];
    ...
    //SELECTOR VERSION
    $nombre_base_version="validadorVIH_v";
	...
	$ruta_inconsistencias_registros_malos=$rutaTemporal."inconsVIH_RM_".$fecha_actual."_".$tiempo_actual_string.".csv";
	...
	$ruta_archivo_inconsistencias_vih=$rutaTemporal."inconsVIH_RT_".$fecha_actual."_".$tiempo_actual_string.".csv";
    ...
    $error_titulos_procesado_csv_excel.="=\"".implode("\";=\"", $columnas_titulos_inconsistencias_para_bd)."\"";
    //FIN PARTE REESCRIBE TITULOS INCONSISTENCIAS PARA CSV SIMPLE DE ABRIR
	fwrite($file_inconsistencias_r4725_vih_sida, $error_titulos_procesado_csv_excel."\n");
	...
	else
	{
		if($nombre_archivo_file!="")
		{
			...
			{
				$es_valido_nombre_archivo=false;
				$errores.="El encabezado del archivo $nombre_archivo_file no corresponde a un archivo VIH-SIDA. <br>";
			}
			else
			{
				...
				*/
				...
				/*
				para agrupado remover de las tablas gioss_tabla_estado_informacion_r4725_sida y gioss_numero_de_secuencia_r4725_sida las condiciones de llave foranea para prestador
				ALTER TABLE gioss_numero_de_secuencia_r4725_sida DROP CONSTRAINT gioss_numero_de_secuencia_r47_codigo_prestador_servicios_s_fkey;
				ALTER TABLE gioss_tabla_estado_informacion_r4725_sida DROP CONSTRAINT gioss_tabla_estado_informacion_codigo_prestador_servicios_fkey1;
				ALTER TABLE gioss_tabla_registros_cargados_exito_r4725_sida DROP CONSTRAINT gioss_tabla_registros_cargado_codigo_prestador_reportante_fkey1;
				ALTER TABLE gioss_tabla_registros_no_cargados_rechazados_r4725_sida DROP CONSTRAINT gioss_tabla_registros_no_carg_codigo_prestador_reportante_fkey1;
				ALTER TABLE gioss_tabla_consolidacion_registros_validados_r4725_sida DROP CONSTRAINT gioss_tabla_consolidacion_regi_codigo_entidad_reportadora_fkey1;
				*/
	...
	//VERIFICA SI EL ARCHIVO YA ESTA SIENDO VALIDADO ACTUALMENTE
        ...
        $query_verificacion_esta_siendo_procesado.=" SELECT * FROM gioss_4725_esta_validando_actualmente ";
	...
	{
		//elimina rechazados porque va a ser validado como exitoso
		$sql_query_delete="";
		$sql_query_delete.=" DELETE FROM gioss_tabla_registros_no_cargados_rechazados_r4725_sida ";
		...
		//fin elimina rechazados porque va a ser validado como exitoso
	
		//ASIGNACION NUMERO SECUENCIA
		//consulta ultimo numero de secuencia
		$numero_secuencia_actual=$utilidades->obtenerSecuencia("gioss_numero_secuencia_r4725_sida");
					
		$sql_query_inserta_seq="";
		$sql_query_inserta_seq.=" INSERT INTO gioss_numero_de_secuencia_r4725_sida ";
		$sql_query_inserta_seq.=" ( ";
		...
		$query_insert_esta_siendo_procesado.=" INSERT INTO gioss_4725_esta_validando_actualmente ";
		$query_insert_esta_siendo_procesado.=" ( ";
		...
		// parte donde valida los campos del archivo vih sida
		...
		{
			$mensaje_errores_vih="";
			$lineas_del_archivo = count(file($ruta_archivo_sida)); 
			...
			while (!feof($file_vih)) 
			{
				...
				$verificar_si_ejecucion_fue_cancelada.=" SELECT esta_ejecutando FROM gioss_4725_esta_validando_actualmente ";
				...
				if(count($campos)==185)
				{
					...
					if($muestra_mensaje_nuevo)
					{
						...
						$mensaje_contador_errores="revisando linea ".($nlinea+1)." de $lineas_del_archivo del archivo VIH. <br> registros buenos $registros_buenos , registros malos $registros_malos .<br>Porcentaje actual $porcentaje % ";
					$html_del_mensaje="";
					...
					*/
					...
					$query_update_esta_siendo_procesado.=" UPDATE gioss_4725_esta_validando_actualmente ";
					...
					}//fin muestra mensaje y actualiza en BD
					...
					$campo_n8_ti=trim($campos[8]);//TIPO IDENTIFICACION
					$campo_n9_ni=trim($campos[9]);//NUMERO IDENTIFICACION	
					...
					$campo_n4_primer_nombre=trim($campos[4]);
					$campo_n5_segundo_nombre=trim($campos[5]);
					$campo_n6_primer_apellido=trim($campos[6]);
					$campo_n7_segundo_apellido=trim($campos[7]);
					$campo_n8_ti=trim($campos[8]);
					$campo_n9_ni=trim($campos[9]);
					$campo_n10_fecha_nacimiento=trim($campos[10]);
					$campo_n11_sexo=trim($campos[11]);


					$tipo_de_regimen_de_la_informacion_reportada=trim($campos[2]);
					...
					if($si_existe==true)
					{
					$array_resultados_validacion=validar_vih($campos,
										 $nlinea,
										 $consecutivo_errores,
					...
					if($array_resultados_validacion["error"]==false)
					{
						...
						if($se_creo_tabla_indice==false)
						{
							...
							$sql_insertar_en_tabla_indice_analisis_coherencia.="insert into gioss_indice_archivo_para_analisis_4725";
							$sql_insertar_en_tabla_indice_analisis_coherencia.="(";
						...
						if($se_creo_tabla_indice==true)
						{
							...
							$sql_insertar_en_tabla_analisis_coherencia.="insert into gioss_archivo_para_analisis_4725";
							$sql_insertar_en_tabla_analisis_coherencia.="(";
							...
							$cont_campo_ins=0;
							while($cont_campo_ins<=184)
							{
								
								$sql_insertar_en_tabla_analisis_coherencia.="campo_sida_de_numero_orden_".$cont_campo_ins.",";
							...
							while($cont_campo_ins<=184)
							{
								$sql_insertar_en_tabla_analisis_coherencia.="'".procesar_mensaje($campos[$cont_campo_ins])."',";
								$cont_campo_ins++;
							}
							if($bool_fecha_nacimiento_valida==true)
							{
					...
					$query_insertar_campos_a_bd.="INSERT INTO gioss_tabla_registros_no_cargados_rechazados_r4725_sida ";
					$query_insertar_campos_a_bd.="(";
					...
					$cont_campo_ins=0;
					while($cont_campo_ins<185)
					{
						if($cont_campo_ins!=184)
						{
							$query_insertar_campos_a_bd.="campo_sida_de_numero_orden_".$cont_campo_ins.",";
						}
						else
						{
							$query_insertar_campos_a_bd.="campo_sida_de_numero_orden_".$cont_campo_ins." ";
					...
					foreach($array_mensajes_errores_campos as $msg_error)
					{
						...
						if($msg_error!="")
						{
							...
							fwrite($file_inconsistencias_r4725_vih_sida, $error_procesado_csv_excel."\n");
						...
						if(count($columnas_inconsistencias_para_bd)==10)
						{
							...
							$sql_insertar_inconsistencia_sida="";
							$sql_insertar_inconsistencia_sida.=" INSERT INTO gioss_reporte_inconsistencias_r4725_sida ";
							$sql_insertar_inconsistencia_sida.=" ( ";
							$sql_insertar_inconsistencia_sida.=" numero_orden, ";
							$sql_insertar_inconsistencia_sida.=" nombre_archivo, ";
							$sql_insertar_inconsistencia_sida.=" cod_tipo_inconsitencia, ";
							$sql_insertar_inconsistencia_sida.=" nombre_tipo_inconsistencia, ";
							$sql_insertar_inconsistencia_sida.=" cod_grupo_inconsistencia, ";
							$sql_insertar_inconsistencia_sida.=" nombre_grupo_inconsistencia, ";
							$sql_insertar_inconsistencia_sida.=" cod_detalle_inconsistencia, ";
							$sql_insertar_inconsistencia_sida.=" detalle_inconsistencia, ";
							$sql_insertar_inconsistencia_sida.=" numero_linea, ";
							$sql_insertar_inconsistencia_sida.=" numero_campo, ";
							$sql_insertar_inconsistencia_sida.=" fecha_validacion, ";
							$sql_insertar_inconsistencia_sida.=" hora_validacion ";
							$sql_insertar_inconsistencia_sida.=" ) ";
							$sql_insertar_inconsistencia_sida.=" VALUES ";
							$sql_insertar_inconsistencia_sida.=" ( ";
							$sql_insertar_inconsistencia_sida.=" '".$numero_secuencia_actual."', ";
							$sql_insertar_inconsistencia_sida.=" '".$nombre_archivo_registrado."', ";
							$sql_insertar_inconsistencia_sida.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[2]))."', ";
							$sql_insertar_inconsistencia_sida.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[3]))."', ";
							$sql_insertar_inconsistencia_sida.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[4]))."', ";
							$sql_insertar_inconsistencia_sida.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[5]))."', ";
							$sql_insertar_inconsistencia_sida.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[6]))."', ";
							$sql_insertar_inconsistencia_sida.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[7]))."', ";
							$sql_insertar_inconsistencia_sida.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[8]))."', ";
							$sql_insertar_inconsistencia_sida.=" '".preg_replace("/[^A-Za-z0-9:.\-\s+]/", "", trim($columnas_inconsistencias_para_bd[9]))."', ";
							$sql_insertar_inconsistencia_sida.=" '".$fecha_actual."', ";
							$sql_insertar_inconsistencia_sida.=" '".$tiempo_actual."' ";
							$sql_insertar_inconsistencia_sida.=" ); ";
							$error_bd_ins="";
							$bool_hubo_error_query=$coneccionBD->insertar_no_warning_get_error_no_crea_cierra($sql_insertar_inconsistencia_sida, $error_bd_ins);
							if($error_bd_ins!="")
							{
								$error_mostrar_bd.="AL SUBIR LAS INCONSISTENCIAS ".$error_bd_ins."<br>";
							}
							//fin se insertan los datos de detalles de inconsistencia 
						}
						//FIN SUBIDA DE INCONSISTENCIAS A LA BASE DE DATOS
				    ...
				    if(count($campos)>=185)
				    {
					    try
					    {
							...
							FROM gioss_index_reporte_ips_4725 
							WHERE 
							...
							$sql_update_en_reporte_ips.="UPDATE gioss_index_reporte_ips_4725 SET ";
							...
							if($filas_afectadas==0)
							{
								...
								$sql_insertar_en_reporte_ips.="insert into gioss_index_reporte_ips_4725";
								$sql_insertar_en_reporte_ips.="(";
				...
				{
					$mensaje_contador_errores="revisando linea ".($nlinea+1)." de $lineas_del_archivo del archivo VIH ";
					$html_del_mensaje="";
					$html_del_mensaje.="<table>";
					$html_del_mensaje.="<tr>";
					$html_del_mensaje.="<td colspan=\'2\'>";
					...
					$error_longitud=$consecutivo_errores.",".$nombre_archivo_registrado.",01,".$array_tipo_validacion["01"].",0104,".$array_grupo_validacion["0104"].",0104099,$cadena_descripcion_inconsistencia ".(count($campos)-1)." ,".($nlinea+1).","."-1";
					$consecutivo_errores++;
		...
		if($es_valido_nombre_archivo)
		{
			...
			if($hubo_inconsistencias_en_vih==false)
			{
				...
				//elimina existosos previos sin completar
				$sql_query_delete="";
				$sql_query_delete.=" DELETE FROM gioss_tabla_registros_cargados_exito_r4725_sida ";
				$sql_query_delete.=" WHERE fecha_corte='".$fecha_de_corte."' ";
				$sql_query_delete.=" AND codigo_eapb_a_reportar='".$cod_eapb."' ";
				$sql_query_delete.=" AND codigo_prestador_reportante='".$cod_prestador."' ";
				$sql_query_delete.=" AND nombre_archivo='".$nombre_archivo_registrado."' ; ";
				...
				while (!feof($file_vih)) 
				{
					...
					{
						$query_insertar_campos_a_bd="";
						$query_insertar_campos_a_bd.="INSERT INTO gioss_tabla_registros_cargados_exito_r4725_sida ";
						$query_insertar_campos_a_bd.="(";
						$query_insertar_campos_a_bd.="nombre_archivo,";
						$query_insertar_campos_a_bd.="numero_secuencia,";
						$query_insertar_campos_a_bd.="codigo_prestador_reportante,";
				...
				$query_registrar_resumen_cargado_con_exito.="INSERT INTO gioss_resumen_cargados_exito_4725 ";
				$query_registrar_resumen_cargado_con_exito.="(";
			...
			//SUBIR A TABLA CONSOLIDADOS PARA VIH
			$estado_validacion_sida=2;
			if($hubo_inconsistencias_en_vih==false)
			{
				$estado_validacion_sida=1;
			}
			...
			$query_delete_estado_validacion.="DELETE FROM gioss_tabla_consolidacion_registros_validados_r4725_sida ";
			...
			$query_registrar_estado_validacion.="INSERT INTO gioss_tabla_consolidacion_registros_validados_r4725_sida ";
			$query_registrar_estado_validacion.="(";
			...
			$query_registrar_estado_validacion.="(";
			$query_registrar_estado_validacion.="'".$estado_validacion_sida."',";
			...
			//FIN SUBIR A TABLA CONSOLIDADOS PARA VIH
			
			//SUBIR A TABLA DE ESTADO INFORMACION PARA VIH
			$estado_validacion_sida=2;
			if($hubo_inconsistencias_en_vih==false)
			{
				$estado_validacion_sida=1;
			...
			$query_descripcion_estado_validacion.=" SELECT * FROM gioss_estado_validacion_archivos WHERE codigo_estado_validacion='$estado_validacion_sida' ; ";
			...
			$query_delete_estado_informacion.=" DELETE FROM gioss_tabla_estado_informacion_r4725_sida ";
			...
			$query_registrar_estado_informacion.="INSERT INTO gioss_tabla_estado_informacion_r4725_sida ";
			$query_registrar_estado_informacion.="(";
			...
			$query_registrar_estado_informacion.="(";
			$query_registrar_estado_informacion.="'".$estado_validacion_sida."',";//1
			...
			//FIN SUBIR A TABLA DE ESTADO INFORMACION PARA VIH
			
			if(connection_aborted()==false)
			{
				echo "<script>document.getElementById('mensaje').innerHTML='Se ha terminado de revisar y validar el archivo VIH. <br> registros buenos $registros_buenos , registros malos $registros_malos .';</script>";
				ob_flush();
				flush();
			}
		...
		if($numero_registros_para_reporte_por_ips>0)
		{
			...
			$parte_inicial_reporte_registros_por_ips.="RESOLUCION 4725\n";
		...
		$parte_inicial_reporte_evaluacion_resultado_validacion.="Resolucion 4725\n";
		...
		$archivos_a_comprimir[0]=$ruta_archivo_inconsistencias_vih;
		$archivos_a_comprimir[1]=$ruta_registros_malos;
		$archivos_a_comprimir[2]=$ruta_registros_buenos;
		$archivos_a_comprimir[3]=$ruta_inconsistencias_registros_malos;
		...
		$ruta_zip=$rutaTemporal."inconsistenciasR4725VIH_".$cod_prestador."_".$fecha_actual."_".$tiempo_actual_string.'.zip';
		...
		//BOTONES DESCARGA
		$botones="";
		$botones.=" <input type=\'button\' value=\'Descargar archivo de inconsistencias para VIH\'  class=\'btn btn-success color_boton\' onclick=\"download_inconsistencias_campos(\'$ruta_zip\');\"/> ";
		...
		try
		{
			...
			$mail->FromName = "GIOSS";
			$mail->Subject = "Inconsistencias VIH R4725 ";
