...
$smarty->display('conseapb4725.html.tpl');
...
{	
	...
	if($_POST["tipo_busqueda"]=="1" && isset($_POST["dpto"]) && $_POST["dpto"]!="none")
	{
		...
		$sql_numero_registros.="SELECT count(*) as contador FROM gioss_tabla_estado_informacion_r4725_sida WHERE codigo_eapb='$cod_eapb' AND ";
		...
		$sql_vista_estado_informacion.="CREATE OR REPLACE VIEW veinf4725_".$nick_user."_".$tipo_id."_".$identificacion." ";
		$sql_vista_estado_informacion.=" AS SELECT * FROM gioss_tabla_estado_informacion_r4725_sida WHERE  codigo_eapb='$cod_eapb' AND ";
		...
		$sql_vista_estado_informacion.=" order by fecha_corte,numero_secuencia ; ";
		$error_bd_seq="";
		...
		while($contador_offset<$numero_registros)
		{
			...
			$sql_query_busqueda="";
			$sql_query_busqueda.="SELECT * FROM veinf4725_".$nick_user."_".$tipo_id."_".$identificacion." LIMIT $limite OFFSET $contador_offset;  ";
			$resultado_query_inconsistencias=$coneccionBD->consultar2($sql_query_busqueda);
		
			if(count($resultado_query_inconsistencias)>0)
			{
				
				$nombre_archivo_estado_informacion=$cod_eapb."_estado_info_4725.csv";
				...
				if($hubo_resultados==false)
				{
					...
					$titulos="";
					$titulos.="consecutivo,codigo_estado_informacion,nombre_estado_informacion,fecha_validacion,";
					$titulos.="fecha_corte,numero_secuencia,codigo_eapb,nombre_eapb,codigo_prestador_servicios,tipo_identificacion_prestador,";
					$titulos.="numero_identificacion_prestado,nombre_del_archivo,total_registros,codigo_departamento,codigo_municipio";
				...
				foreach($resultado_query_inconsistencias as $resultado)
				{
					...
					$linea_estado_informacion.=$resultado["fecha_validacion"].",";
					$linea_estado_informacion.=$resultado["fecha_corte"].",".$resultado["numero_secuencia"].",".$resultado["codigo_eapb"].",";
					...
					$linea_estado_informacion.=$resultado["numero_identificacion_prestador"].",".$resultado["nombre_del_archivo"].",".$resultado["total_registros"].",";
					$linea_estado_informacion.=$resultado["codigo_departamento"].",".$resultado["codigo_municipio"];
		...
		if($hubo_resultados)
		{
			//borrando vistas	
			$sql_borrar_vistas="";
			$sql_borrar_vistas.=" DROP VIEW veinf4725_".$nick_user."_".$tipo_id."_".$identificacion." ; ";
			$error_bd="";		
			...
			$boton_ventana=" <input type=\'button\' value=\'Ver el estado informacion para 0247\'  class=\'btn btn-success color_boton\' onclick=\"re_abrir_nueva_ventana();\"/> ";				
			
				
			$boton_descarga=" <input type=\'button\' value=\'Descargar archivo con el estado informacion para PyP 4725\'  class=\'btn btn-success color_boton\' onclick=\"download_archivo(\'$ruta_archivo_estado_informacion\');\"/> ";
	...
	if($_POST["tipo_busqueda"]=="2" && isset($_POST["prestador"]) )
	{
		...
		$sql_numero_registros="";
		$sql_numero_registros.="SELECT count(*) as contador FROM gioss_tabla_estado_informacion_r4725_sida WHERE codigo_eapb='$cod_eapb'  ";
		...
		$sql_vista_estado_informacion="";
		$sql_vista_estado_informacion.="CREATE OR REPLACE VIEW veinf4725_".$nick_user."_".$tipo_id."_".$identificacion." ";
		$sql_vista_estado_informacion.=" AS SELECT * FROM gioss_tabla_estado_informacion_r4725_sida WHERE codigo_eapb='$cod_eapb'  ";
		if($prestador!="none")
		{
			$sql_vista_estado_informacion.=" AND ";
		...
		if($error_bd_seq!="")
		{
			echo "<script>alert(' hubo error $numero_registros ".preg_replace("/[^A-Za-z0-9]/","",$error_bd_seq)."');</script>";
		}
		//echo "<script>alert('se genero vista $numero_registros');</script>";
	...
	if($_POST["tipo_busqueda"]=="3" && isset($_POST["year_de_corte"]) )
	{
		...
		$sql_numero_registros.="SELECT count(*) as contador FROM gioss_tabla_estado_informacion_r4725_sida WHERE codigo_eapb='$cod_eapb'  ";		
		$sql_numero_registros.=" AND ";
		$sql_numero_registros.=" (fecha_corte BETWEEN '$fecha_inicio' AND '$fecha_fin' ) ";
		$sql_numero_registros.=" ; ";
		$resultado_query_numero_registros=$coneccionBD->consultar2($sql_numero_registros);
		$numero_registros=intval($resultado_query_numero_registros[0]["contador"]);
		...
		$sql_vista_estado_informacion.=" AS SELECT * FROM gioss_tabla_estado_informacion_r4725_sida WHERE codigo_eapb='$cod_eapb'  ";
		$sql_vista_estado_informacion.=" AND ";
		$sql_vista_estado_informacion.=" (fecha_corte BETWEEN '$fecha_inicio' AND '$fecha_fin' ) ";
		$sql_vista_estado_informacion.=" order by fecha_corte,numero_secuencia ; ";
		$error_bd_seq="";
		$bool_hubo_error_query=$coneccionBD->insertar_no_warning_get_error($sql_vista_estado_informacion, $error_bd_seq);
		//echo "<script>alert('se genero vista $numero_registros');</script>";
