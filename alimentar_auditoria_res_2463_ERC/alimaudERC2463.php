...
$smarty->display('alimaudERC2463.html.tpl');


/*
INSERT INTO gios_menus_opciones_sistema(id_principal,id_padre,nombre_opcion,descripcion_ayuda,tiene_submenus,ruta_interfaz,prioridad_jerarquica) VALUES ('237','131','Alimentar Auditoria','',FALSE,'..|alimentar_auditoria_res_2463_ERC|alimaudERC2463.php','50.02');

INSERT INTO gios_menus_perfiles(id_menu,id_perfil) VALUES ('237','5'); --admin sistema
*/
...
)//fin if
{
	...
	--TABLA INDICE PERIODOS USUARIOS REPORTADOS erc (AUN NO HAY TABLA DE USUARIOS REPORTADOS PARA ESTA NORMA)
	CREATE TABLE tabla_aud_us_rep_erc_indice
	(
	...
	*/
	...
	$query_busca_ultima_fecha_reporte_obligatorio="SELECT max(fecha_corte_reporte) as maxima_fecha_corte from gioss_consulta_reporte_obligatorio_erc2463_exitoso WHERE codigo_entidad_eapb_generadora='$codigo_entidad_reporta';";
	...
	$query_buscar_ultimo_periodo_en_tabla_usuarios_reportados_cohorte="SELECT max(periodo_reportado::numeric) as maximo_periodo from tabla_auditoria_usuarios_reportados_erc WHERE codigo_entidad_reporta='$codigo_entidad_reporta' ;";
	...
	$query_verifica_si_se_proceso_para_el_periodo_indicado="SELECT * FROM tabla_aud_us_rep_erc_indice WHERE periodo_reportado='$periodo_indicado' AND codigo_entidad_reporta='$codigo_entidad_reporta' ; ";
	...
	{
		$query_delete_registros_proceso_incompleto="DELETE * FROM tabla_auditoria_usuarios_reportados_erc WHERE periodo_reportado='$periodo_indicado' AND codigo_entidad_reporta='$codigo_entidad_reporta' ;";
		$error_bd_seq="";
	...
	if($fecha_corte_ultimo_periodo!="" && $periodo_anterior!="" && $bool_fue_diligenciado==false)
	{
		...
		$nombre_view_ultimo_reporte_obligatorio_generado="valim2463aud_".$nick_user.str_replace("-", "", $fecha_corte_ultimo_periodo);
		$query_view_ultimo_reporte_obligatorio="CREATE OR REPLACE VIEW $nombre_view_ultimo_reporte_obligatorio_generado AS 
		( 
			SELECT * FROM gioss_consulta_reporte_obligatorio_erc2463_exitoso WHERE fecha_corte_reporte='$fecha_corte_ultimo_periodo' ORDER BY numero_registro ASC
		);
		";
		$error_bd_seq="";
		 ...
		 )//fin if
		{
			...
			while($contador_offset<$numero_registros)
			{
				...
				{
					$valor_campo_23_erc=trim($registro['campo_erc_de_numero_orden_23']);//cambia 
					$valor_campo_24_erc=trim($registro['campo_erc_de_numero_orden_24']);//cambia
					$tipo_identificacion_campo_5_erc=trim($registro['campo_erc_de_numero_orden_5']);//ya que erc en bd cuanta desde uno
					$numero_identificacion_campo_6_erc=trim($registro['campo_erc_de_numero_orden_6']);//ya que erc en bd cuanta desde uno

					$array_campos_registro=array();
					$contador_campos=0;
					while(isset($registro['campo_erc_de_numero_orden_'.$contador_campos])==true)
					{
						$array_campos_registro[$contador_campos]=trim($registro['campo_erc_de_numero_orden_'.$contador_campos]);
						$contador_campos++;
					}//fin while

					//aqui va query busqueda si fue reportado previamente el usuario en tabla_auditoria_usuarios_reportados_erc
					$query_buscar_paciente="SELECT * FROM tabla_auditoria_usuarios_reportados_erc WHERE campo_erc_5='$tipo_identificacion_campo_5_erc' AND campo_erc_6='$numero_identificacion_campo_6_erc' AND periodo_reportado='$periodo_anterior' ;";
					...
					{
						//ANTERIORES
						if($valor_campo_23_erc=="0" || $valor_campo_23_erc=="1")
						{
							//COHORETE HEMOFILIA
							$tipo_cohorte_auditoria_para_insert="2";
						}//fin if
						else if(
							$valor_campo_23_erc=="2"
							|| $valor_campo_23_erc=="3"
							|| $valor_campo_23_erc=="4"
							|| $valor_campo_23_erc=="5"
							|| $valor_campo_23_erc=="6"
							|| $valor_campo_23_erc=="7"
							|| $valor_campo_23_erc=="8"
							|| $valor_campo_23_erc=="9"
							|| $valor_campo_23_erc=="10"
							|| $valor_campo_23_erc=="11"
						)//fin if
						{
							//OTRAS COAGULOPATIAS
						...
						$valor_campo_23_usuarios_reportados_erc=trim($resultados_buscar_paciente['campo_erc_22']);
						$valor_campo_24_usuarios_reportados_erc=trim($resultados_buscar_paciente['campo_erc_23']);
						if($valor_campo_23_usuarios_reportados_erc!=$valor_campo_23_erc)
						{
							//I. definicion tecnica cambio de diagnostico
							$tipo_cohorte_auditoria_para_insert="5";
						}//fin if
						else if(
							$valor_campo_23_usuarios_reportados_erc==$valor_campo_23_erc 
							&& $valor_campo_24_erc!=$valor_campo_24_usuarios_reportados_erc
						)//fin condicion
						{
							if($valor_campo_23_erc=="0" || $valor_campo_23_erc=="1")
							{
					...
					*/
					...
					)
					{
						...
						$query_preparacion_insert.="INSERT INTO tabla_auditoria_usuarios_reportados_erc
							(
							";
						$cont_campos_ins_hf=$inicializa_contador_campos_en;
						while ($cont_campos_ins_hf<$numero_total_campos_norma) 
						{
							$query_preparacion_insert.="campo_erc_".$cont_campos_ins_hf.",";
							$cont_campos_ins_hf++;
						}//fin while
						$query_preparacion_insert.="
						periodo_reportado,
						...
						while ($cont_campos_ins_hf<$numero_total_campos_norma) 
						{
							$query_preparacion_insert.="'".trim($registro['campo_erc_de_numero_orden_'.$cont_campos_ins_hf])."',";
							$cont_campos_ins_hf++;
						}//fin while
						$query_preparacion_insert.="
						'".$periodo_reportado_para_insert."',
			...
			{
				$query_insert_indice="";
				$query_insert_indice.="INSERT INTO tabla_aud_us_rep_erc_indice
				(
				periodo_reportado,
				fecha_de_corte,
				codigo_entidad_reporta,
