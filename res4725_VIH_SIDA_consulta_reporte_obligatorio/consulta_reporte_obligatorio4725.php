...
$smarty->display('consulta_reporte_obligatorio4725.html.tpl');
   ...
   )
{
	...
	if($_POST["tipo_consulta_radio"]=="detallado")
	{
		...
		$query_verificar_nombre_archivo="SELECT * FROM gioss_archivos_obligatorios_reportados_sida WHERE codigo_entidad_eapb_generadora='$codigo_eapb' AND nombre_archivo_vih_sida='$nombre_zip_sin_consecutivo' ;";
		...
		{
			$existe_validacion=false;
			//echo "<script>document.getElementById('mensaje_div').innerHTML='No se hallaron archivos 4725 reportados.';</script>";
	...
	if($existe_validacion==true && $valor_radio=="detallado")
	{
		...
		{
			$sql_vista_consulta_reporte_obligatorio.=" AS SELECT * FROM gioss_consulta_reporte_obligatorio_sida4725_exitoso WHERE codigo_entidad_eapb_generadora='$codigo_eapb' AND nombre_archivo_vih_sida='$nombre_zip_sin_consecutivo' ORDER BY numero_secuencia asc, numero_registro asc  ; ";
		}
		else if($estado_validacion_seleccionado=="rechazada")
		{
			$sql_vista_consulta_reporte_obligatorio.=" AS SELECT * FROM gioss_consulta_reporte_obligatorio_sida4725_rechazado WHERE codigo_entidad_eapb_generadora='$codigo_eapb' AND nombre_archivo_vih_sida='$nombre_zip_sin_consecutivo' ORDER BY numero_secuencia asc, numero_registro asc  ; ";
		}
		$error_bd_seq="";
		...
		if($flag_creacion_archivo)
		{
			...
			$boton_descarga=" <input type=\'button\' value=\'Descargar el reporte obligatorio almacenado para  vih 4725\'  class=\'btn btn-success color_boton\' onclick=\"download_inconsistencias_campos(\'$ruta_zip\');\"/> ";
			...
			if (!$mail->Send()) 
			    ...
			    // echo "Mensaje enviado.";
					    echo "<script>alert('Se ha enviado una copia del comprimido del reporte obligatorio de 4725 a su correo $correo_electronico ')</script>";
			}
	...
	if($valor_radio=="consolidado" && $estado_validacion_seleccionado=="validada")
	{
		...
		$query_consolidado_rep_oblig_sida="";
		$query_consolidado_rep_oblig_sida.="SELECT * FROM gioss_archivos_obligatorios_reportados_sida WHERE codigo_entidad_eapb_generadora='$codigo_eapb' ";
		$query_consolidado_rep_oblig_sida.=" AND (fecha_de_generacion BETWEEN '$fecha_ini_bd' AND '$fecha_fin_bd') ";
		$query_consolidado_rep_oblig_sida.=" AND estado_informacion='1' ;";
		
		$resultado_consolidado_rep_oblig_sida=$coneccionBD->consultar2($query_consolidado_rep_oblig_sida);
		
		if(count($resultado_consolidado_rep_oblig_sida)>0)
		{
			$nombre_consolidado_rep_oblig_cancer=$codigo_eapb."_".$fecha_ini_bd."_".$fecha_fin_bd."_consulta_cons_rep_oblig_cancer.csv";
			$ruta_consolidado_rep_oblig_sida=$rutaTemporal.$nombre_consolidado_rep_oblig_cancer;
			
			$file_consolidado_rep_oblig= fopen($ruta_consolidado_rep_oblig_sida, "w") or die("fallo la creacion del archivo");
			...
			$file_consolidado_rep_oblig= fopen($ruta_consolidado_rep_oblig_sida, "a") or die("fallo la creacion del archivo");
			...
			foreach($resultado_consolidado_rep_oblig_sida as $resultado)
			{
				...
				$linea_estado_informacion.=$resultado["nombre_archivo_vih_sida"].",";
			...
			$boton_ventana=" <input type=\'button\' value=\'Ver el consolidado de reporte obligatorio para VIH SIDA \'  class=\'btn btn-success color_boton\' onclick=\"re_abrir_nueva_ventana();\"/> ";	
			
			$boton_descarga=" <input type=\'button\' value=\'Descargar el consolidado de reporte obligatorio para VIH SIDA \'  class=\'btn btn-success color_boton\' onclick=\"download_archivo(\'$ruta_consolidado_rep_oblig_sida\');\"/> ";
