...
$smarty->display('consulta_reporte_obligatorio2463.html.tpl');
   ...
   )
{
	...
	if($_POST["tipo_consulta_radio"]=="detallado")
	{
		...
		$query_verificar_nombre_archivo="SELECT * FROM gioss_archivos_obligatorios_reportados_erc WHERE codigo_entidad_eapb_generadora='$codigo_eapb' AND nombre_archivo_erc='$nombre_zip_sin_consecutivo' ;";
	...
	if($existe_validacion==true && $valor_radio=="detallado")
	{
		...
		{
			$sql_vista_consulta_reporte_obligatorio.=" AS SELECT * FROM gioss_consulta_reporte_obligatorio_erc2463_exitoso WHERE codigo_entidad_eapb_generadora='$codigo_eapb' AND nombre_archivo_erc='$nombre_zip_sin_consecutivo' ORDER BY numero_secuencia asc, numero_registro asc  ; ";
		}
		else if($estado_validacion_seleccionado=="rechazada")
		{
			$sql_vista_consulta_reporte_obligatorio.=" AS SELECT * FROM gioss_consulta_reporte_obligatorio_erc2463_rechazado WHERE codigo_entidad_eapb_generadora='$codigo_eapb' AND nombre_archivo_erc='$nombre_zip_sin_consecutivo' ORDER BY numero_secuencia asc, numero_registro asc  ; ";
		}
		$error_bd_seq="";
		...
		if($flag_creacion_archivo)
		{
			...
			$boton_descarga=" <input type=\'button\' value=\'Descargar el reporte obligatorio almacenado para  erc 2463\'  class=\'btn btn-success color_boton\' onclick=\"download_inconsistencias_campos(\'$ruta_zip\');\"/> ";
			...
			if (!$mail->Send()) 
			    ...
			    // echo "Mensaje enviado.";
					    echo "<script>alert('Se ha enviado una copia del comprimido del reporte obligatorio de 2463 a su correo $correo_electronico ')</script>";
			}
	...
	if($valor_radio=="consolidado" && $estado_validacion_seleccionado=="validada")
	{
		...
		$query_consolidado_rep_oblig_sida.="SELECT * FROM gioss_archivos_obligatorios_reportados_erc WHERE codigo_entidad_eapb_generadora='$codigo_eapb' ";
		...
		if(count($resultado_consolidado_rep_oblig_sida)>0)
		{
			...
			foreach($resultado_consolidado_rep_oblig_sida as $resultado)
			{
				...
				$linea_estado_informacion.=$resultado["nombre_archivo_erc"].",";
			...
			$boton_ventana=" <input type=\'button\' value=\'Ver el consolidado de reporte obligatorio para ERC \'  class=\'btn btn-success color_boton\' onclick=\"re_abrir_nueva_ventana();\"/> ";	
			
			$boton_descarga=" <input type=\'button\' value=\'Descargar el consolidado de reporte obligatorio para ERC \'  class=\'btn btn-success color_boton\' onclick=\"download_archivo(\'$ruta_consolidado_rep_oblig_sida\');\"/> ";
