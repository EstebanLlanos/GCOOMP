...
$smarty->display('eapbRIPS3374.html.tpl');
...
{	
	...
	if($_POST["tipo_busqueda"]=="1" && isset($_POST["dpto"]) && $_POST["dpto"]!="none")
	{
		//POR AREA GEOGRAFICA
		$dpto=$_POST["dpto"];
		$mpio=$_POST["mpio"];
		...
		$sql_numero_registros.="SELECT count(*) as contador FROM gioss_tabla_estado_informacion_rips WHERE codigo_eapb='$cod_eapb' AND ";
		$sql_numero_registros.=" codigo_departamento='$dpto' ";
		if($mpio!="none")
		{
			$sql_numero_registros.=" AND ";
			$sql_numero_registros.=" codigo_municipio='$mpio' ";
		}
		$sql_numero_registros.=" ; ";
		...
		$sql_vista_estado_informacion.="CREATE OR REPLACE VIEW veir_".$nick_user."_".$tipo_id."_".$identificacion." ";
		$sql_vista_estado_informacion.=" AS SELECT * FROM gioss_tabla_estado_informacion_rips WHERE  codigo_eapb='$cod_eapb' AND ";
		$sql_vista_estado_informacion.=" codigo_departamento='$dpto' ";
		if($mpio!="none")
		{
			$sql_vista_estado_informacion.=" AND ";
			$sql_vista_estado_informacion.=" codigo_municipio='$mpio' ";
		}
		$sql_vista_estado_informacion.=" order by fecha_validacion,numero_secuencia ; ";
		$error_bd_seq="";
		...
		while($contador_offset<$numero_registros)
		{
			...
			$sql_query_busqueda="";
			$sql_query_busqueda.="SELECT * FROM veir_".$nick_user."_".$tipo_id."_".$identificacion." LIMIT $limite OFFSET $contador_offset;  ";
			$resultado_query_inconsistencias=$coneccionBD->consultar2($sql_query_busqueda);
		
			if(count($resultado_query_inconsistencias)>0)
			{
				
				$nombre_archivo_estado_informacion=$cod_eapb."_estado_info_rips.csv";
				...
				if($hubo_resultados==false)
				{
					...
					$titulos="";
					$titulos.="N. Orden,Cod. Estado,Nombre Estado,Fech Validacion,Fech Remision,N. Secuencia,Cod. EAPB,";
					$titulos.="Nombre EAPB,Cod. Prestador,Nombre Prestador,T.I.,N. Indetifi.,";
					$titulos.="Nombre CT,Valor Fact.,N. Reg. AF,N. Reg Us,N. Reg. AC,N. Reg. AP,N. Reg. AU,N. Reg. AH,";
					$titulos.="N. Reg. AN,N. Reg. AM,N. Reg. AT,Cod. Dept.,Nom. Dept.,Cod. Mpio.,Nom. Mpio.";
				...
				foreach($resultado_query_inconsistencias as $resultado)
				{
					...
					$linea_estado_informacion.=$resultado["fecha_validacion"].",".$resultado["fecha_remision"].",".$resultado["numero_secuencia"].",".$resultado["codigo_eapb"].",";
					$linea_estado_informacion.=$resultado["nombre_eapb"].",".$resultado["codigo_prestador_servicios"].",".$resultado["nombre_prestador_servicios"].",";
					$linea_estado_informacion.=$resultado["tipo_identificacion_prestador"].",".$resultado["numero_identificacion_prestador"].",".$resultado["nombre_del_archivo_ct"].",";
					$linea_estado_informacion.=$resultado["valor_neto_a_pagar"].",".$resultado["numero_registros_af"].",".$resultado["numero_registros_us"].",";
					$linea_estado_informacion.=$resultado["numero_registros_ac"].",".$resultado["numero_registros_ap"].",".$resultado["numero_registros_au"].",";
					$linea_estado_informacion.=$resultado["numero_registros_ah"].",".$resultado["numero_registros_an"].",".$resultado["numero_registros_am"].",";
					$linea_estado_informacion.=$resultado["numero_registros_at"].",".$resultado["codigo_departamento"].",".$resultado["nombre_del_departamento"].",";
					...
					foreach($array_linea_estado_informacion as $columna_estado_informacion)
					{
						...
						&& $cont_columnas_estado_informacion!=6
						&& $cont_columnas_estado_informacion!=10
						&& $cont_columnas_estado_informacion!=11
						&& $cont_columnas_estado_informacion!=23
						&& $cont_columnas_estado_informacion!=25
						)
						{
		...
		if($hubo_resultados)
		{
			...
			$sql_borrar_vistas="";
			$sql_borrar_vistas.=" DROP VIEW veir_".$nick_user."_".$tipo_id."_".$identificacion." ; ";
			$error_bd="";		
			...
			$boton_ventana=" <input type=\'button\' value=\'Ver el estado informacion para RIPS\'  class=\'btn btn-success color_boton\' onclick=\"re_abrir_nueva_ventana();\"/> ";	
			
				
			$boton_descarga=" <input type=\'button\' value=\'Descargar archivo con el estado informacion para RIPS\'  class=\'btn btn-success color_boton\' onclick=\"download_archivo(\'$ruta_archivo_estado_informacion\');\"/> ";
		...
		else
		{
			...
			echo "<script>document.getElementById('mensaje_div').innerHTML='No se hallaron coincidencias por medio del area geografica especificada.';</script>";
		}		
		
		
	}//fin if
	if($_POST["tipo_busqueda"]=="2" && isset($_POST["prestador"]) )
	{
		//POR PRESTADOR
		$prestador=$_POST["prestador"];
				
		$sql_numero_registros="";
		$sql_numero_registros.="SELECT count(*) as contador FROM gioss_tabla_estado_informacion_rips WHERE codigo_eapb='$cod_eapb'  ";
		
		if($prestador!="none")
		{
			$sql_numero_registros.=" AND ";
			$sql_numero_registros.=" codigo_prestador_servicios='$prestador' ";
		}
		...
		$sql_vista_estado_informacion="";
		$sql_vista_estado_informacion.="CREATE OR REPLACE VIEW veir_".$nick_user."_".$tipo_id."_".$identificacion." ";
		$sql_vista_estado_informacion.=" AS SELECT * FROM gioss_tabla_estado_informacion_rips WHERE codigo_eapb='$cod_eapb'  ";
		if($prestador!="none")
		{
			$sql_vista_estado_informacion.=" AND ";
			$sql_vista_estado_informacion.=" codigo_prestador_servicios='$prestador' ";
		}
		$sql_vista_estado_informacion.=" order by fecha_validacion,numero_secuencia ; ";
		$error_bd_seq="";
		$bool_hubo_error_query=$coneccionBD->insertar_no_warning_get_error($sql_vista_estado_informacion, $error_bd_seq);
		...
		else
		{
			...
			echo "<script>document.getElementById('mensaje_div').innerHTML='No se hallaron coincidencias con dicho prestador/es.';</script>";
		}		
		
	}//fin if
	if($_POST["tipo_busqueda"]=="3" && isset($_POST["year_de_corte"]) )
	{
		//POR PERIODO
		...
		$sql_numero_registros.="SELECT count(*) as contador FROM gioss_tabla_estado_informacion_rips WHERE codigo_eapb='$cod_eapb'  ";		
		$sql_numero_registros.=" AND ";
		$sql_numero_registros.=" (fecha_validacion BETWEEN '$fecha_inicio' AND '$fecha_fin' ) ";
		$sql_numero_registros.=" ; ";
		$resultado_query_numero_registros=$coneccionBD->consultar2($sql_numero_registros);
		$numero_registros=intval($resultado_query_numero_registros[0]["contador"]);
		...
		$sql_vista_estado_informacion.=" AS SELECT * FROM gioss_tabla_estado_informacion_rips WHERE codigo_eapb='$cod_eapb'  ";
		$sql_vista_estado_informacion.=" AND ";
		$sql_vista_estado_informacion.=" (fecha_validacion BETWEEN '$fecha_inicio' AND '$fecha_fin' ) ";
		$sql_vista_estado_informacion.=" order by fecha_validacion,numero_secuencia ; ";
		$error_bd_seq="";
		$bool_hubo_error_query=$coneccionBD->insertar_no_warning_get_error($sql_vista_estado_informacion, $error_bd_seq);
		//echo "<script>alert('se genero vista $numero_registros');</script>";
