...
$smarty->display('alimaudVIH4725.html.tpl');


/*
INSERT INTO gios_menus_opciones_sistema(id_principal,id_padre,nombre_opcion,descripcion_ayuda,tiene_submenus,ruta_interfaz,prioridad_jerarquica) VALUES ('238','12','Alimentar Auditoria','',FALSE,'..|alimentar_auditoria_res_4725_VIH|alimaudVIH4725.php','50.02');

INSERT INTO gios_menus_perfiles(id_menu,id_perfil) VALUES ('238','5'); --admin sistema
*/
...
)//fin if
{
	...
	alter table Tablagioss_tabla_registros_no_cargados_rechazados_r4725_sida add column campo_sida_de_numero_orden_185 character varying(320);
	...
	--TABLA INDICE PERIODOS USUARIOS REPORTADOS VIH (AUN NO HAY TABLA DEUSUARIOR REPORTADOS PARA ESTA NORMA)
	CREATE TABLE tabla_aud_us_rep_vih_indice
	(
	...
	*/
	...
	$query_busca_ultima_fecha_reporte_obligatorio="SELECT max(fecha_corte_reporte) as maxima_fecha_corte from gioss_consulta_reporte_obligatorio_sida4725_exitoso WHERE codigo_entidad_eapb_generadora='$codigo_entidad_reporta';";
	...
	$query_buscar_ultimo_periodo_en_tabla_usuarios_reportados_cohorte="SELECT max(periodo_reportado::numeric) as maximo_periodo from tabla_auditoria_usuarios_reportados_vih WHERE codigo_entidad_reporta='$codigo_entidad_reporta' ;";
	...
	$query_verifica_si_se_proceso_para_el_periodo_indicado="SELECT * FROM tabla_aud_us_rep_vih_indice WHERE periodo_reportado='$periodo_indicado' AND codigo_entidad_reporta='$codigo_entidad_reporta' ; ";
	...
	{
		$query_delete_registros_proceso_incompleto="DELETE * FROM tabla_auditoria_usuarios_reportados_vih WHERE periodo_reportado='$periodo_indicado' AND codigo_entidad_reporta='$codigo_entidad_reporta' ;";
		$error_bd_seq="";
	...
	if($fecha_corte_ultimo_periodo!="" && $periodo_anterior!="" && $bool_fue_diligenciado==false)
	{
		...
		$nombre_view_ultimo_reporte_obligatorio_generado="valim4725aud_".$nick_user.str_replace("-", "", $fecha_corte_ultimo_periodo);
		$query_view_ultimo_reporte_obligatorio="CREATE OR REPLACE VIEW $nombre_view_ultimo_reporte_obligatorio_generado AS 
		( 
			SELECT * FROM gioss_consulta_reporte_obligatorio_sida4725_exitoso WHERE fecha_corte_reporte='$fecha_corte_ultimo_periodo' ORDER BY numero_registro ASC
		);
		";
		$error_bd_seq="";
		 ...
		 )//fin if
		{
			...
			while($contador_offset<$numero_registros)
			{
				...
				{
					$valor_campo_23_vih=trim($registro['campo_vih_de_numero_orden_23']);//cambia 
					$valor_campo_24_vih=trim($registro['campo_vih_de_numero_orden_24']);//cambia
					$tipo_identificacion_campo_5_vih=trim($registro['campo_vih_de_numero_orden_5']);//ya que vih en bd cuanta desde uno
					$numero_identificacion_campo_6_vih=trim($registro['campo_vih_de_numero_orden_6']);//ya que vih en bd cuanta desde uno

					$array_campos_registro=array();
					$contador_campos=0;
					while(isset($registro['campo_vih_de_numero_orden_'.$contador_campos])==true)
					{
						$array_campos_registro[$contador_campos]=trim($registro['campo_vih_de_numero_orden_'.$contador_campos]);
						$contador_campos++;
					}//fin while

					//aqui va query busqueda si fue reportado previamente el usuario en tabla_auditoria_usuarios_reportados_vih
					$query_buscar_paciente="SELECT * FROM tabla_auditoria_usuarios_reportados_vih WHERE campo_vih_5='$tipo_identificacion_campo_5_vih' AND campo_vih_6='$numero_identificacion_campo_6_vih' AND periodo_reportado='$periodo_anterior' ;";
					...
					{
						//ANTERIORES
						if($valor_campo_23_vih=="0" || $valor_campo_23_vih=="1")
						{
							//COHORETE HEMOFILIA
							$tipo_cohorte_auditoria_para_insert="2";
						}//fin if
						else if(
							$valor_campo_23_vih=="2"
							|| $valor_campo_23_vih=="3"
							|| $valor_campo_23_vih=="4"
							|| $valor_campo_23_vih=="5"
							|| $valor_campo_23_vih=="6"
							|| $valor_campo_23_vih=="7"
							|| $valor_campo_23_vih=="8"
							|| $valor_campo_23_vih=="9"
							|| $valor_campo_23_vih=="10"
							|| $valor_campo_23_vih=="11"
						)//fin if
						{
							//OTRAS COAGULOPATIAS
						...
						$valor_campo_23_usuarios_reportados_vih=trim($resultados_buscar_paciente['campo_vih_22']);
						$valor_campo_24_usuarios_reportados_vih=trim($resultados_buscar_paciente['campo_vih_23']);
						if($valor_campo_23_usuarios_reportados_vih!=$valor_campo_23_vih)
						{
							//I. definicion tecnica cambio de diagnostico
							$tipo_cohorte_auditoria_para_insert="5";
						}//fin if
						else if(
							$valor_campo_23_usuarios_reportados_vih==$valor_campo_23_vih 
							&& $valor_campo_24_vih!=$valor_campo_24_usuarios_reportados_vih
						)//fin condicion
						{
							if($valor_campo_23_vih=="0" || $valor_campo_23_vih=="1")
							{
					...
					*/
					...
					)
					{
						...
						$query_preparacion_insert.="INSERT INTO tabla_auditoria_usuarios_reportados_vih
							(
							";
						$cont_campos_ins_hf=$inicializa_contador_campos_en;
						while ($cont_campos_ins_hf<$numero_total_campos_norma) 
						{
							$query_preparacion_insert.="campo_vih_".$cont_campos_ins_hf.",";
							$cont_campos_ins_hf++;
						}//fin while
						$query_preparacion_insert.="
						periodo_reportado,
						...
						while ($cont_campos_ins_hf<$numero_total_campos_norma) 
						{
							$query_preparacion_insert.="'".trim($registro['campo_vih_de_numero_orden_'.$cont_campos_ins_hf])."',";
							$cont_campos_ins_hf++;
						}//fin while
						$query_preparacion_insert.="
						'".$periodo_reportado_para_insert."',
			...
			{
				$query_insert_indice="";
				$query_insert_indice.="INSERT INTO tabla_aud_us_rep_vih_indice
				(
				periodo_reportado,
				fecha_de_corte,
				codigo_entidad_reporta,
