...
require_once 'reparacion_duplicados_por_txt.php';
...
if(isset($_POST["year_de_corte"]) && isset($_POST['eapb']) && $_POST['eapb']!="none" && $_POST["year_de_corte"]!="" && ctype_digit($_POST["year_de_corte"]) )
{
	...
	$consecutivo="";
	//$consecutivo="01";
	//debe ser con los separadores para alto costo
	$nombre_archivo_para_zip=$array_fcbd[2].$array_fcbd[0].$array_fcbd[1]."_".$string_cod_eapb."_"."CANCER";
	if($tipo_tiempo_periodo=="anual")
	{
		$nombre_archivo_para_zip=$nombre_archivo_para_zip.$consecutivo;
	}
	else if($tipo_tiempo_periodo=="semestral")
	{
		$nombre_archivo_para_zip=$nombre_archivo_para_zip.$consecutivo."S";
	}
	else if($tipo_tiempo_periodo=="mensual")
	{
		$nombre_archivo_para_zip=$nombre_archivo_para_zip.$consecutivo."M";
	}
	...
	if($ya_se_reporto_archivo==false)
	{
					...
					foreach($resultados_query_cancer0247_duplicados as $duplicado_actual)
					{
						...
						if(count($array_campos_del_duplicado_del_afiliado)==210)
						{
							...
							while($numero_actual_campo_registro_corregido<210)
							{
								...
								else if($numero_actual_campo_registro_corregido==5)
								{
								    ...
								    {
									echo "<script>alert('numero registro: ".$numero_linea_dupl." los numero id son diferentes del array: $num_id_temp_del_array de la tabla indexador:$numero_id_duplicado_actual lista: $lista_string_filas_donde_esta_duplicado');</script>";
								    }
								}
							...
							$tipo_id_temp_del_array=trim(alphanumericAndSpace4($array_campos_del_duplicado_del_afiliado[4]));
							$num_id_temp_del_array=trim(alphanumericAndSpace4($array_campos_del_duplicado_del_afiliado[5]));							
							$diagnostico_temp_del_array=trim(alphanumericAndSpace4($array_campos_del_duplicado_del_afiliado[16]));
							if($num_id_temp_del_array==$numero_id_duplicado_actual
							&& $tipo_id_temp_del_array==$tipo_id_duplicado_actual
							&& $diagnostico_temp_del_array==$diagnostico_duplicado_actual
							...
							else 
							{
								...
								$mensaje_verificador_id_dupl.="(para txt) numero registro: ".$numero_linea_dupl;
					    ...
					    if($bool_ya_se_proceso==false)
					    {
							...
							$array_campos_procesados_de_los_duplicados_del_duplicado=reparacion_duplicados_por_txt($tipo_id_duplicado_actual,
																	    ...
																	    $identificacion,
																	    $tipo_id,
																	    $numero_filas_donde_esta_afiliado_actual,
		...
		{
			$sql_vista_consulta_reporte_obligatorio.=" AS SELECT * FROM gioss_consulta_reporte_obligatorio_cancer0247_exitoso ";
		}
		else if($estado_validacion_seleccionado=="rechazada")
		{
			$sql_vista_consulta_reporte_obligatorio.=" AS SELECT * FROM gioss_consulta_reporte_obligatorio_cancer0247_rechazado ";
		}		
		$sql_vista_consulta_reporte_obligatorio.=" WHERE codigo_entidad_eapb_generadora='$cod_eapb' AND nombre_archivo_cancer='$nombre_archivo_para_zip' ";		
